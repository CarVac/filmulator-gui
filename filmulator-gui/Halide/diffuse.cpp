Func performBlur(Func f, Func coeff, Expr size, Expr sigma)
{
  ...
  blurred(x, 1) = (coeff(0) * f(x, 1) + coeff(1) * blurred(x, 0));
  blurred(x, 2) = (coeff(0) * f(x, 2) + coeff(1) * blurred(x, 1) + coeff(2) * blurred(x, 0));

  // Top to bottom
  RDom fwd(3, size - 3);
  blurred(x, fwd) = (coeff(0) * f(x, fwd) + coeff(1) * blurred(x, fwd - 1) + coeff(2) * blurred(x, fwd - 2)
  ...
  blurred(x, tail) =
    (coeff(1) * blurred(x, tail - 1) + coeff(2) * blurred(x, tail - 2) + coeff(3) * blurred(x, tail - 3));

  // Bottom to top
  Expr last = size + padding - 1;
  RDom backwards(0, last - 2);
  Expr b = last - 3 - backwards;// runs from last - 3 down to zero
  blurred(x, b) = (coeff(0) * blurred(x, b) + coeff(1) * blurred(x, b + 1) + coeff(2) * blurred(x, b + 2)
...
Func blur_then_transpose(Func f, Func coeff, Expr size, Expr sigma)
{
  ...
  out.compute_root().tile(x, y, xi, yi, 8, 32).tile(xi, yi, xii, yii, 8, 8).vectorize(xii).unroll(yii).parallel(y);
  ...
  for (int i = 0; i < blurred.num_update_definitions(); i++) {
    ...
    if (r.defined()) { blurred.update(i).reorder(x, r); }
...
Func blur(Func input, Expr sigma, Expr width, Expr height)
{
  ...
  Func coeff;
  Expr q = select(sigma < 2.5f, 3.97156f - 4.14554f * sqrt(1 - 0.26891f * sigma), 0.98711f * sigma - 0.96330f);
