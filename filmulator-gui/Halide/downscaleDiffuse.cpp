...
Func upscale(Func input, Expr radius)
{
  ...
  output(x, y) = lerp(lerp(Ix0y0, Ix1y0, xf), lerp(Ix0y1, Ix1y1, xf), yf);
  return output;
}
...
{
  Func blurx, blury;
  blurx(x, y) = (input(x - 2, y) + input(x - 1, y) * 4 + input(x, y) * 6 + input(x + 1, y) * 4 + input(x + 2, y));
  blury(x, y) = (blurx(x, y - 2) + blurx(x, y - 1) * 4 + blurx(x, y) * 6 + blurx(x, y + 1) * 4 + blurx(x, y + 2));
  return blury;
}
...
int main(int argc, char **argv)
{
  ...
  lambda(
    x, y, select(((x - 500) * (x - 500) + (y - 500) * (y - 500)) < 100 * 100, cast<uint8_t>(255), cast<uint8_t>(0)))
    .realize(input_image);
