...
public:
  ...
  Pipeline build()
  {
    ...
    Func developed;
    developed = develop(filmulationData,
      crystalGrowthConst,
    ...
    initialDeveloperMirrored =
      BoundaryConditions::mirror_interior(initialDeveloper, 0, input.width(), 0, input.height());
    Expr pixelsPerMillimeter = sqrt(input.width() * input.height() / filmArea);
    diffused = diffuse(initialDeveloper, sigmaConst, pixelsPerMillimeter, stepTime, input.width(), input.height());
    ...
    filmulationDataOut(x, y, c) = select(c == DEVEL_CONC && doDiffuse == 1, layerMixed(x, y), developed(x, y, c));
    Func reservoirConcentrationOut;
    reservoirConcentrationOut(x) = select(doDiffuse == 1, newReservoirConcentration(x), reservoirConcentration);
