...
namespace Internal {
  ...
  //@{
  template<typename T> std::vector<T> vec(T a)
  {
    std::vector<T> v(1);
    v[0] = a;
    return v;
  ...
  template<typename T> std::vector<T> vec(T a, T b)
  {
    std::vector<T> v(2);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c)
  {
    std::vector<T> v(3);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c, T d)
  {
    std::vector<T> v(4);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c, T d, T e)
  {
    std::vector<T> v(5);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c, T d, T e, T f)
  {
    std::vector<T> v(6);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c, T d, T e, T f, T g)
  {
    std::vector<T> v(7);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c, T d, T e, T f, T g, T h)
  {
    std::vector<T> v(8);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c, T d, T e, T f, T g, T h, T i)
  {
    std::vector<T> v(9);
    v[0] = a;
    v[1] = b;
  ...
  template<typename T> std::vector<T> vec(T a, T b, T c, T d, T e, T f, T g, T h, T i, T j)
  {
    std::vector<T> v(10);
    v[0] = a;
    v[1] = b;
  ...
  template<typename DstType, typename SrcType> DstType reinterpret_bits(const SrcType &src)
  {
...
namespace Halide {
namespace Internal {
  static bool check_introspection(const void *var,
    const std::string &type,
    const std::string &correct_name,
    const std::string &correct_file,
    int line)
  {
...
struct A
{
  ...
  public:
    float a_float;
    A *parent;
    B() : private_member(17) { a_float = private_member * 2.0f; }
  };
...
static bool test_a(const A &a, const std::string &my_name)
{
  ...
  success &= Halide::Internal::check_introspection(
    &a.a_b, "HalideIntrospectionCanary::A::B", my_name + ".a_b", __FILE__, __LINE__);
  success &= Halide::Internal::check_introspection(
    &a.a_b.parent, "HalideIntrospectionCanary::A *", my_name + ".a_b.parent", __FILE__, __LINE__);
  success &=
    Halide::Internal::check_introspection(&a.a_b.a_float, "float", my_name + ".a_b.a_float", __FILE__, __LINE__);
  success &=
    Halide::Internal::check_introspection(a.a_b.parent, "HalideIntrospectionCanary::A", my_name, __FILE__, __LINE__);
  return success;
}
...
namespace {
  ...
  {
    TestCompilationUnit() { Halide::Internal::test_compilation_unit(&test, &offset_marker); }
  };
}// namespace
...
struct Type
{
  ...
  bool operator==(const Type &other) const { return code == other.code && bits == other.bits && width == other.width; }
...
namespace {
  template<typename T> struct type_of_helper;

  template<typename T> struct type_of_helper<T *>
  {
    operator Type() { return Handle(); }
  };

  template<> struct type_of_helper<float>
  {
    operator Type() { return Float(32); }
  };

  template<> struct type_of_helper<double>
  {
    operator Type() { return Float(64); }
  };

  template<> struct type_of_helper<uint8_t>
  {
    operator Type() { return UInt(8); }
  };

  template<> struct type_of_helper<uint16_t>
  {
    operator Type() { return UInt(16); }
  };

  template<> struct type_of_helper<uint32_t>
  {
    operator Type() { return UInt(32); }
  };

  template<> struct type_of_helper<uint64_t>
  {
    operator Type() { return UInt(64); }
  };

  template<> struct type_of_helper<int8_t>
  {
    operator Type() { return Int(8); }
  };

  template<> struct type_of_helper<int16_t>
  {
    operator Type() { return Int(16); }
  };

  template<> struct type_of_helper<int32_t>
  {
    operator Type() { return Int(32); }
  };

  template<> struct type_of_helper<int64_t>
  {
    operator Type() { return Int(64); }
  };

  template<> struct type_of_helper<bool>
  {
...
template<typename T> Type type_of() { return Type(type_of_helper<T>()); }
...
struct Argument
{
  ...
  Argument(const std::string &_name, bool _is_buffer, Type _type) : name(_name), is_buffer(_is_buffer), type(_type)
  {
...
// Read the debug level from the environment
    ...
    template<typename T> debug &operator<<(T x)
    {
...
namespace Internal {
  ...
  struct ErrorReport
  {
    ...
    template<typename T> ErrorReport &operator<<(T x)
    {
...
namespace Internal {
  ...
  private:
    ...
    void decref(T *p)
    {
      if (p) {
        ...
        if (ref_count(p).is_zero()) { destroy(p); }
      }
    }
  ...
  public:
    ...
    bool defined() const { return ptr != NULL; }
...
public:
  ...
  EXPORT Buffer(Type t, const std::vector<int32_t> &sizes, uint8_t *data = NULL, const std::string &name = "");
...
namespace Internal {
  ...
  template<> EXPORT inline RefCount &ref_count<IRNode>(const IRNode *n) { return n->ref_count; }

  template<> EXPORT inline void destroy<IRNode>(const IRNode *n) { delete n; }
  ...
  struct IRHandle : public IntrusivePtr<const IRNode>
  {
    ...
    {
      if (ptr->type_info() == &T::_type_info) { return (const T *)ptr; }
      return NULL;
    }
  };
  ...
  struct IntImm : public ExprNode<IntImm>
  {
    ...
    {
      if (value >= -8 && value <= 8 && !small_int_cache[value + 8].ref_count.is_zero()) {
...
struct Expr : public Internal::IRHandle
{
  ...
  EXPORT Expr(double x) : IRHandle(Internal::FloatImm::make((float)x))
  {
    user_warning << "Halide cannot represent double constants. "
                 ...
                 << "If you wanted a double, use cast<double>(" << x << (x == (int64_t)(x) ? ".0f" : "f") << ")\n";
  }
  ...
  Type type() const { return ((const Internal::BaseExprNode *)ptr)->type; }
};
...
namespace Internal {
  ...
  public:
    ...
    template<typename T> NO_INLINE T get_scalar()
    {
      user_assert(type() == type_of<T>())
        << "Can't get Param<" << type() << "> as scalar of type " << type_of<T>() << "\n";
      return *((T *)(get_scalar_address()));
    }

    /** If the parameter is a scalar parameter, set its current
     * value. Only relevant when jitting */
    template<typename T> NO_INLINE void set_scalar(T val)
    {
      user_assert(type() == type_of<T>())
        << "Can't set Param<" << type() << "> to scalar of type " << type_of<T>() << "\n";
...
namespace Internal {
  ...
  struct Stmt : public IRHandle
  {
    ...
    struct Compare
    {
      bool operator()(const Stmt &a, const Stmt &b) const { return a.ptr < b.ptr; }
    };
  };
...
namespace Internal {
  ...
  struct Allocate : public StmtNode<Allocate>
  {
    ...
    EXPORT static Stmt make(std::string name, Type type, const std::vector<Expr> &extents, Expr condition, Stmt body);
  };
  ...
  struct Realize : public StmtNode<Realize>
  {
    ...
    EXPORT static Stmt
      make(const std::string &name, const std::vector<Type> &types, const Region &bounds, Expr condition, Stmt body);
  };
...
namespace Internal {
  ...
  struct LoopLevel
  {
    ...
    static LoopLevel root() { return LoopLevel("", "__root"); }
    ...
    bool match(const std::string &loop) const { return starts_with(loop, func + ".") && ends_with(loop, "." + var); }

    bool match(const LoopLevel &other) const
    {
      return (
        func == other.func && (var == other.var || ends_with(var, "." + other.var) || ends_with(other.var, "." + var)));
    }

    /** Check if two loop levels are exactly the same. */
    bool operator==(const LoopLevel &other) const { return func == other.func && var == other.var; }
  };
...
namespace Internal {
  ...
  public:
    ...
    ReductionDomain(const std::vector<ReductionVariable> &domain) : contents(new ReductionDomainContents)
    {
     ...
     * that */
    bool same_as(const ReductionDomain &other) const { return contents.same_as(other.contents); }

    /** Immutable access to the reduction variables. */
    const std::vector<ReductionVariable> &domain() const { return contents.ptr->domain; }
  };
  ...
  public:
    ...
    Function(const std::string &n) : contents(new FunctionContents)
    {
      for (size_t i = 0; i < n.size(); i++) {
        user_assert(n[i] != '.') << "Func name \"" << n << "\" is invalid. "
    ...
    const std::vector<std::string> &args() const { return contents.ptr->args; }

    /** Get the dimensionality */
    int dimensions() const { return (int)args().size(); }

    /** Get the number of outputs */
    int outputs() const { return (int)output_types().size(); }

    /** Get the types of the outputs */
    const std::vector<Type> &output_types() const { return contents.ptr->output_types; }

    /** Get the right-hand-side of the pure definition */
    const std::vector<Expr> &values() const { return contents.ptr->values; }

    /** Does this function have a pure definition */
    bool has_pure_definition() const { return !contents.ptr->values.empty(); }
     ...
     * it */
    Schedule &schedule() { return contents.ptr->schedule; }

    /** Get a const handle to the schedule for inspecting it */
    const Schedule &schedule() const { return contents.ptr->schedule; }

    /** Get a handle on the output buffer used for setting constraints
     * on it. */
    const std::vector<Parameter> &output_buffers() const { return contents.ptr->output_buffers; }

    /** Get a mutable handle to the schedule for the update
     * stage */
    Schedule &update_schedule(int idx = 0) { return contents.ptr->updates[idx].schedule; }

    /** Get a const reference to this function's update definitions. */
    const std::vector<UpdateDefinition> &updates() const { return contents.ptr->updates; }

    /** Does this function have an update definition */
    bool has_update_definition() const { return !contents.ptr->updates.empty(); }

    /** Check if the function has an extern definition */
    bool has_extern_definition() const { return !contents.ptr->extern_function_name.empty(); }
    ...
    const std::vector<ExternFuncArgument> &extern_arguments() const { return contents.ptr->extern_arguments; }

    /** Get the name of the extern function called for an extern
     * definition. */
    const std::string &extern_function_name() const { return contents.ptr->extern_function_name; }

    /** Equality of identity */
    bool same_as(const Function &other) const { return contents.same_as(other.contents); }

    /** Get a const handle to the debug filename */
    const std::string &debug_file() const { return contents.ptr->debug_file; }

    /** Get a handle to the debug filename */
    std::string &debug_file() { return contents.ptr->debug_file; }

    /** Use an an extern argument to another function. */
    operator ExternFuncArgument() const { return ExternFuncArgument(contents); }

    /** Tracing calls and accessors, passed down from the Func
     * equivalents. */
    // @{
    void trace_loads() { contents.ptr->trace_loads = true; }
    void trace_stores() { contents.ptr->trace_stores = true; }
    void trace_realizations() { contents.ptr->trace_realizations = true; }
    bool is_tracing_loads() { return contents.ptr->trace_loads; }
    bool is_tracing_stores() { return contents.ptr->trace_stores; }
    bool is_tracing_realizations() { return contents.ptr->trace_realizations; }
    // @}

    /** Mark function as frozen, which means it cannot accept new
     * definitions. */
    void freeze() { contents.ptr->frozen = true; }

    /** Check if a function has been frozen. If so, it is an error to
     * add new definitions. */
    bool frozen() const { return contents.ptr->frozen; }
  };
...
namespace Internal {
  ...
  struct Call : public ExprNode<Call>
  {
    ...
    EXPORT static const std::string debug_to_file, shuffle_vector, interleave_vectors, reinterpret, bitwise_and,
      bitwise_not, bitwise_xor, bitwise_or, shift_left, shift_right, abs, rewrite_buffer, profiling_timer, random, lerp,
      create_buffer_t, extract_buffer_min, extract_buffer_max, set_host_dirty, set_dev_dirty, popcount,
      count_leading_zeros, count_trailing_zeros, undef, null_handle, address_of, return_second, if_then_else, trace,
      trace_expr, glsl_texture_load, glsl_texture_store, make_struct, stringify, memoize_expr, copy_memory;
    ...
    {
      internal_assert(idx >= 0 && idx < func.outputs()) << "Value index out of range in call to halide function\n";
  ...
  struct Variable : public ExprNode<Variable>
  {
    ...
    static Expr make(Type type, std::string name) { return make(type, name, Buffer(), Parameter(), ReductionDomain()); }
    ...
    EXPORT static Expr
      make(Type type, std::string name, Buffer image, Parameter param, ReductionDomain reduction_domain);
  };
...
template<typename T> inline Expr cast(Expr a) { return cast(type_of<T>(), a); }
...
inline Expr operator&&(Expr a, Expr b) { return Internal::And::make(a, b); }

/** Returns the logical or of the two arguments */
inline Expr operator||(Expr a, Expr b) { return Internal::Or::make(a, b); }

/** Returns the logical not the argument */
inline Expr operator!(Expr a) { return Internal::Not::make(a); }
...
{
  user_assert(a.defined() && b.defined()) << "max of undefined Expr\n";
  Internal::match_types(a, b);
...
{
  user_assert(a.defined() && b.defined()) << "min of undefined Expr\n";
  Internal::match_types(a, b);
...
{
  user_assert(a.defined() && min_val.defined() && max_val.defined()) << "clamp of undefined Expr\n";
...
{
  user_assert(a.defined()) << "abs of undefined Expr\n";
  Type t = a.type();
  if (t.is_int()) {
    t.code = Type::UInt;
  } else if (t.is_uint()) {
  ...
  return Internal::Call::make(t, Internal::Call::abs, vec(a), Internal::Call::Intrinsic);
}
...
inline Expr select(Expr condition, Expr true_value, Expr false_value)
{
  ...
  if (as_const_int(true_value)) { true_value = cast(false_value.type(), true_value); }
  if (as_const_int(false_value)) { false_value = cast(true_value.type(), false_value); }
...
// @{
inline Expr select(Expr c1, Expr v1, Expr c2, Expr v2, Expr default_val)
{
  return select(c1, v1, select(c2, v2, default_val));
}
inline Expr select(Expr c1, Expr v1, Expr c2, Expr v2, Expr c3, Expr v3, Expr default_val)
{
  return select(c1, v1, c2, v2, select(c3, v3, default_val));
}
inline Expr select(Expr c1, Expr v1, Expr c2, Expr v2, Expr c3, Expr v3, Expr c4, Expr v4, Expr default_val)
{
  return select(c1, v1, c2, v2, c3, v3, select(c4, v4, default_val));
}
inline Expr
  select(Expr c1, Expr v1, Expr c2, Expr v2, Expr c3, Expr v3, Expr c4, Expr v4, Expr c5, Expr v5, Expr default_val)
{
  return select(c1, v1, c2, v2, c3, v3, c4, v4, select(c5, v5, default_val));
}
inline Expr select(Expr c1,
  Expr v1,
  Expr c2,
  Expr v2,
  Expr c3,
  ...
  Expr default_val)
{
  return select(c1, v1, c2, v2, c3, v3, c4, v4, c5, v5, select(c6, v6, default_val));
}
inline Expr select(Expr c1,
  Expr v1,
  Expr c2,
  ...
  Expr default_val)
{
  return select(c1, v1, c2, v2, c3, v3, c4, v4, c5, v5, c6, v6, select(c7, v7, default_val));
}
inline Expr select(Expr c1,
  Expr v1,
  Expr c2,
  ...
  Expr default_val)
{
  return select(c1, v1, c2, v2, c3, v3, c4, v4, c5, v5, c6, v6, c7, v7, select(c8, v8, default_val));
}
inline Expr select(Expr c1,
  Expr v1,
  Expr c2,
  ...
  Expr default_val)
{
  return select(c1, v1, c2, v2, c3, v3, c4, v4, c5, v5, c6, v6, c7, v7, c8, v8, select(c9, v9, default_val));
}
inline Expr select(Expr c1,
  Expr v1,
  Expr c2,
  ...
  Expr default_val)
{
  return select(c1, v1, c2, v2, c3, v3, c4, v4, c5, v5, c6, v6, c7, v7, c8, v8, c9, v9, select(c10, v10, default_val));
}
// @}
...
inline Expr hypot(Expr x, Expr y) { return sqrt(x * x + y * y); }
...
inline Expr pow(Expr x, Expr y)
{
  ...
  if (const int *i = as_const_int(y)) { return raise_to_integer_power(x, *i); }
...
inline Expr reinterpret(Type t, Expr e)
{
  ...
  user_assert(from_bits == to_bits) << "Reinterpret cast from type " << e.type() << " which has " << from_bits
                                    << " bits, to type " << t << " which has " << to_bits << " bits\n";
  return Internal::Call::make(t, Internal::Call::reinterpret, vec(e), Internal::Call::Intrinsic);
}

template<typename T> inline Expr reinterpret(Expr e) { return reinterpret(type_of<T>(), e); }
...
inline Expr operator&(Expr x, Expr y)
{
  ...
  if (y.type() != x.type()) { y = reinterpret(x.type(), y); }
...
inline Expr lerp(Expr zero_val, Expr one_val, Expr weight)
{
  ...
  if (as_const_int(zero_val)) { zero_val = cast(one_val.type(), zero_val); }
  if (as_const_int(one_val)) { one_val = cast(zero_val.type(), one_val); }

  user_assert(zero_val.type() == one_val.type())
    << "Can't lerp between " << zero_val << " of type " << zero_val.type() << " and " << one_val
  ...
  if (!zero_val.type().is_float()) {
    ...
    if (const_weight) {
      user_assert(*const_weight >= 0.0f && *const_weight <= 1.0f)
        << "Floating-point weight for lerp with integer arguments is " << *const_weight
        << ", which is not in the range [0.0f, 1.0f].\n";
    }
  }
  return Internal::Call::make(
    zero_val.type(), Internal::Call::lerp, vec(zero_val, one_val, weight), Internal::Call::Intrinsic);
}
...
inline Expr popcount(Expr x)
{
  ...
  return Internal::Call::make(x.type(), Internal::Call::popcount, vec(x), Internal::Call::Intrinsic);
}
...
inline Expr count_leading_zeros(Expr x)
{
  ...
  return Internal::Call::make(x.type(), Internal::Call::count_leading_zeros, vec(x), Internal::Call::Intrinsic);
}
...
inline Expr count_trailing_zeros(Expr x)
{
  ...
  return Internal::Call::make(x.type(), Internal::Call::count_trailing_zeros, vec(x), Internal::Call::Intrinsic);
}
...
inline Expr random_float(Expr seed = Expr())
{
  ...
  std::vector<Expr> args;
  if (seed.defined()) {
    user_assert(seed.type() == Int(32)) << "The seed passed to random_float must have type Int(32), but instead is "
                                        << seed << " of type " << seed.type() << "\n";
    args.push_back(seed);
  }
  args.push_back(counter);

  return Internal::Call::make(Float(32), Internal::Call::random, args, Internal::Call::Intrinsic);
}
...
inline Expr random_int(Expr seed = Expr())
{
  ...
  std::vector<Expr> args;
  if (seed.defined()) {
    user_assert(seed.type() == Int(32)) << "The seed passed to random_int must have type Int(32), but instead is "
                                        << seed << " of type " << seed.type() << "\n";
    args.push_back(seed);
  }
  args.push_back(counter);

  return Internal::Call::make(Int(32), Internal::Call::random, args, Internal::Call::Intrinsic);
}
...
inline Expr print(PrintArg a) { return print(Internal::vec<Expr>(a)); }
inline Expr print(PrintArg a, PrintArg b) { return print(Internal::vec<Expr>(a, b)); }
inline Expr print(PrintArg a, PrintArg b, PrintArg c) { return print(Internal::vec<Expr>(a, b, c)); }
inline Expr print(PrintArg a, PrintArg b, PrintArg c, PrintArg d) { return print(Internal::vec<Expr>(a, b, c, d)); }
...
inline Expr print_when(Expr condition, PrintArg a) { return print_when(condition, Internal::vec<Expr>(a)); }
...
inline Expr
  print_when(Expr condition, PrintArg a, PrintArg b, PrintArg c, PrintArg d, PrintArg e, PrintArg f, PrintArg g)
{
  return print_when(condition, Internal::vec<Expr>(a, b, c, d, e, f, g));
}
inline Expr print_when(Expr condition,
  PrintArg a,
  PrintArg b,
  PrintArg c,
  PrintArg d,
...
{
  return Internal::Call::make(t, Internal::Call::undef, std::vector<Expr>(), Internal::Call::Intrinsic);
}
...
inline Expr memoize_tag(Expr result) { return memoize_tag(result, std::vector<Expr>()); }
inline Expr memoize_tag(Expr result, Expr a) { return memoize_tag(result, Internal::vec<Expr>(a)); }
inline Expr memoize_tag(Expr result, Expr a, Expr b) { return memoize_tag(result, Internal::vec<Expr>(a, b)); }
...
namespace Internal {
  ...
  public:
    ...
    T &ref(const std::string &name)
    {
      ...
      if (iter == table.end() || iter->second.empty()) { internal_error << "Symbol '" << name << "' not found\n"; }
     ...
     */
    void push(const std::string &name, const T &value) { table[name].push(value); }
    ...
    void pop(const std::string &name)
    {
      ...
      iter->second.pop();
      if (iter->second.empty()) { table.erase(iter); }
    }
    ...
    public:
      explicit const_iterator(const typename std::map<std::string, SmallStack<T>>::const_iterator &i) : iter(i) {}
    ...
    public:
      explicit iterator(typename std::map<std::string, SmallStack<T>>::iterator i) : iter(i) {}
...
namespace Internal {
  ...
  // @{
  Box box_required(Expr e,
    std::string fn,
    const Scope<Interval> &scope = Scope<Interval>::empty_scope(),
    const FuncValueBounds &func_bounds = FuncValueBounds());
  Box box_required(Stmt s,
    std::string fn,
    const Scope<Interval> &scope = Scope<Interval>::empty_scope(),
    const FuncValueBounds &func_bounds = FuncValueBounds());

  Box box_provided(Expr e,
    std::string fn,
    const Scope<Interval> &scope = Scope<Interval>::empty_scope(),
    const FuncValueBounds &func_bounds = FuncValueBounds());
  Box box_provided(Stmt s,
    std::string fn,
    const Scope<Interval> &scope = Scope<Interval>::empty_scope(),
    const FuncValueBounds &func_bounds = FuncValueBounds());

  Box box_touched(Expr e,
    std::string fn,
    const Scope<Interval> &scope = Scope<Interval>::empty_scope(),
    const FuncValueBounds &func_bounds = FuncValueBounds());
  Box box_touched(Stmt s,
    std::string fn,
    const Scope<Interval> &scope = Scope<Interval>::empty_scope(),
    const FuncValueBounds &func_bounds = FuncValueBounds());
  // @}
...
namespace Runtime {
  namespace Internal {
    ...
    template<int type> class Printer
    {
    public:
      char buf[1024];
      char *dst, *end;
      ...
      Printer(void *ctx) : dst(buf), end(buf + 1023), user_context(ctx) { *end = 0; }
    ...
    template<typename T> SinkPrinter operator<<(const SinkPrinter &s, T) { return s; }
...
namespace Internal {
  ...
  struct JITCompiledModule
  {
    ...
    void (
      *set_custom_allocator)(void *(*malloc)(void *user_context, size_t), void (*free)(void *user_context, void *ptr));
    ...
    void (*set_custom_do_par_for)(int (*custom_do_par_for)(void *user_context, HalideTask, int, int, uint8_t *));

    /** Set a custom do parallel task. See
     * \ref Func::set_custom_do_task */
    void (*set_custom_do_task)(int (*custom_do_task)(void *user_context, HalideTask, int, uint8_t *));
    ...
    JITCompiledModule()
      : function(NULL), wrapped_function(NULL), copy_to_host(NULL), copy_to_dev(NULL), free_dev_buffer(NULL),
        set_error_handler(NULL), set_custom_allocator(NULL), set_custom_do_par_for(NULL), set_custom_do_task(NULL),
        set_custom_trace(NULL), set_custom_print(NULL), shutdown_thread_pool(NULL), memoization_cache_set_size(NULL)
    {}
...
struct Target
{
  ...
  Target(OS o, Arch a, int b, std::vector<Feature> initial_features = std::vector<Feature>()) : os(o), arch(a), bits(b)
  {
    for (size_t i = 0; i < initial_features.size(); i++) { set_feature(initial_features[i]); }
  }
  ...
  {
    for (size_t i = 0; i < features_to_set.size(); i++) { set_feature(features_to_set[i], value); }
  }
  ...
  bool features_any_of(std::vector<Feature> test_features) const
  {
    for (size_t i = 0; i < test_features.size(); i++) {
      ...
      if (features[test_features[i]]) { return true; }
    }
    return false;
  }
  ...
  bool features_all_of(std::vector<Feature> test_features) const
  {
    for (size_t i = 0; i < test_features.size(); i++) {
      ...
      if (!features[test_features[i]]) { return false; }
    }
    return true;
  }
  ...
  bool has_gpu_feature() const { return has_feature(CUDA) || has_feature(OpenCL); }

  bool operator==(const Target &other) const
  {
    return os == other.os && arch == other.arch && bits == other.bits && features == other.features;
  }
...
namespace Internal {
  ...
  public:
     ...
     * compile_to_native. */
    virtual void compile(Stmt stmt,
      std::string name,
      const std::vector<Argument> &args,
      const std::vector<Buffer> &images_to_embed);
    ...
    virtual void
      jit_finalize(llvm::ExecutionEngine *, llvm::Module *, std::vector<JITCompiledModule::CleanupRoutine> *);
...
namespace Internal {
  ...
  private:
    ...
    Allocation create_allocation(const std::string &name, Type type, const std::vector<Expr> &extents, Expr condition);
...
namespace Internal {
  ...
  template<typename CodeGen_CPU> class CodeGen_GPU_Host : public CodeGen_CPU
  {
  public:
  ...
  protected:
     ...
     * cuda context */
    void jit_finalize(llvm::ExecutionEngine *ee,
      llvm::Module *mod,
...
namespace Internal {
  ...
  struct GPU_Argument : public Argument
  {
    ...
    GPU_Argument() : size(0) {}
    GPU_Argument(const std::string &_name, bool _is_buffer, Type _type) : Argument(_name, _is_buffer, _type), size(0) {}
    GPU_Argument(const std::string &_name, bool _is_buffer, Type _type, size_t _size)
  ...
  struct CodeGen_GPU_Dev
  {
    ...
    virtual void add_kernel(Stmt stmt, const std::string &name, const std::vector<GPU_Argument> &args) = 0;
...
namespace Internal {
  ...
  public:
    ...
    void add_kernel(Stmt stmt, const std::string &name, const std::vector<GPU_Argument> &args);
...
#define _halide_check_arg_type(t, name, e, n)                                                                   \
  _halide_user_assert(e.type() == t) << "Type mismatch for argument " << n << " to extern function " << #name   \
                                     << ". Type expected is " << t << " but the argument " << e << " has type " \
                                     << e.type() << ".\n";
...
#define HalideExtern_3(rt, name, t1, t2, t3)                                          \
  ...
  {                                                                                   \
    ...
    return Halide::Internal::Call::make(                                              \
      Halide::type_of<rt>(), #name, vec(a1, a2, a3), Halide::Internal::Call::Extern); \
  }
...
#define HalideExtern_4(rt, name, t1, t2, t3, t4)                                          \
  ...
  {                                                                                       \
    ...
    return Halide::Internal::Call::make(                                                  \
      Halide::type_of<rt>(), #name, vec(a1, a2, a3, a4), Halide::Internal::Call::Extern); \
  }
...
#define HalideExtern_5(rt, name, t1, t2, t3, t4, t5)                                                     \
  ...
  {                                                                                                      \
    ...
    return Halide::Internal::Call::make(                                                                 \
      Halide::type_of<rt>(), #name, vec(a1, a2, a3, a4, a5), Halide::Internal::Call::Extern);            \
  }
...
public:
  ...
  {
    return Internal::starts_with(name, "_") && name.find_first_not_of("0123456789", 1) == std::string::npos;
  }
  bool is_implicit() const { return is_implicit(name()); }
  //}
   ...
   */
  //{
  static int implicit_index(const std::string &name) { return is_implicit(name) ? atoi(name.c_str() + 1) : -1; }
  int implicit_index() const { return implicit_index(name()); }
  //}

  /** Test if a var is the placeholder variable \ref _ */
  //{
  static bool is_placeholder(const std::string &name) { return name == "_"; }
  bool is_placeholder() const { return is_placeholder(name()); }
  //}

  /** A Var can be treated as an Expr of type Int(32) */
  operator Expr() const { return Internal::Variable::make(Int(32), name()); }

  /** Vars to use for scheduling producer/consumer pairs on the gpu. */
  // @{
  static Var gpu_blocks() { return Var("__block_id_x"); }
  static Var gpu_threads() { return Var("__thread_id_x"); }
  // @}
...
public:
  ...
  {
    if (min.type() != type_of<T>()) { min = Internal::Cast::make(type_of<T>(), min); }
    param.set_min_value(min);
  }

  void set_max_value(Expr max)
  {
    if (max.type() != type_of<T>()) { max = Internal::Cast::make(type_of<T>(), max); }
   ...
   * function definition */
  operator Expr() const { return Internal::Variable::make(type_of<T>(), name(), param); }
 ...
 * runtime function.  */
inline Param<void *> user_context_param() { return Param<void *>("__user_context"); }
...
class RVar
{
  ...
  const Internal::ReductionVariable &_var() const { return _domain.domain().at(_index); }
...
public:
  ...
  RVar(Internal::ReductionDomain domain, int index) : _domain(domain), _index(index) {}
...
public:
  ...
  Tuple(Expr a, Expr b, Expr c, Expr d) : exprs(Internal::vec<Expr>(a, b, c, d)) {}

  Tuple(Expr a, Expr b, Expr c, Expr d, Expr e) : exprs(Internal::vec<Expr>(a, b, c, d, e)) {}

  Tuple(Expr a, Expr b, Expr c, Expr d, Expr e, Expr f) : exprs(Internal::vec<Expr>(a, b, c, d, e, f)) {}
  //@}
...
public:
  ...
  Realization(Buffer a, Buffer b, Buffer c) : buffers(Internal::vec<Buffer>(a, b, c)) {}

  Realization(Buffer a, Buffer b, Buffer c, Buffer d) : buffers(Internal::vec<Buffer>(a, b, c, d)) {}

  Realization(Buffer a, Buffer b, Buffer c, Buffer d, Buffer e) : buffers(Internal::vec<Buffer>(a, b, c, d, e)) {}
...
inline Tuple tuple_select(Tuple condition, const Tuple &true_value, const Tuple &false_value)
{
  ...
  for (size_t i = 0; i < result.size(); i++) { result[i] = select(condition[i], true_value[i], false_value[i]); }
  return result;
}
...
inline Tuple tuple_select(Expr condition, const Tuple &true_value, const Tuple &false_value)
{
  ...
  for (size_t i = 0; i < result.size(); i++) { result[i] = select(condition, true_value[i], false_value[i]); }
  return result;
}
// @}
...
template<typename T> class Image : public ImageBase
{
public:
  /** Construct an undefined image handle */
  Image() : ImageBase() {}
  ...
  NO_INLINE Image(int x, int y, int z, const std::string &name) : ImageBase(type_of<T>(), x, y, z, 0, name) {}

  NO_INLINE Image(int x, int y, const std::string &name) : ImageBase(type_of<T>(), x, y, 0, 0, name) {}

  NO_INLINE Image(int x, const std::string &name) : ImageBase(type_of<T>(), x, 0, 0, 0, name) {}
  // @}
  ...
  /** Wrap a buffer_t in an Image object, so that we can access its
   * pixels. */
  NO_INLINE Image(const buffer_t *b, const std::string &name = "") : ImageBase(type_of<T>(), b, name) {}
  ...
  const T &operator()(int x) const { return ((T *)origin)[x * stride_0]; }

  /** Assuming this image is two-dimensional, get the value of the
   * element at position (x, y) */
  const T &operator()(int x, int y) const { return ((T *)origin)[x * stride_0 + y * stride_1]; }

  /** Assuming this image is three-dimensional, get the value of the
   * element at position (x, y, z) */
  const T &operator()(int x, int y, int z) const { return ((T *)origin)[x * stride_0 + y * stride_1 + z * stride_2]; }
   ...
   * element at position x */
  T &operator()(int x) { return ((T *)origin)[x * stride_0]; }

  /** Assuming this image is two-dimensional, get a reference to the
   * element at position (x, y) */
  T &operator()(int x, int y) { return ((T *)origin)[x * stride_0 + y * stride_1]; }

  /** Assuming this image is three-dimensional, get a reference to the
   * element at position (x, y, z) */
  T &operator()(int x, int y, int z) { return ((T *)origin)[x * stride_0 + y * stride_1 + z * stride_2]; }
  ...
  operator Argument() const { return Argument(buffer); }

  /** Convert this image to an argument to an extern stage. */
  operator ExternFuncArgument() const { return ExternFuncArgument(buffer); }
...
enum GPUAPI { GPU_Default, GPU_CUDA, GPU_OpenCL, GPU_GLSL };
...
public:
  Stage(Internal::Schedule s, const std::string &n) : schedule(s), stage_name(n) { s.touched(); }
  ...
  EXPORT Stage &
    tile(VarOrRVar x, VarOrRVar y, VarOrRVar xo, VarOrRVar yo, VarOrRVar xi, VarOrRVar yi, Expr xfactor, Expr yfactor);
  EXPORT Stage &tile(VarOrRVar x, VarOrRVar y, VarOrRVar xi, VarOrRVar yi, Expr xfactor, Expr yfactor);
  ...
  EXPORT Stage &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w);
  EXPORT Stage &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w, VarOrRVar t);
  EXPORT Stage &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w, VarOrRVar t1, VarOrRVar t2);
  EXPORT Stage &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w, VarOrRVar t1, VarOrRVar t2, VarOrRVar t3);
  EXPORT Stage &
    reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w, VarOrRVar t1, VarOrRVar t2, VarOrRVar t3, VarOrRVar t4);
  EXPORT Stage &reorder(VarOrRVar x,
    VarOrRVar y,
    VarOrRVar z,
    VarOrRVar w,
    VarOrRVar t1,
  ...
  EXPORT Stage &
    gpu(VarOrRVar block_x, VarOrRVar block_y, VarOrRVar thread_x, VarOrRVar thread_y, GPUAPI gpu_api = GPU_Default);
  EXPORT Stage &gpu(VarOrRVar block_x,
    VarOrRVar block_y,
    VarOrRVar block_z,
    VarOrRVar thread_x,
    VarOrRVar thread_y,
  ...
  EXPORT Stage &gpu_tile(VarOrRVar x, VarOrRVar y, Expr x_size, Expr y_size, GPUAPI gpu_api = GPU_Default);
  EXPORT Stage &gpu_tile(VarOrRVar x,
    VarOrRVar y,
    VarOrRVar z,
    Expr x_size,
    Expr y_size,
  ...
  EXPORT Stage &cuda_threads(VarOrRVar thread_x) { return gpu_threads(thread_x); }
  EXPORT Stage &cuda_threads(VarOrRVar thread_x, VarOrRVar thread_y) { return gpu_threads(thread_x, thread_y); }
  ...
  EXPORT Stage &cuda_blocks(VarOrRVar block_x) { return gpu_blocks(block_x); }
  EXPORT Stage &cuda_blocks(VarOrRVar block_x, VarOrRVar block_y) { return gpu_blocks(block_x, block_y); }
  EXPORT Stage &cuda_blocks(VarOrRVar block_x, VarOrRVar block_y, VarOrRVar block_z)
  {
    return gpu_blocks(block_x, block_y, block_z);
  }

  EXPORT Stage &cuda(VarOrRVar block_x, VarOrRVar thread_x) { return gpu(block_x, thread_x); }
  EXPORT Stage &cuda(VarOrRVar block_x, VarOrRVar block_y, VarOrRVar thread_x, VarOrRVar thread_y)
  {
    return gpu(block_x, thread_x, block_y, thread_y);
  }
  EXPORT Stage &cuda(VarOrRVar block_x,
    VarOrRVar block_y,
    VarOrRVar block_z,
    VarOrRVar thread_x,
    VarOrRVar thread_y,
  ...
  EXPORT Stage &cuda_tile(VarOrRVar x, int x_size) { return gpu_tile(x, x_size); }
  EXPORT Stage &cuda_tile(VarOrRVar x, VarOrRVar y, int x_size, int y_size) { return gpu_tile(x, y, x_size, y_size); }
  EXPORT Stage &cuda_tile(VarOrRVar x, VarOrRVar y, VarOrRVar z, int x_size, int y_size, int z_size)
  {
...
public:
  FuncRefExpr(Internal::Function, const std::vector<Expr> &, int placeholder_pos = -1);
  FuncRefExpr(Internal::Function, const std::vector<std::string> &, int placeholder_pos = -1);
...
class Func
{
  ...
  // @{
  int (*custom_do_par_for)(void *user_context, int (*)(void *, int, uint8_t *), int, int, uint8_t *);
  int (*custom_do_task)(void *user_context, int (*)(void *, int, uint8_t *), int, uint8_t *);
  // @}
...
public:
  ...
  EXPORT Realization
    realize(int x_size, int y_size, int z_size, int w_size, const Target &target = get_jit_target_from_environment());
  EXPORT Realization realize(int x_size,
    int y_size,
    int z_size,
    const Target &target = get_jit_target_from_environment());
  EXPORT Realization realize(int x_size, int y_size, const Target &target = get_jit_target_from_environment());
  EXPORT Realization realize(int x_size = 0, const Target &target = get_jit_target_from_environment());
  // @}
  ...
  template<typename T> void realize(Image<T> dst, const Target &target = get_jit_target_from_environment())
  {
  ...
  EXPORT void compile_to_file(const std::string &filename_prefix,
    ...
    const Target &target = get_target_from_environment());
  EXPORT void compile_to_file(const std::string &filename_prefix, const Target &target = get_target_from_environment());
  EXPORT void compile_to_file(const std::string &filename_prefix,
    Argument a,
    const Target &target = get_target_from_environment());
  EXPORT void compile_to_file(const std::string &filename_prefix,
  ...
  */
  ...
  EXPORT void set_custom_do_par_for(
    int (*custom_do_par_for)(void *, int (*)(void *, int, uint8_t *), int, int, uint8_t *));
  ...
  EXPORT Func &tile(VarOrRVar x, VarOrRVar y, VarOrRVar xi, VarOrRVar yi, Expr xfactor, Expr yfactor);
   ...
   * innermost out */
  EXPORT Func &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w);

  /** Reorder five dimensions to have the given nesting order, from
   * innermost out */
  EXPORT Func &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w, VarOrRVar t);

  /** Reorder six dimensions to have the given nesting order, from
   * innermost out */
  EXPORT Func &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w, VarOrRVar t1, VarOrRVar t2);

  /** Reorder seven dimensions to have the given nesting order, from
   * innermost out */
  EXPORT Func &reorder(VarOrRVar x, VarOrRVar y, VarOrRVar z, VarOrRVar w, VarOrRVar t1, VarOrRVar t2, VarOrRVar t3);
   ...
   * innermost out */
  EXPORT Func &reorder(VarOrRVar x,
    VarOrRVar y,
    VarOrRVar z,
    VarOrRVar w,
    VarOrRVar t1,
  ...
  // @{
  EXPORT Func &cuda_threads(VarOrRVar thread_x) { return gpu_threads(thread_x); }
  EXPORT Func &cuda_threads(VarOrRVar thread_x, VarOrRVar thread_y) { return gpu_threads(thread_x, thread_y); }
  ...
  // @{
  EXPORT Func &cuda_blocks(VarOrRVar block_x) { return gpu_blocks(block_x); }
  EXPORT Func &cuda_blocks(VarOrRVar block_x, VarOrRVar block_y) { return gpu_blocks(block_x, block_y); }
  ...
  EXPORT Func &
    gpu(VarOrRVar block_x, VarOrRVar block_y, VarOrRVar thread_x, VarOrRVar thread_y, GPUAPI gpu_api = GPU_Default);
  EXPORT Func &gpu(VarOrRVar block_x,
    VarOrRVar block_y,
    VarOrRVar block_z,
    VarOrRVar thread_x,
    VarOrRVar thread_y,
  ...
  // @{
  EXPORT Func &cuda(VarOrRVar block_x, VarOrRVar thread_x) { return gpu(block_x, thread_x); }
  EXPORT Func &cuda(VarOrRVar block_x, VarOrRVar block_y, VarOrRVar thread_x, VarOrRVar thread_y)
  {
    return gpu(block_x, thread_x, block_y, thread_y);
  }
  EXPORT Func &cuda(VarOrRVar block_x,
    VarOrRVar block_y,
    VarOrRVar block_z,
    VarOrRVar thread_x,
    VarOrRVar thread_y,
  ...
  EXPORT Func &
    gpu_tile(VarOrRVar x, VarOrRVar y, VarOrRVar z, int x_size, int y_size, int z_size, GPUAPI gpu_api = GPU_Default);
  // @}

  /** \deprecated Old name for #gpu_tile. */
  // @{
  EXPORT Func &cuda_tile(VarOrRVar x, int x_size) { return gpu_tile(x, x_size); }
  EXPORT Func &cuda_tile(VarOrRVar x, VarOrRVar y, int x_size, int y_size) { return gpu_tile(x, y, x_size, y_size); }
  EXPORT Func &cuda_tile(VarOrRVar x, VarOrRVar y, VarOrRVar z, int x_size, int y_size, int z_size)
  {
    return gpu_tile(x, y, z, x_size, y_size, z_size);
  }
  // @}
  ...
  */
  ...
  operator ExternFuncArgument() const { return ExternFuncArgument(func); }
 ...
 * \ref Func::realize */
template<typename T> NO_INLINE T evaluate(Expr e)
{
  user_assert(e.type() == type_of<T>()) << "Can't evaluate expression " << e << " of type " << e.type()
...
// @{
template<typename A, typename B> NO_INLINE void evaluate(Tuple t, A *a, B *b)
{
  user_assert(t[0].type() == type_of<A>()) << "Can't evaluate expression " << t[0] << " of type " << t[0].type()
                                           << " as a scalar of type " << type_of<A>() << "\n";
  user_assert(t[1].type() == type_of<B>()) << "Can't evaluate expression " << t[1] << " of type " << t[1].type()
...
template<typename A, typename B, typename C> NO_INLINE void evaluate(Tuple t, A *a, B *b, C *c)
{
  user_assert(t[0].type() == type_of<A>()) << "Can't evaluate expression " << t[0] << " of type " << t[0].type()
                                           << " as a scalar of type " << type_of<A>() << "\n";
  user_assert(t[1].type() == type_of<B>()) << "Can't evaluate expression " << t[1] << " of type " << t[1].type()
                                           << " as a scalar of type " << type_of<B>() << "\n";
  user_assert(t[2].type() == type_of<C>()) << "Can't evaluate expression " << t[2] << " of type " << t[2].type()
...
template<typename A, typename B, typename C, typename D> NO_INLINE void evaluate(Tuple t, A *a, B *b, C *c, D *d)
{
  user_assert(t[0].type() == type_of<A>()) << "Can't evaluate expression " << t[0] << " of type " << t[0].type()
                                           << " as a scalar of type " << type_of<A>() << "\n";
  user_assert(t[1].type() == type_of<B>()) << "Can't evaluate expression " << t[1] << " of type " << t[1].type()
  ...
  user_assert(t[2].type() == type_of<C>()) << "Can't evaluate expression " << t[2] << " of type " << t[2].type()
                                           << " as a scalar of type " << type_of<C>() << "\n";
  user_assert(t[3].type() == type_of<D>()) << "Can't evaluate expression " << t[3] << " of type " << t[3].type()
 ...
 * specifies one.
 */
template<typename T> NO_INLINE T evaluate_may_gpu(Expr e)
{
  user_assert(e.type() == type_of<T>()) << "Can't evaluate expression " << e << " of type " << e.type()
                                        << " as a scalar of type " << type_of<T>() << "\n";
  ...
  Func f;
  f() = e;
  if (has_gpu_feature) { f.gpu_single_thread(); }
  Image<T> im = f.realize();
  return im(0);
}
...
// @{
template<typename A, typename B> NO_INLINE void evaluate_may_gpu(Tuple t, A *a, B *b)
{
  user_assert(t[0].type() == type_of<A>()) << "Can't evaluate expression " << t[0] << " of type " << t[0].type()
                                           << " as a scalar of type " << type_of<A>() << "\n";
  user_assert(t[1].type() == type_of<B>()) << "Can't evaluate expression " << t[1] << " of type " << t[1].type()
...
template<typename A, typename B, typename C> NO_INLINE void evaluate_may_gpu(Tuple t, A *a, B *b, C *c)
{
  user_assert(t[0].type() == type_of<A>()) << "Can't evaluate expression " << t[0] << " of type " << t[0].type()
                                           << " as a scalar of type " << type_of<A>() << "\n";
  user_assert(t[1].type() == type_of<B>()) << "Can't evaluate expression " << t[1] << " of type " << t[1].type()
...
namespace Internal {
  ...
  public:
    ...
    bool contains(const Expr &a, const Expr &b) const
    {
      ...
      return ((a.same_as(e.a) && b.same_as(e.b)) || (a.same_as(e.b) && b.same_as(e.a)));
    }
...
namespace Internal {
   ...
   */
  // @{
  template<typename T> inline T mod_imp(T a, T b)
  {
    Type t = type_of<T>();
    if (t.is_int()) {
      T r = a % b;
  ...
  template<typename T> inline T div_imp(T a, T b)
  {
    Type t = type_of<T>();
    if (t.is_int()) {
      int q = a / b;
  ...
  template<> inline float div_imp<float>(float a, float b) { return a / b; }
  template<> inline double div_imp<double>(double a, double b) { return a / b; }
...
namespace Internal {
  ...
  Stmt storage_flattening(Stmt s, const std::string &output, const std::map<std::string, Function> &env);
...
namespace Internal {
  ...
  template<typename T> class ExprUsesVars : public IRVisitor
  {
    using IRVisitor::visit;
    ...
    {
      if (scope.contains(v->name)) { result = true; }
    }
  ...
  template<typename T> inline bool expr_uses_vars(Expr e, const Scope<T> &s)
  {
...
namespace Internal {
  ...
  public:
    ...
    void compile(Stmt stmt, std::string name, const std::vector<GPU_Argument> &args, const Target &target);
...
namespace Internal {
   ...
   */
  bool can_parallelize_rvar(const std::string &rvar, const std::string &func, const UpdateDefinition &r);
...
namespace BoundaryConditions {
  ...
  namespace Internal {
    ...
    inline NO_INLINE void collect_bounds(std::vector<std::pair<Expr, Expr>> &collected_bounds, Expr min, Expr extent)
    {
      collected_bounds.push_back(std::make_pair(min, extent));
    }

    template<typename... Bounds>
    inline NO_INLINE void
      collect_bounds(std::vector<std::pair<Expr, Expr>> &collected_bounds, Expr min, Expr extent, Bounds... bounds)
    {
      collected_bounds.push_back(std::make_pair(min, extent));
    ...
    inline const Func &func_like_to_func(const Func &func) { return func; }

    template<typename T> inline NO_INLINE Func func_like_to_func(T func_like)
    {
      std::vector<Var> args;
      for (int i = 0; i < func_like.dimensions(); i++) { args.push_back(Var::implicit(i)); }
      Func func;
   ...
   */
  // @{
  EXPORT Func constant_exterior(const Func &source, Expr value, const std::vector<std::pair<Expr, Expr>> &bounds);

  template<typename T> inline NO_INLINE Func constant_exterior(T func_like, Expr value)
  {
  ...
  inline NO_INLINE Func constant_exterior(T func_like, Expr value, Bounds... bounds)
  {
...
#else
  template<typename T> inline NO_INLINE Func constant_exterior(T func_like, Expr value, Expr min0, Expr extent0)
  {
  ...
  template<typename T>
  inline NO_INLINE Func constant_exterior(T func_like, Expr value, Expr min0, Expr extent0, Expr min1, Expr extent1)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
   ...
   */
  // @{
  EXPORT Func repeat_edge(const Func &source, const std::vector<std::pair<Expr, Expr>> &bounds);

  template<typename T> inline NO_INLINE Func repeat_edge(T func_like)
  {
    std::vector<std::pair<Expr, Expr>> object_bounds;
    for (int i = 0; i < func_like.dimensions(); i++) {
      object_bounds.push_back(std::make_pair(Expr(func_like.min(i)), Expr(func_like.extent(i))));
...
#if __cplusplus > 199711L// C++11 arbitrary number of args support
  template<typename T, typename... Bounds> inline NO_INLINE Func repeat_edge(T func_like, Bounds... bounds)
  {
    std::vector<std::pair<Expr, Expr>> collected_bounds;
    Internal::collect_bounds(collected_bounds, bounds...);
...
#else
  template<typename T> inline NO_INLINE Func repeat_edge(T func_like, Expr min0, Expr extent0)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
  ...
  template<typename T> inline NO_INLINE Func repeat_edge(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
  ...
  template<typename T>
  inline NO_INLINE Func
    repeat_edge(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1, Expr min2, Expr extent2)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
   ...
   */
  // @{
  EXPORT Func repeat_image(const Func &source, const std::vector<std::pair<Expr, Expr>> &bounds);

  template<typename T> inline NO_INLINE Func repeat_image(T func_like)
  {
    std::vector<std::pair<Expr, Expr>> object_bounds;
    for (int i = 0; i < func_like.dimensions(); i++) {
      object_bounds.push_back(std::make_pair(Expr(func_like.min(i)), Expr(func_like.extent(i))));
...
#if __cplusplus > 199711L// C++11 arbitrary number of args support
  template<typename T, typename... Bounds> inline NO_INLINE Func repeat_image(T func_like, Bounds... bounds)
  {
    std::vector<std::pair<Expr, Expr>> collected_bounds;
    Internal::collect_bounds(collected_bounds, bounds...);
...
#else
  template<typename T> inline NO_INLINE Func repeat_image(T func_like, Expr min0, Expr extent0)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
  ...
  template<typename T> inline NO_INLINE Func repeat_image(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
  ...
  template<typename T>
  inline NO_INLINE Func
    repeat_image(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1, Expr min2, Expr extent2)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
   ...
   */
  // @{
  EXPORT Func mirror_image(const Func &source, const std::vector<std::pair<Expr, Expr>> &bounds);

  template<typename T> inline NO_INLINE Func mirror_image(T func_like)
  {
    std::vector<std::pair<Expr, Expr>> object_bounds;
    for (int i = 0; i < func_like.dimensions(); i++) {
      object_bounds.push_back(std::make_pair(Expr(func_like.min(i)), Expr(func_like.extent(i))));
...
#if __cplusplus > 199711L// C++11 arbitrary number of args support
  template<typename T, typename... Bounds> inline NO_INLINE Func mirror_image(T func_like, Bounds... bounds)
  {
    std::vector<std::pair<Expr, Expr>> collected_bounds;
    Internal::collect_bounds(collected_bounds, bounds...);
...
#else
  template<typename T> inline NO_INLINE Func mirror_image(T func_like, Expr min0, Expr extent0)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
  ...
  template<typename T> inline NO_INLINE Func mirror_image(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
  ...
  template<typename T>
  inline NO_INLINE Func
    mirror_image(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1, Expr min2, Expr extent2)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
   ...
   */
  // @{
  EXPORT Func mirror_interior(const Func &source, const std::vector<std::pair<Expr, Expr>> &bounds);

  template<typename T> inline NO_INLINE Func mirror_interior(T func_like)
  {
    std::vector<std::pair<Expr, Expr>> object_bounds;
    for (int i = 0; i < func_like.dimensions(); i++) {
      object_bounds.push_back(std::make_pair(Expr(func_like.min(i)), Expr(func_like.extent(i))));
...
#if __cplusplus > 199711L// C++11 arbitrary number of args support
  template<typename T, typename... Bounds> inline NO_INLINE Func mirror_interior(T func_like, Bounds... bounds)
  {
    std::vector<std::pair<Expr, Expr>> collected_bounds;
    Internal::collect_bounds(collected_bounds, bounds...);
...
#else
  template<typename T> inline NO_INLINE Func mirror_interior(T func_like, Expr min0, Expr extent0)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
  ...
  template<typename T>
  inline NO_INLINE Func mirror_interior(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
  ...
  template<typename T>
  inline NO_INLINE Func
    mirror_interior(T func_like, Expr min0, Expr extent0, Expr min1, Expr extent1, Expr min2, Expr extent2)
  {
    std::vector<std::pair<Expr, Expr>> bounds;
    bounds.push_back(std::make_pair(min0, extent0));
    bounds.push_back(std::make_pair(min1, extent1));
...
namespace Internal {
   ...
   */
  Stmt inject_memoization(Stmt s, const std::map<std::string, Function> &env, const std::string &name);
...
namespace Internal {
  ...
  EXPORT Stmt human_readable_stmt(Function f, Stmt s, Buffer buf, std::map<std::string, Expr> additional_replacements);
  // @}
