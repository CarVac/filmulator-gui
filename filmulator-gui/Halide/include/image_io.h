...
template<typename T> Image<T> load_png(std::string filename)
{
  png_byte header[8];
  png_structp png_ptr;
  png_infop info_ptr;
  ...
  for (int y = 0; y < im.height(); y++) { row_pointers[y] = new png_byte[png_get_rowbytes(png_ptr, info_ptr)]; }
  ...
  if (bit_depth == 8) {
    for (int y = 0; y < im.height(); y++) {
      ...
      for (int x = 0; x < im.width(); x++) {
        for (int c = 0; c < im.channels(); c++) { convert(*srcPtr++, ptr[c * c_stride]); }
        ptr++;
      }
    }
  ...
  // clean up
  for (int y = 0; y < im.height(); y++) { delete[] row_pointers[y]; }
  delete[] row_pointers;
...
template<typename T> void save_png(Image<T> im, std::string filename)
{
  png_structp png_ptr;
  png_infop info_ptr;
  png_bytep *row_pointers;
  ...
  _assert(
    im.channels() > 0 && im.channels() < 5, "Can't write PNG files that have other than 1, 2, 3, or 4 channels\n");

  png_byte color_types[4] = {
...
template<typename T> Image<T> load_ppm(std::string filename)
{
  ...
  if (maxval == 255) {
    ...
    _assert(false, "Invalid bit depth in PPM\n");
  }
  ...
  if (bit_depth == 8) {
    ...
    _assert(fread((void *)data, sizeof(uint8_t), width * height * 3, f) == (size_t)(width * height * 3),
    ...
    _assert(fread((void *)data, sizeof(uint16_t), width * height * 3, f) == (size_t)(width * height * 3),
    ...
    for (int y = 0; y < im.height(); y++) {
      ...
      for (int x = 0; x < im.width(); x++) {
        ...
        convert(value, im_data[(0 * height + y) * width + x]);
        value = *row++;
        SWAP_ENDIAN16(little_endian, value);
        convert(value, im_data[(1 * height + y) * width + x]);
        value = *row++;
        SWAP_ENDIAN16(little_endian, value);
        convert(value, im_data[(2 * height + y) * width + x]);
      }
    }
    delete[] data;
  }
...
template<typename T> void save_ppm(Image<T> im, std::string filename)
{
  ...
  if (bit_depth == 8) {
    ...
    for (int y = 0; y < im.height(); y++) {
      for (int x = 0; x < im.width(); x++) {
        ...
        for (int c = 0; c < im.channels(); c++) { convert(im(x, y, c), p[c]); }
      }
    }
    _assert(fwrite((void *)data, sizeof(uint8_t), width * height * 3, f) == (size_t)(width * height * 3),
    ...
    _assert(fwrite((void *)data, sizeof(uint16_t), width * height * 3, f) == (size_t)(width * height * 3),
...
template<typename T> Image<T> load(std::string filename)
{
...
template<typename T> void save(Image<T> im, std::string filename)
{
  if (ends_with_ignore_case(filename, ".png")) {
