...
extern "C" float shadows_highlights(float x, float shadowsX, float shadowsY, float highlightsX, float highlightsY)
{
  return x;
}
...
public:
  ...
  Func build()
  {
    ...
    Func LUT;
    Expr maxval = 2 ^ 16 - 1;
    LUT(x) = shadows_highlights(cast<float>(x) / (maxval), shadowsX, shadowsY, highlightsX, highlightsY);
