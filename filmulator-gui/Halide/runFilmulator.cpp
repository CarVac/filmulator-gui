...
int main()
{
  ...
  error = setupFilmulator(inputImage,
    crystalsPerPixel,
    initialCrystalRadius,
    initialSilverSaltDensity,
    reservoirConcentration,
    rolloffBoundary,
    &filmulationData);
  if (error) cout << "setup error" << endl;
  ...
  for (int i = 0; i < 1 * (developmentSteps - 0); i++) {
    ...
    error = develop(&filmulationData,
      ...
      timeStep,
      &filmulationData);
    if (error) cout << "development error on iteration " << i << endl;
    ...
    if (error) cout << "diffuse error on iteration " << i << endl;
    totalDiffuseTime += current_time() - beforeDiffuseTime;

    //*
    double beforeCalcLayer = current_time();
    error = calcLayerMix(&devConc, layerMixConst, layerTimeDivisor, reservoirConcentration, timeStep, &devMoved);
    if (error) cout << "calcLayer error on iteration " << i << endl;
    calcLayer += current_time() - beforeCalcLayer;

    double beforeCalcRes = current_time();
    error = calcReservoirConcentration(
      &devMoved, activeLayerThickness, filmArea, reservoirConcentration, reservoirThickness, &resBuffer);
    if (error) cout << "calcRes error on iteration " << i << endl;
    ...
    if (error) cout << "perfLayer error on iteration " << i << endl;
  ...
  if (error) cout << "output error" << endl;
