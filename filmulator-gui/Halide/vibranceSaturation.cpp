// Compile with:
// g++ vibranceSaturation.cpp -g -I include/ -L bin/ -lHalide `libpng-config --cflags --ldflags` -lpthread -ldl -o
// vibranceSaturation Run With: LD_LIBRARY_PATH=bin ./vibranceSaturation
#include <Halide.h>
#include <iostream>
#include <stdio.h>
#include <sys/time.h>
...
Halide::Func rgb(Func in)
{
  ...
  r(x, y) = select(i(x, y) == 0 || i(x, y) == 5,
    ...
    select(i(x, y) == 1,
      q(x, y),
      select(i(x, y) == 2 || i(x, y) == 3, p(x, y), select(i(x, y) == 4, t(x, y), v(x, y)))));// default
  g(x, y) = select(i(x, y) == 0,
    t(x, y),
    select(i(x, y) == 1 || i(x, y) == 2, v(x, y), select(i(x, y) == 3, q(x, y), p(x, y))));// 4,5,default
  b(x, y) = select(i(x, y) == 0 || i(x, y) == 1,
    p(x, y),
    select(i(x, y) == 2, t(x, y), select(i(x, y) == 3 || i(x, y) == 4, v(x, y), q(x, y))));// 5,default
  out(c, x, y) = select(s(x, y) != 0, select(c == 0, r(x, y), select(c == 1, g(x, y), b(x, y))), v(x, y));
  // out(x,y,c) = h(x,y)/360;
  return out;
}
...
int main(int argc, char **argv)
{
  ...
  toHSV = hsv(toFloat);
  Func saturated;
  saturated(c, x, y) = select(c != 1, toHSV(c, x, y), clamp(1 * fast_pow(toHSV(c, x, y), 0.5), 0, 1));
  Func toRGB, toInt;
  toRGB = rgb(saturated);
