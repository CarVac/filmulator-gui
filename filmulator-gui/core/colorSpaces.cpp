...
// Reference: http://www.brucelindbloom.com/index.html?Eqn_XYZ_to_Lab.html
void XYZ_to_Lab(float fx, float fy, float fz, float &L, float &a, float &b)
{
...
void Lab_to_XYZ(float L, float a, float b, float &fx, float &fy, float &fz)
{
  fy = (L + 16) / 116;
  fx = a / 500 + fy;
  fz = fy - b / 200;
...
void sRGB_linearize(matrix<unsigned short> &in, matrix<float> &out)
{
  int nRows = in.nr();
  int nCols = in.nc();
  ...
  for (int i = 0; i < 65536; i++) { invgamma[i] = sRGB_inverse_gamma(i / 65535.0f); }
...
// Reference: http://www.brucelindbloom.com/index.html?Eqn_RGB_to_XYZ.html
void sRGB_gammacurve(matrix<float> &in, matrix<unsigned short> &out)
{
  int nRows = in.nr();
  int nCols = in.nc();
...
void sRGB_to_oklab(matrix<float> &in, matrix<float> &out)
{
  int nRows = in.nr();
  int nCols = in.nc();
...
#pragma omp for schedule(dynamic) nowait
    for (int i = 0; i < nRows; i++) {
      for (int j = 0; j < nCols; j += 3) {
        ...
        out(i, j + 0) = (0.2104542553f * l_ + 0.7936177850f * m_ - 0.0040720468f * s_) * 200;
        out(i, j + 1) = (1.9779984951f * l_ - 2.4285922050f * m_ + 0.4505937099f * s_) * 200;
        out(i, j + 2) = (0.0259040371f * l_ + 0.7827717662f * m_ - 0.8086757660f * s_) * 200;
      }
    }
  }
}
...
#pragma omp for schedule(dynamic) nowait
    for (int i = 0; i < nRows; i++) {
      for (int j = 0; j < nCols; j += 3) {
        const float L = in(i, j + 0) / 200;
        const float a = in(i, j + 1) / 200;
        const float b = in(i, j + 2) / 200;
...
void raw_to_sRGB(matrix<float> &input, matrix<float> &output, const float cam2rgb[3][3])
{
  int nRows = input.nr();
  int nCols = input.nc();
...
#pragma omp for schedule(dynamic) nowait
    for (int i = 0; i < nRows; i++) {
      for (int j = 0; j < nCols; j += 3) {
        ...
        output(i, j + 1) =
          cam2rgb[1][0] * input(i, j) + cam2rgb[1][1] * input(i, j + 1) + cam2rgb[1][2] * input(i, j + 2);
        output(i, j + 2) =
          cam2rgb[2][0] * input(i, j) + cam2rgb[2][1] * input(i, j + 1) + cam2rgb[2][2] * input(i, j + 2);
      }
    }
  }
}
// Convert raw color to sRGB, don't clip negatives.
void sRGB_to_raw(matrix<float> &input, matrix<float> &output, const float cam2rgb[3][3])
{
  int nRows = input.nr();
  int nCols = input.nc();
...
#pragma omp for schedule(dynamic) nowait
    for (int i = 0; i < nRows; i++) {
      for (int j = 0; j < nCols; j += 3) {
        ...
        output(i, j + 1) =
          rgb2cam[1][0] * input(i, j) + rgb2cam[1][1] * input(i, j + 1) + rgb2cam[1][2] * input(i, j + 2);
        output(i, j + 2) =
          rgb2cam[2][0] * input(i, j) + rgb2cam[2][1] * input(i, j + 1) + rgb2cam[2][2] * input(i, j + 2);
      }
    }
  }
}
...
// Reference: https://bottosson.github.io/posts/oklab/
void raw_to_oklab(matrix<float> &input, matrix<float> &output, const float cam2rgb[3][3])
{
  int nRows = input.nr();
  int nCols = input.nc();
...
#pragma omp for schedule(dynamic) nowait
    for (int i = 0; i < nRows; i++) {
      for (int j = 0; j < nCols; j += 3) {
        // raw to sRGB 0-1
        const float r =
          (cam2rgb[0][0] * input(i, j) + cam2rgb[0][1] * input(i, j + 1) + cam2rgb[0][2] * input(i, j + 2)) / 65535.0f;
        const float g =
          (cam2rgb[1][0] * input(i, j) + cam2rgb[1][1] * input(i, j + 1) + cam2rgb[1][2] * input(i, j + 2)) / 65535.0f;
        const float b =
          (cam2rgb[2][0] * input(i, j) + cam2rgb[2][1] * input(i, j + 1) + cam2rgb[2][2] * input(i, j + 2)) / 65535.0f;
...
// Reference: https://bottosson.github.io/posts/oklab/
void oklab_to_xyz(float L, float a, float b, float &x, float &y, float &z)
{
  L /= 200;
  a /= 200;
  b /= 200;
...
void oklab_to_raw(matrix<float> &input, matrix<float> &output, const float cam2rgb[3][3])
{
  int nRows = input.nr();
  int nCols = input.nc();
