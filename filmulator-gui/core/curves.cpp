...
#pragma omp for schedule(dynamic) nowait
    for (int i = 0; i < ysize; i++) {
      for (int j = 0; j < xsize; j = j + 3) {
        ...
        if (r >= g) {
          if (g > b)
            midValueShift(r, g, b, lookup);// Case1: r>= g>  b
          else if (b > r)
            midValueShift(b, r, g, lookup);// Case2: b>  r>= g
          else if (b > g)
            midValueShift(r, b, g, lookup);// Case3: r>= b>  g
          else// Case4: r>= g== b
          {
          ...
          if (r >= b)
            midValueShift(g, r, b, lookup);// Case5: g>  r>= b
          else if (b > g)
            midValueShift(b, g, r, lookup);// Case6: b>  g>  r
          else
            midValueShift(g, b, r, lookup);// Case7: g>= b>  r
        }
        output(i, j) = r;
        output(i, j + 1) = g;
        output(i, j + 2) = b;
