...
#pragma omp parallel                                                                                         \
  shared(develConcentration, silverSaltDensity, crystalRad, activeCrystalsPerPixel, cgc, dcc, sscc) private( \
      row, col, colr, colg, colb, dCrystalRadR, dCrystalRadG, dCrystalRadB, dCrystalVolR, dCrystalVolG, dCrystalVolB)
  {
...
#pragma omp for schedule(dynamic) nowait
    for (row = 0; row < height; row++) {
      for (col = 0; col < width; col++) {
        ...
        dCrystalVolR = dCrystalRadR * crystalRad(row, colr) * crystalRad(row, colr) * activeCrystalsPerPixel(row, colr);
        dCrystalVolG = dCrystalRadG * crystalRad(row, colg) * crystalRad(row, colg) * activeCrystalsPerPixel(row, colg);
        dCrystalVolB = dCrystalRadB * crystalRad(row, colb) * crystalRad(row, colb) * activeCrystalsPerPixel(row, colb);
