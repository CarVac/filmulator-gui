...
#pragma omp for schedule(dynamic)
        for (int row = 0; row < height; row++) {
            ...
            for (int col = 3; col < width; col++) {
                devel_concX[col] = coeff0 * devel_concX[col] + coeff1 * devel_concX[col - 1]
                                   + coeff2 * devel_concX[col - 2] + coeff3 * devel_concX[col - 3];
            }
...
#pragma omp simd
                    for (int col = 0; col < 8; col++) {
                        developer_concentration(row, col + offset) =
                          devel_concY(row, col) * attenuationY[row]; // not going SIMD
                    }
                }
            }
        }
