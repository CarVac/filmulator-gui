...
void diffuse(matrix<float> &developer_concentration, float sigma_const, float pixels_per_millimeter, float timestep)
{
  ...
  if (convlength % 2 == 0) { convlength += 1; }
  ...
  diffuse_y(developer_concentration, convlength, convrad, pad, paddedheight, order, swell_factor);
  return;
}
...
#pragma omp parallel shared(developer_concentration, convlength, convrad, order, paddedwidth, pad, swell_factor)
  {
...
#pragma omp for schedule(dynamic) nowait
    for (int row = 0; row < length; row++) {
      ...
      for (int col = 0; col < pad; col++) { hpadded[col] = developer_concentration(row, pad - col); }
      // Fill the center of padded with a row.
      for (int col = 0; col < width; col++) { hpadded[col + pad] = developer_concentration(row, col); }
      ...
      for (int pass = 0; pass < order; pass++) {
        ...
        float running_sum = 0;
        for (int col = pass * convrad; col < (pass * convrad) + convlength; col++) { running_sum += hpadded[col]; }
      ...
      for (int col = 0; col < width; col++) { developer_concentration(row, col) = hpadded[col + pad] * swell_factor; }
    }
  }
}
...
#pragma omp parallel shared(developer_concentration, convlength, convrad, order, paddedlength, pad, swell_factor)
  {
...
#pragma omp for nowait
    for (int col = 0; col < width - (numcols - 1); col += numcols) {
      ...
      for (int row = 0; row < pad; row++) {
        for (int c = 0; c < numcols; ++c) hpadded[row][c] = developer_concentration(pad - row, col + c);
      }
      // Fill the center of padded with a row.
      for (int row = 0; row < length; row++) {
        for (int c = 0; c < numcols; ++c) hpadded[row + pad][c] = developer_concentration(row, col + c);
      }
      // Mirror the row onto the end of padded.
      ...
      for (int pass = 0; pass < order; pass++) {
        ...
        for (int row = pass * convrad; row < (pass * convrad) + convlength; row++) {
          for (int c = 0; c < numcols; ++c) running_sum[c] += hpadded[row][c];
        }
      ...
      // divisions that we never did during our previous averaging.
      for (int row = 0; row < length; row++) {
        for (int c = 0; c < numcols; ++c) developer_concentration(row, col + c) = hpadded[row + pad][c] * swell_factor;
      }
    }
// remaining columns
#pragma omp single
    for (int col = width - (width % numcols); col < width; col++) {
      // Mirror the start of padded from the row.
      for (int row = 0; row < pad; row++) { hpadded[row][0] = developer_concentration(pad - row, col); }
      // Fill the center of padded with a row.
      for (int row = 0; row < length; row++) { hpadded[row + pad][0] = developer_concentration(row, col); }
      // Mirror the row onto the end of padded.
      for (int row = 0; row < pad + 1; row++) {
      ...
      for (int pass = 0; pass < order; pass++) {
        ...
        // Start the running sum going at the beginning of the pad.
        float running_sum = 0;
        for (int row = pass * convrad; row < (pass * convrad) + convlength; row++) { running_sum += hpadded[row][0]; }
  ...
  const float timestep)
{
  ...
  for (int i = 3; i < width; i++) {
    attenuationX[i] = coeff0 +// times 1
                      coeff1 * attenuationX[i - 1] + coeff2 * attenuationX[i - 2] + coeff3 * attenuationX[i - 3];
  }
  // Fill in the padding (which is all zeros)
  for (int i = width; i < paddedWidth; i++) {
    // All zeros, so no coeff0*1 here.
    attenuationX[i] = coeff1 * attenuationX[i - 1] + coeff2 * attenuationX[i - 2] + coeff3 * attenuationX[i - 3];
  }
  // And go back.
  for (int i = paddedWidth - 3 - 1; i >= 0; i--) {
    attenuationX[i] = coeff0 * attenuationX[i] + coeff1 * attenuationX[i + 1] + coeff2 * attenuationX[i + 2]
...
#pragma omp parallel for
  ...
  for (int i = 3; i < height; i++) {
    attenuationY[i] = coeff0 +// times 1
                      coeff1 * attenuationY[i - 1] + coeff2 * attenuationY[i - 2] + coeff3 * attenuationY[i - 3];
  }
  // Fill in the padding (which is all zeros)
  for (int i = height; i < paddedHeight; i++) {
    // All zeros, so no coeff0*1 here.
    attenuationY[i] = coeff1 * attenuationY[i - 1] + coeff2 * attenuationY[i - 2] + coeff3 * attenuationY[i - 3];
  }
  // And go back.
  for (int i = paddedHeight - 3 - 1; i >= 0; i--) {
    attenuationY[i] = coeff0 * attenuationY[i] + coeff1 * attenuationY[i + 1] + coeff2 * attenuationY[i + 2]
...
#pragma omp for schedule(dynamic)
    for (int row = 0; row < height; row++) {
      ...
      for (int col = 0; col < width; col++) { devel_concX[col] = double(developer_concentration(row, col)); }
      // Set up the boundary
      devel_concX[0] = coeff0 * devel_concX[0];
      devel_concX[1] = coeff0 * devel_concX[1] + coeff1 * devel_concX[0];
      devel_concX[2] = coeff0 * devel_concX[2] + coeff1 * devel_concX[1] + coeff2 * devel_concX[0];
      // Iterate over the main part of the image, except for the setup
      for (int col = 3; col < width; col++) {
        devel_concX[col] = coeff0 * devel_concX[col] + coeff1 * devel_concX[col - 1] + coeff2 * devel_concX[col - 2]
                           + coeff3 * devel_concX[col - 3];
      }
      // Iterate over the zeroed tail
      for (int col = width; col < paddedWidth; col++) {
        devel_concX[col] =
          coeff1 * devel_concX[col - 1] + coeff2 * devel_concX[col - 2] + coeff3 * devel_concX[col - 3];
      }
      // And go back
...
#pragma omp for schedule(dynamic)
    for (int slice = 0; slice < slices; slice++) {
      ...
      {
        for (int row = 0; row < height; row++) {
          for (int col = 0; col < iter; col++) { devel_concY(row, col) = developer_concentration(row, col + offset); }
        }
...
#pragma omp simd
      for (int col = 0; col < 8; col++) {
        ...
        devel_concY(1, col) = coeff0 * devel_concY(1, col) + coeff1 * devel_concY(0, col);
        devel_concY(2, col) =
          coeff0 * devel_concY(2, col) + coeff1 * devel_concY(1, col) + coeff2 * devel_concY(0, col);
      }
      // Iterate over the main part of the image, except for the setup.
      for (int row = 3; row < height; row++) {
#pragma omp simd
        for (int col = 0; col < 8; col++) {
          devel_concY(row, col) = coeff0 * devel_concY(row, col) + coeff1 * devel_concY(row - 1, col)
                                  + coeff2 * devel_concY(row - 2, col) + coeff3 * devel_concY(row - 3, col);
        }
      }
      // Iterate over the zeroed tail
...
#pragma omp simd
        for (int col = 0; col < 8; col++) {
          devel_concY(row, col) = coeff1 * devel_concY(row - 1, col) + coeff2 * devel_concY(row - 2, col)
