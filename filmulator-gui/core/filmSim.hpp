...
// Uses Nelder-Mead method to find the WB parameters that yield (1,1,1) RGB
...
void sRGB_linearize(matrix<unsigned short> &RGB, matrix<float> &out);

// Converts linear SRGB to gamma-curved, float to short int.
void sRGB_gammacurve(matrix<float> &RGB, matrix<unsigned short> &out);
