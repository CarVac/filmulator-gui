...
int main(int argc, char *argv[])
{
  ...
  if (imload(input_filename_list, input_exposure_compensation, input_image, tiff, jpeg_in, exifData, highlights)) {
    FILM_ERROR("Error loading images");
    return 1;
  }
  exifData["Exif.Image.ProcessingSoftware"] = "Filmulator";

  FILM_DEBUG("Read time: {} seconds", timeDiff(read_start));
  FILM_INFO("Image loaded.");
  ...
  postprocess(output_density, set_whitepoint, whitepoint, tonecurve_out, std_cutoff, output_r, output_g, output_b);

  // Output the file
  output_file(output_r, output_g, output_b, input_filename_list, jpeg_out, exifData);

  FILM_DEBUG("Total time: {} seconds", timeDiff(total_start));
