...
bool ppmb_write_data(ofstream &output,
  int xsize,
  int ysize,
  matrix<float> &densityr,
  ...
  bool sixteen_bit)
{
  ...
  if (!output) {
    FILM_ERROR("PPMB_WRITE: Fatal error! Cannot open the output file {}", output_name);
    return;
  }
  //
  //  Compute the maximum.
  ...
  if (error) {
    FILM_ERROR("PPMB_WRITE: Fatal error! PPMB_WRITE_HEADER failed.");
    return;
  }
  //
  //  Write the data.
  //
  error = ppmb_write_data(output, xsize, ysize, densityr, densityg, densityb, sixteen_bit);

  if (error) {
    FILM_ERROR("PPMB_WRITE: Fatal error! PPMB_WRITE_DATA failed.");
    return;
  }
  //
  //  Close the file.
