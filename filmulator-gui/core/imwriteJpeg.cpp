...
static void remove_exif_keys(Exiv2::ExifData &exifData, const char *keys[], unsigned int n_keys)
{
  for (unsigned int i = 0; i < n_keys; i++) {
    try {
      ...
      while ((pos = exifData.findKey(Exiv2::ExifKey(keys[i]))) != exifData.end()) { exifData.erase(pos); }
    } catch (Exiv2::Error &e) {
      // catch invalid tag
    }
  }
...
void cleanExif(Exiv2::ExifData &exifData)
{
  ...
  thumb.erase();
  {
    static const char *keys[] = { "Exif.Thumbnail.Compression",
  ...
  {
    static const char *keys[] = { "Exif.Image.ImageWidth",
  ...
  {
    static const char *keys[] = { // Canon color space info
...
#if EXIV2_MINOR_VERSION >= 23
  {
    ...
    static const char *keys[] = { "Exif.Samsung2.SensorAreas",
  ...
  {
    static const char *keys[] = { // Embedded color profile info
