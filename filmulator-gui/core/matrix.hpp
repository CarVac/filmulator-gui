...
private:
  ...
  inline void transpose_block_SSE4x4(float *A,
    float *B,
    const int n,
    const int m,
    const int lda,
...
#endif
  inline void transpose_scalar_block(float *A, float *B, const int lda, const int ldb, const int block_size) const;
  inline void transpose_block(float *A,
    float *B,
    const int n,
    const int m,
    const int lda,
...
template<class T> matrix<T>::matrix(const int nrows, const int ncols)
{
  ...
  if (nrows == 0 || ncols == 0) {
    ...
    ptr = new T *[nrows];
    for (int row = 0; row < nrows; row++) ptr[row] = data + row * num_cols;
  }
}
...
template<class T> matrix<T>::matrix(const matrix<T> &toCopy)
{
  ...
  ptr = new T *[num_rows];
  for (int row = 0; row < num_rows; row++) ptr[row] = data + row * num_cols;
...
{
  assert(nrows >= 0 && ncols >= 0);
  if (num_rows == nrows && num_cols == ncols) { return; }
  ...
  if (ua_data == nullptr) std::cout << "matrix::set_size memory could not be alloc'd" << std::endl;
  void *buffer = ua_data;
  void *aligned_ptr = std::align(16, sizeof(T), buffer, ua_num_elements);
  data = static_cast<T *>(aligned_ptr);
...
template<class T> void matrix<T>::free() { set_size(0, 0); }

template<class T> int matrix<T>::nr() const { return num_rows; }

template<class T> int matrix<T>::nc() const { return num_cols; }
...
template<class T> void matrix<T>::swap(matrix<T> &swapTarget)
{
  if (this != &swapTarget) {
...
#pragma omp parallel for shared(pdata, pnum_cols)
  for (int row = 0; row < num_rows; row++)
    for (int col = 0; col < num_cols; col++) result.data[row * num_cols + col] = data[row * num_cols + col] + value;
  return result;
}
...
template<class T> inline void matrix<T>::fast_transpose_to(const matrix<T> &target) const { slow_transpose_to(target); }
#endif

template<class T> inline void matrix<T>::transpose_to(const matrix<T> &target) const { slow_transpose_to(target); }
...
#ifdef __SSE2__
template<class T> inline void matrix<T>::transpose4x4_SSE(float *A, float *B, const int lda, const int ldb) const
{
...
template<class T>
inline void matrix<T>::transpose_block_SSE4x4(float *A,
  float *B,
  const int n,
  const int m,
  const int lda,
  ...
  for (int i = 0; i < n; i += block_size)
    for (int j = 0; j < m; j += block_size) {
      ...
      for (int i2 = i; i2 < max_i2; i2 += 4)
        for (int j2 = j; j2 < max_j2; j2 += 4) transpose4x4_SSE(&A[i2 * lda + j2], &B[j2 * ldb + i2], lda, ldb);
    }
}
#endif
...
template<class T>
inline void
  matrix<T>::transpose_scalar_block(float *A, float *B, const int lda, const int ldb, const int block_size) const
{
#pragma omp parallel for
  for (int i = 0; i < block_size; i++) {
    for (int j = 0; j < block_size; j++) { B[j * ldb + i] = A[i * lda + j]; }
  }
}

template<class T>
inline void matrix<T>::transpose_block(float *A,
  float *B,
  const int n,
  const int m,
  const int lda,
...
template<class T> double sum(matrix<T> &mat) { return mat.sum(); }

template<class T> T max(matrix<T> &mat) { return mat.max(); }

template<class T> T min(matrix<T> &mat) { return mat.min(); }


template<class T> double mean(matrix<T> &mat) { return mat.mean(); }
...
template<class T, class U> const matrix<T> operator+(const matrix<T> &mat, const U value) { return mat.add(value); }
...
template<class T, class U> const matrix<T> operator*(const matrix<T> &mat, const U value) { return mat.mult(value); }

template<class T, class U> const matrix<T> operator*=(matrix<T> &mat, const U value) { return mat.mult_this(value); }

template<class T, class U> const matrix<T> operator/(const matrix<T> &mat, const U value) { return mat.divide(value); }
