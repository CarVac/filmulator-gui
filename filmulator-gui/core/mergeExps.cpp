  ...
  float input_exposure_comp)
{
  ...
  while (i < max(max(numrows, numcols / 3), 300)) {
    ...
    if (input_image(ycoord, xcoord * 3) / init_exp_factor < 60000
        ...
        && input_image(ycoord, xcoord * 3 + 2) / init_exp_factor < 60000 && temp_image(ycoord, xcoord * 3) < 60000
        && temp_image(ycoord, xcoord * 3 + 1) < 60000 && temp_image(ycoord, xcoord * 3 + 2) < 60000) {
      input_sum += double(input_image(ycoord, xcoord * 3)) + double(input_image(ycoord, xcoord * 3 + 1))
                   + double(input_image(ycoord, xcoord * 3 + 2));
      temp_sum += double(temp_image(ycoord, xcoord * 3)) + double(temp_image(ycoord, xcoord * 3 + 1))
  ...
  for (int col = 0; col < numcols / 3; col++) {
    ...
    for (int row = 0; row < numrows; row++) {
      temp_max_channel = max(max(temp_image(row, colr), temp_image(row, colg)), temp_image(row, colb));
      ...
      input_image(row, colr) = (input_image(row, colr) * exposure_weight
                                 + temp_image(row, colr) * exp_factor * new_exp_weight * exp_weight_factor)
                               / (exposure_weight + new_exp_weight * exp_weight_factor);
      input_image(row, colg) = (input_image(row, colg) * exposure_weight
                                 + temp_image(row, colg) * exp_factor * new_exp_weight * exp_weight_factor)
                               / (exposure_weight + new_exp_weight * exp_weight_factor);
      input_image(row, colb) = (input_image(row, colb) * exposure_weight
                                 + temp_image(row, colb) * exp_factor * new_exp_weight * exp_weight_factor)
                               / (exposure_weight + new_exp_weight * exp_weight_factor);
    }
  }
