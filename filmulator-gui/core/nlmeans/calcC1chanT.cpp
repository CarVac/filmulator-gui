...
template<class MatT>
Eigen::Matrix<typename MatT::Scalar, MatT::ColsAtCompileTime, MatT::RowsAtCompileTime> pseudoinverse(const MatT &mat,
...
std::vector<float> calcC1ChanT(std::vector<float> centers, int numClusters, const float h)
{
  ...
  for (int row = 0; row < numClusters; row++) {
    for (int col = 0; col < numClusters; col++) {
      ...
      C1(col, row) =
        std::exp(-squaredDistance / (2 * h * h));// We're assigning to (col,row) in order to do the transpose
    }
  }
