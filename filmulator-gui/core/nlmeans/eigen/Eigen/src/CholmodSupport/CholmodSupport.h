 ...
 * The data are not copied but shared. */
template<typename Derived> cholmod_dense viewAsCholmod(MatrixBase<Derived> &mat)
{
  EIGEN_STATIC_ASSERT(
    (internal::traits<Derived>::Flags & RowMajorBit) == 0, THIS_METHOD_IS_ONLY_FOR_COLUMN_MAJOR_MATRICES);
 ...
 */
template<typename _MatrixType, int _UpLo, typename Derived> class CholmodBase : public SparseSolverBase<Derived>
{
protected:
...
public:
  ...
  ~CholmodBase()
  {
    if (m_cholmodFactor) cholmod_free_factor(&m_cholmodFactor, &m_cholmod);
