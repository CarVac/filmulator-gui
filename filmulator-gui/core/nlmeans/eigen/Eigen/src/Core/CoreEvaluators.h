...
namespace internal {
  ...
  template<typename ExpressionType> struct evaluator_base : public noncopyable
  {
  ...
  template<typename Derived> struct evaluator<PlainObjectBase<Derived>> : evaluator_base<Derived>
  {
    ...
    EIGEN_DEVICE_FUNC evaluator()
      : m_data(0), m_outerStride(IsVectorAtCompileTime ? 0
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType coeff(Index index) const { return m_data[index]; }
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Scalar &coeffRef(Index index) { return const_cast<Scalar *>(m_data)[index]; }
    ...
    : evaluator<PlainObjectBase<Matrix<Scalar, Rows, Cols, Options, MaxRows, MaxCols>>>
  {
    ...
    EIGEN_DEVICE_FUNC explicit evaluator(const XprType &m) : evaluator<PlainObjectBase<XprType>>(m) {}
  };
...
struct nullary_wrapper<Scalar,NullaryOp,true,true,true>
{
    ...
    : evaluator_base<CwiseNullaryOp<NullaryOp, PlainObjectType>>
  {
    ...
    template<typename IndexType> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType coeff(IndexType index) const
    {
  ...
  struct unary_evaluator<CwiseUnaryOp<UnaryOp, ArgType>, IndexBased> : evaluator_base<CwiseUnaryOp<UnaryOp, ArgType>>
  {
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE explicit unary_evaluator(const XprType &op)
    ...
    : evaluator_base<CwiseTernaryOp<TernaryOp, Arg1, Arg2, Arg3>>
  {
    ...
    enum {
      CoeffReadCost = evaluator<Arg1>::CoeffReadCost + evaluator<Arg2>::CoeffReadCost + evaluator<Arg3>::CoeffReadCost
      ...
      Alignment = EIGEN_PLAIN_ENUM_MIN(EIGEN_PLAIN_ENUM_MIN(evaluator<Arg1>::Alignment, evaluator<Arg2>::Alignment),
        evaluator<Arg3>::Alignment)
    };

    EIGEN_DEVICE_FUNC explicit ternary_evaluator(const XprType &xpr)
      : m_functor(xpr.functor()), m_arg1Impl(xpr.arg1()), m_arg2Impl(xpr.arg2()), m_arg3Impl(xpr.arg3())
    {
    ...
    : evaluator_base<CwiseBinaryOp<BinaryOp, Lhs, Rhs>>
  {
    ...
    template<int LoadMode, typename PacketType> EIGEN_STRONG_INLINE PacketType packet(Index index) const
    {
      return m_functor.packetOp(
        m_lhsImpl.template packet<LoadMode, PacketType>(index), m_rhsImpl.template packet<LoadMode, PacketType>(index));
    }
    ...
    : evaluator_base<CwiseUnaryView<UnaryOp, ArgType>>
  {
    ...
    EIGEN_DEVICE_FUNC explicit unary_evaluator(const XprType &op)
      : m_unaryOp(op.functor()), m_argImpl(op.nestedExpression())
    {
      EIGEN_INTERNAL_CHECK_COST_VALUE(functor_traits<UnaryOp>::Cost);
      EIGEN_INTERNAL_CHECK_COST_VALUE(CoeffReadCost);
    }
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Scalar &coeffRef(Index index) { return m_unaryOp(m_argImpl.coeffRef(index)); }
  ...
  template<typename Derived, typename PlainObjectType> struct mapbase_evaluator : evaluator_base<Derived>
  {
    ...
    {
      EIGEN_STATIC_ASSERT(EIGEN_IMPLIES(evaluator<Derived>::Flags & PacketAccessBit,
    ...
    : public mapbase_evaluator<Ref<PlainObjectType, RefOptions, StrideType>, PlainObjectType>
  {
    ...
    EIGEN_DEVICE_FUNC explicit evaluator(const XprType &ref) : mapbase_evaluator<XprType, PlainObjectType>(ref) {}
  };
    ...
    : unary_evaluator<Block<ArgType, BlockRows, BlockCols, InnerPanel>>
  {
    ...
    EIGEN_DEVICE_FUNC explicit block_evaluator(const XprType &block) : unary_evaluator<XprType>(block) {}
  };
    ...
    : evaluator_base<Block<ArgType, BlockRows, BlockCols, InnerPanel>>
  {
    ...
    EIGEN_DEVICE_FUNC explicit unary_evaluator(const XprType &block)
      : m_argImpl(block.nestedExpression()), m_startRow(block.startRow()), m_startCol(block.startCol()),
        m_linear_offset(
          InnerPanel ? (XprType::IsRowMajor ? block.startRow() * block.cols() : block.startCol() * block.rows()) : 0)
    {}
        ...
        typename Block<ArgType, BlockRows, BlockCols, InnerPanel>::PlainObject>
  {
    ...
    {
      // TODO: for the 3.3 release, this should be turned to an internal assertion, but let's keep it as is for the beta
      // lifetime
      eigen_assert(((internal::UIntPtr(block.data()) % EIGEN_PLAIN_ENUM_MAX(1, evaluator<XprType>::Alignment)) == 0)
                   && "data is not aligned");
    }
  };
    ...
    : evaluator_base<Replicate<ArgType, RowFactor, ColFactor>>
  {
    ...
    EIGEN_DEVICE_FUNC explicit unary_evaluator(const XprType &replicate)
      : m_arg(replicate.nestedExpression()), m_argImpl(m_arg), m_rows(replicate.nestedExpression().rows()),
    ...
    : evaluator_base<PartialReduxExpr<ArgType, MemberOp, Direction>>
  {
    ...
    enum {
      CoeffReadCost = TraversalSize == Dynamic
                        ? HugeCost
  ...
  struct unary_evaluator<Reverse<ArgType, Direction>> : evaluator_base<Reverse<ArgType, Direction>>
  {
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Scalar &coeffRef(Index row, Index col)
    {
      return m_argImpl.coeffRef(
        ReverseRow ? m_rows.value() - row - 1 : row, ReverseCol ? m_cols.value() - col - 1 : col);
    }
