...
namespace internal {
  ...
  template<typename Scalar, int Size, int MaxSize> struct gemv_static_vector_if<Scalar, Size, MaxSize, true>
  {
    ...
    internal::
      plain_array<Scalar, EIGEN_SIZE_MIN_PREFER_FIXED(Size, MaxSize) + (ForceAlignment ? EIGEN_MAX_ALIGN_BYTES : 0), 0>
        m_data;
    EIGEN_STRONG_INLINE Scalar *data()
    {
  ...
  template<> struct gemv_dense_selector<OnTheRight, ColMajor, true>
  {
    ...
    static inline void run(const Lhs &lhs, const Rhs &rhs, Dest &dest, const typename Dest::Scalar &alpha)
    {
      ...
      ResScalar actualAlpha = alpha * LhsBlasTraits::extractScalarFactor(lhs) * RhsBlasTraits::extractScalarFactor(rhs);
  ...
  template<> struct gemv_dense_selector<OnTheRight, RowMajor, true>
  {
    ...
    static void run(const Lhs &lhs, const Rhs &rhs, Dest &dest, const typename Dest::Scalar &alpha)
    {
      ...
      general_matrix_vector_product<Index,
        ...
        dest.data(),
        dest.col(0).innerStride(),// NOTE  if dest is not a vector at compile-time, then dest.innerStride() might be
                                  // wrong. (bug 1166)
        actualAlpha);
    }
  };
...
inline const Product<Derived, OtherDerived> MatrixBase<Derived>::operator*(const MatrixBase<OtherDerived> &other) const
{
...
template<typename Derived>
template<typename OtherDerived>
const Product<Derived, OtherDerived, LazyProduct> MatrixBase<Derived>::lazyProduct(
