...
inline typename internal::enable_if<!(internal::is_same<typename Derived::Scalar, Scalar>::value)
  ...
  const EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE(Scalar, Derived, pow)>::type pow(const Scalar &x,
  ...
  pow(const typename Derived::Scalar &x, const Eigen::ArrayBase<Derived> &exponents)
{
  return EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE(typename Derived::Scalar, Derived, pow)(
    typename internal::plain_constant_type<Derived, typename Derived::Scalar>::type(
      exponents.rows(), exponents.cols(), x),
    exponents.derived());
}
#endif
