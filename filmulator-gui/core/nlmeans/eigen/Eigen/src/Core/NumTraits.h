...
namespace internal {
  ...
  template<typename T> struct default_digits10_impl<T, false, false>// Floating point
  {
    static int run()
    {
      using std::log10;
  ...
  template<typename T> struct default_digits10_impl<T, false, true>// Integer
  {
 ...
 * used as the default implementation if specialized.
 */
...
template<typename T> struct GenericNumTraits
{
  ...
  EIGEN_DEVICE_FUNC
  static inline int digits10() { return internal::default_digits10_impl<T>::run(); }
...
template<typename _Real> struct NumTraits<std::complex<_Real>> : GenericNumTraits<std::complex<_Real>>
{
  typedef _Real Real;
