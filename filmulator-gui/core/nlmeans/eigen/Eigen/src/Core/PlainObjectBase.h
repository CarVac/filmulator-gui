...
#define EIGEN_INITIALIZE_COEFFS_IF_THAT_OPTION_IS_ENABLED \
  for (int i = 0; i < base().size(); ++i) coeffRef(i) = Scalar(0);
#elif defined(EIGEN_INITIALIZE_MATRICES_BY_NAN)
#define EIGEN_INITIALIZE_COEFFS
#define EIGEN_INITIALIZE_COEFFS_IF_THAT_OPTION_IS_ENABLED \
  for (int i = 0; i < base().size(); ++i) coeffRef(i) = std::numeric_limits<Scalar>::quiet_NaN();
#else
...
public:
  ...
  {
    eigen_assert(
      EIGEN_IMPLIES(RowsAtCompileTime != Dynamic, rows == RowsAtCompileTime)
...
public:
  ...
  template<int Outer, int Inner>
  static inline typename StridedConstAlignedMapType<Stride<Outer, Inner>>::type MapAligned(const Scalar *data,
    const Stride<Outer, Inner> &stride)
  {
    return typename StridedConstAlignedMapType<Stride<Outer, Inner>>::type(data, stride);
  }
  template<int Outer, int Inner>
  static inline typename StridedAlignedMapType<Stride<Outer, Inner>>::type MapAligned(Scalar *data,
    const Stride<Outer, Inner> &stride)
  {
...
protected:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _resize_to_match(const EigenBase<OtherDerived> &other)
  {
    ...
    eigen_assert((this->size() == 0
                   || (IsVectorAtCompileTime ? (this->size() == other.size())
  ...
  {
    EIGEN_STATIC_ASSERT(bool(NumTraits<T0>::IsInteger) && bool(NumTraits<T1>::IsInteger),
  ...
  template<typename T0, typename T1>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init2(const Index &val0,
    const Index &val1,
  ...
  template<typename T>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init1(Index size,
      ...
      T>::type * = 0)
  {
    ...
    EIGEN_STATIC_ASSERT(is_integer, FLOATING_POINT_ARGUMENT_PASSED__INTEGER_WAS_EXPECTED)
    resize(size);
  }

  // We have a 1x1 matrix/array => the argument is interpreted as the value of the unique coefficient (case where scalar
  // type can be implicitely converted)
  template<typename T>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init1(const Scalar &val0,
    typename internal::enable_if<Base::SizeAtCompileTime == 1 && internal::is_convertible<T, Scalar>::value, T>::type
      * = 0)
  {
  ...
  template<typename T>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init1(const Index &val0,
  ...
  template<typename T> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init1(const Scalar *data)
  {
  ...
  template<typename T> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init1(const Derived &other)
  {
    this->_set_noalias(other);
  }
  ...
  template<typename T>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void _init1(const Scalar &val0,
    typename internal::enable_if<Base::SizeAtCompileTime != Dynamic && Base::SizeAtCompileTime != 1
...
public:
   ...
   */
  template<typename OtherDerived> EIGEN_DEVICE_FUNC void swap(DenseBase<OtherDerived> &other)
  {
   ...
   */
  template<typename OtherDerived> EIGEN_DEVICE_FUNC void swap(DenseBase<OtherDerived> const &other)
  {
