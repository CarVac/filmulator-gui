    ...
    : public traits<Map<_PlainObjectType, _Options, _StrideType>>
  {
    ...
    template<typename Derived> struct match
    {
      enum {
        ...
        OuterStrideMatch = Derived::IsVectorAtCompileTime || int(StrideType::OuterStrideAtCompileTime) == int(Dynamic)
...
protected:
  ...
  template<typename Expression> EIGEN_DEVICE_FUNC void construct(Expression &expr)
  {
 ...
 * \c #Aligned16, \c #Aligned8 or \c #Unaligned. The default is \c #Unaligned.
 ...
 * number of copies. A Ref<> object can represent either a const expression or a l-value:
 * \code
 * // in-out argument:
 ...
 * in memory. To overcome this issue, one might propose to overload internally calling a template function, e.g.:
 * \code
 * // in the .h:
...
public:
  ...
  template<typename Derived> inline Ref(DenseBase<Derived> &expr)
#endif
  {
...
public:
  ...
  template<typename OtherRef> EIGEN_DEVICE_FUNC inline Ref(const RefBase<OtherRef> &other)
  {
    construct(other.derived(), typename Traits::template match<OtherRef>::type());
  }

protected:
  template<typename Expression> EIGEN_DEVICE_FUNC void construct(const Expression &expr, internal::true_type)
  {
    Base::construct(expr);
  }

  template<typename Expression> EIGEN_DEVICE_FUNC void construct(const Expression &expr, internal::false_type)
  {
