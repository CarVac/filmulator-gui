...
namespace internal {
  ...
  public:
    ...
    EIGEN_DEVICE_FUNC generic_dense_assignment_kernel(DstEvaluatorTypeT &dst,
    ...
    template<int StoreMode, int LoadMode, typename PacketType> void assignPacket(Index row, Index col)
    {
    ...
    template<int StoreMode, int LoadMode, typename PacketType> void assignPacket(Index index)
    {
    ...
    // mean no CRTP (Gael)
    template<int StoreMode, int LoadMode, typename PacketType> void assignPacketByOuterInner(Index outer, Index inner)
    {
