...
public:
  ...
  /** \returns the nested expression */
  EIGEN_DEVICE_FUNC
  typename internal::remove_reference<MatrixTypeNested>::type &nestedExpression() { return m_matrix; }

  /** \internal */
  void resize(Index nrows, Index ncols) { m_matrix.resize(ncols, nrows); }
...
template<typename Derived> inline Transpose<Derived> DenseBase<Derived>::transpose()
{
...
inline typename DenseBase<Derived>::ConstTransposeReturnType DenseBase<Derived>::transpose() const
{
...
namespace internal {
  ...
  template<typename MatrixType,
    bool IsSquare =
      (MatrixType::RowsAtCompileTime == MatrixType::ColsAtCompileTime) && MatrixType::RowsAtCompileTime != Dynamic,
  ...
  {// square matrix
    static void run(MatrixType &m) { m.matrix().template triangularView<StrictlyUpper>().swap(m.matrix().transpose()); }
  };
...
template<typename Derived> inline void DenseBase<Derived>::transposeInPlace()
{
...
template<typename Derived> inline void MatrixBase<Derived>::adjointInPlace() { derived() = adjoint().eval(); }
