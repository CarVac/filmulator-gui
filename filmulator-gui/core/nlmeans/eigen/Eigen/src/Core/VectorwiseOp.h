...
public:
  ...
  EIGEN_DEVICE_FUNC
  explicit PartialReduxExpr(const MatrixType &mat, const MemberOp &func = MemberOp()) : m_matrix(mat), m_functor(func)
  {}
...
#define EIGEN_MEMBER_FUNCTOR(MEMBER, COST)                                                                           \
  template<typename ResultType> struct member_##MEMBER                                                               \
  {                                                                                                                  \
    ...
    template<typename XprType> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ResultType operator()(const XprType &mat) const \
    {                                                                                                                \
      return mat.MEMBER();                                                                                           \
    }                                                                                                                \
  }
...
namespace internal {
  ...
  template<int p, typename ResultType> struct member_lpnorm
  {
    typedef ResultType result_type;
    template<typename Scalar, int Size> struct Cost
    {
    ...
    template<typename XprType> EIGEN_DEVICE_FUNC inline ResultType operator()(const XprType &mat) const
    {
      return mat.template lpNorm<p>();
    }
  };

  template<typename BinaryOp, typename Scalar> struct member_redux
  {
    ...
    template<typename Derived> EIGEN_DEVICE_FUNC inline result_type operator()(const DenseBase<Derived> &mat) const
    {
...
public:
  ...
  template<template<typename _Scalar> class Functor, typename Scalar_ = Scalar> struct ReturnType
  {
...
protected:
  typedef
    typename internal::conditional<isVertical, typename ExpressionType::ColXpr, typename ExpressionType::RowXpr>::type
      SubVector;
  /** \internal
   * \returns the i-th subvector according to the \c Direction */
  EIGEN_DEVICE_FUNC
  SubVector subVector(Index i) { return SubVector(m_matrix.derived(), i); }

  /** \internal
   * \returns the number of subvectors in the direction \c Direction */
  EIGEN_DEVICE_FUNC
  Index subVectors() const { return isVertical ? m_matrix.cols() : m_matrix.rows(); }
...
public:
  ...
  EIGEN_DEVICE_FUNC
  const MinCoeffReturnType minCoeff() const { return MinCoeffReturnType(_expression()); }
  ...
  EIGEN_DEVICE_FUNC
  const MaxCoeffReturnType maxCoeff() const { return MaxCoeffReturnType(_expression()); }
  ...
  EIGEN_DEVICE_FUNC
  const SquaredNormReturnType squaredNorm() const { return SquaredNormReturnType(_expression()); }
   ...
   * \sa DenseBase::norm() */
  EIGEN_DEVICE_FUNC
  const NormReturnType norm() const { return NormReturnType(_expression()); }
   ...
   *
   * \sa DenseBase::norm() */
  template<int p> EIGEN_DEVICE_FUNC const typename LpNormReturnType<p>::Type lpNorm() const
  {
   ...
   * \sa DenseBase::mean() */
  EIGEN_DEVICE_FUNC
  const MeanReturnType mean() const { return MeanReturnType(_expression()); }
  ...
  template<typename OtherDerived> EIGEN_DEVICE_FUNC ExpressionType &operator=(const DenseBase<OtherDerived> &other)
  {
  ...
  typedef Block<const ExpressionType,
    Direction == Vertical ? int(HNormalized_SizeMinusOne) : int(internal::traits<ExpressionType>::RowsAtCompileTime),
    Direction == Horizontal ? int(HNormalized_SizeMinusOne) : int(internal::traits<ExpressionType>::ColsAtCompileTime)>
    HNormalized_Block;
  typedef Block<const ExpressionType,
 ...
 */
template<typename Derived> inline typename DenseBase<Derived>::ColwiseReturnType DenseBase<Derived>::colwise()
{
 ...
 */
template<typename Derived> inline typename DenseBase<Derived>::RowwiseReturnType DenseBase<Derived>::rowwise()
{
