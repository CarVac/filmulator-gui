...
EIGEN_DEVICE_FUNC void DenseBase<Derived>::visit(Visitor &visitor) const
{
  ...
  enum {
    unroll = SizeAtCompileTime != Dynamic
             && SizeAtCompileTime * ThisEvaluator::CoeffReadCost
                    + (SizeAtCompileTime - 1) * internal::functor_traits<Visitor>::Cost
...
namespace internal {
   ...
   */
  template<typename Derived> struct min_coeff_visitor : coeff_visitor<Derived>
  {
    typedef typename Derived::Scalar Scalar;
    EIGEN_DEVICE_FUNC
    void operator()(const Scalar &value, Index i, Index j)
  ...
  template<typename Scalar> struct functor_traits<min_coeff_visitor<Scalar>>
  {
   ...
   */
  template<typename Derived> struct max_coeff_visitor : coeff_visitor<Derived>
  {
    typedef typename Derived::Scalar Scalar;
    EIGEN_DEVICE_FUNC
    void operator()(const Scalar &value, Index i, Index j)
  ...
  template<typename Scalar> struct functor_traits<max_coeff_visitor<Scalar>>
  {
    enum { Cost = NumTraits<Scalar>::AddCost };
  };
