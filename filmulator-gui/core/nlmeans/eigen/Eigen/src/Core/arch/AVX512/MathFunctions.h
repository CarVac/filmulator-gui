...
#define _EIGEN_DECLARE_CONST_Packet16f(NAME, X) const Packet16f p16f_##NAME = pset1<Packet16f>(X)

#define _EIGEN_DECLARE_CONST_Packet16f_FROM_INT(NAME, X) const Packet16f p16f_##NAME = (__m512)pset1<Packet16i>(X)

#define _EIGEN_DECLARE_CONST_Packet8d(NAME, X) const Packet8d p8d_##NAME = pset1<Packet8d>(X)
...
// be easily approximated by a polynomial centered on m=1 for stability.
  ...
  template<>
  EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_UNUSED Packet16f plog<Packet16f>(const Packet16f &_x)
  {
    Packet16f x = _x;
    ...
    __mmask16 iszero_mask = _mm512_cmp_ps_mask(x, _mm512_setzero_ps(), _CMP_EQ_UQ);
    ...
    return _mm512_mask_blend_ps(iszero_mask, _mm512_mask_blend_ps(invalid_mask, x, p16f_nan), p16f_minus_inf);
  }
#endif
  ...
  template<>
  EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_UNUSED Packet16f pexp<Packet16f>(const Packet16f &_x)
  {
    _EIGEN_DECLARE_CONST_Packet16f(1, 1.0f);
    _EIGEN_DECLARE_CONST_Packet16f(half, 0.5f);
...
// further reducing its effective latency.
#if EIGEN_FAST_MATH
  template<>
  EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_UNUSED Packet16f psqrt<Packet16f>(const Packet16f &_x)
  {
  ...
  template<>
  EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_UNUSED Packet8d psqrt<Packet8d>(const Packet8d &_x)
  {
...
#else
  template<> EIGEN_STRONG_INLINE Packet16f psqrt<Packet16f>(const Packet16f &x) { return _mm512_sqrt_ps(x); }
  template<> EIGEN_STRONG_INLINE Packet8d psqrt<Packet8d>(const Packet8d &x) { return _mm512_sqrt_pd(x); }
#endif
...
// computing the reciprocal square root in AVX-512.
  ...
  template<>
  EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_UNUSED Packet16f prsqrt<Packet16f>(const Packet16f &_x)
  {
    ...
    Packet16f infs_and_nans =
      _mm512_mask_blend_ps(neg_mask, _mm512_mask_blend_ps(le_zero_mask, _mm512_setzero_ps(), p16f_inf), p16f_nan);
  ...
  template<>
  EIGEN_DEFINE_FUNCTION_ALLOWING_MULTIPLE_DEFINITIONS EIGEN_UNUSED Packet8d prsqrt<Packet8d>(const Packet8d &_x)
  {
    ...
    Packet8d infs_and_nans =
      _mm512_mask_blend_pd(neg_mask, _mm512_mask_blend_pd(le_zero_mask, _mm512_setzero_pd(), p8d_inf), p8d_nan);
  ...
  template<> EIGEN_STRONG_INLINE Packet16f prsqrt<Packet16f>(const Packet16f &x) { return _mm512_rsqrt28_ps(x); }
#endif
#endif
