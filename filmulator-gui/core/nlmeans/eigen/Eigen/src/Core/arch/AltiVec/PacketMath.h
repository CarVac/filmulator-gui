...
#define EIGEN_PPC_PREFETCH(ADDR) asm("   dcbt [%[addr]]\n" ::[addr] "r"(ADDR) : "cc");
  ...
  inline std::ostream &operator<<(std::ostream &s, const Packet16uc &v)
  {
    ...
    vt.v = v;
    for (int i = 0; i < 16; i++) s << (int)vt.n[i] << ", ";
    return s;
  }
  ...
  template<> EIGEN_STRONG_INLINE Packet4f ploaddup<Packet4f>(const float *from)
  {
    ...
    if ((std::ptrdiff_t(from) % 16) == 0)
      p = pload<Packet4f>(from);
    else
      p = ploadu<Packet4f>(from);
  ...
  template<> EIGEN_STRONG_INLINE Packet4i ploaddup<Packet4i>(const int *from)
  {
    Packet4i p;
    if ((std::ptrdiff_t(from) % 16) == 0)
      p = pload<Packet4i>(from);
    else
      p = ploadu<Packet4i>(from);
    return vec_perm(p, p, p16uc_DUPLICATE32_HI);
  }
  ...
  template<int Offset> struct palign_impl<Offset, Packet4f>
  {
    static EIGEN_STRONG_INLINE void run(Packet4f &first, const Packet4f &second)
    {
      ...
      switch (Offset % 4) {
      case 1:
        first = vec_sld(second, first, 12);
        break;
      case 2:
        first = vec_sld(second, first, 8);
        break;
      case 3:
        first = vec_sld(second, first, 4);
        break;
      }
#endif
    }
    ...
    const Packet4i &elsePacket)
  {
    ...
    Packet4ui mask =
      reinterpret_cast<Packet4ui>(vec_cmpeq(reinterpret_cast<Packet4ui>(select), reinterpret_cast<Packet4ui>(p4i_ONE)));
...
//---------- double ----------
  ...
  template<int Offset> struct palign_impl<Offset, Packet2d>
  {
    static EIGEN_STRONG_INLINE void run(Packet2d &first, const Packet2d &second)
    {
      if (Offset == 1)
#ifdef _BIG_ENDIAN
        first = reinterpret_cast<Packet2d>(
