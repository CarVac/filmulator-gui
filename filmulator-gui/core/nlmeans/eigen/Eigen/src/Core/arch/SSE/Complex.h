...
// to leverage AVX instructions.
  ...
  template<> EIGEN_STRONG_INLINE Packet2cf pmul<Packet2cf>(const Packet2cf &a, const Packet2cf &b)
  {
    ...
    return Packet2cf(_mm_addsub_ps(
      _mm_mul_ps(_mm_moveldup_ps(a.v), b.v), _mm_mul_ps(_mm_movehdup_ps(a.v), vec4f_swizzle1(b.v, 1, 0, 3, 2))));
...
#pragma GCC diagnostic pop
    ...
    pscatter<std::complex<float>, Packet2cf>(std::complex<float> *to, const Packet2cf &from, Index stride)
  {
    to[stride * 0] = std::complex<float>(
      _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 0)), _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 1)));
    to[stride * 1] = std::complex<float>(
      _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 2)), _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 3)));
  }
  ...
  template<int Offset> struct palign_impl<Offset, Packet2cf>
  {
  ...
  template<> struct conj_helper<Packet2cf, Packet2cf, false, true>
  {
    ...
    EIGEN_STRONG_INLINE Packet2cf pmul(const Packet2cf &a, const Packet2cf &b) const
    {
      ...
      return Packet2cf(_mm_add_ps(_mm_xor_ps(_mm_mul_ps(vec4f_swizzle1(a.v, 0, 0, 2, 2), b.v), mask),
        _mm_mul_ps(vec4f_swizzle1(a.v, 1, 1, 3, 3), vec4f_swizzle1(b.v, 1, 0, 3, 2))));
#endif
    }
  };
...
// to leverage AVX instructions.
  ...
  template<> EIGEN_STRONG_INLINE Packet1cd pmul<Packet1cd>(const Packet1cd &a, const Packet1cd &b)
  {
    ...
    return Packet1cd(_mm_addsub_pd(
      _mm_mul_pd(_mm_movedup_pd(a.v), b.v), _mm_mul_pd(vec2d_swizzle1(a.v, 1, 1), vec2d_swizzle1(b.v, 1, 0))));
#else
  ...
  template<int Offset> struct palign_impl<Offset, Packet1cd>
  {
