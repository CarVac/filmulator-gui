...
#define EIGEN_HAS_SINGLE_INSTRUCTION_MADD 1
  ...
  // structure:
  template<typename T> struct eigen_packet_wrapper
  {
...
#define vec2d_swizzle1(v, p, q) \
  (_mm_castsi128_pd(            \
    _mm_shuffle_epi32(_mm_castpd_si128(v), ((q * 2 + 1) << 6 | (q * 2) << 4 | (p * 2 + 1) << 2 | (p * 2)))))
...
// to leverage AVX instructions.
  ...
  // TODO: let's check whether there does not exist a better fix, like adding a pset0() function. (it crashed on
  // pset1(0)).
...
// Also note that with AVX, we want it to generate a vbroadcastss.
  ...
  template<> EIGEN_STRONG_INLINE Packet4i pmul<Packet4i>(const Packet4i &a, const Packet4i &b)
  {
    ...
    return vec4i_swizzle1(
      vec4i_swizzle2(
        _mm_mul_epu32(a, b), _mm_mul_epu32(vec4i_swizzle1(a, 1, 0, 3, 2), vec4i_swizzle1(b, 1, 0, 3, 2)), 0, 2, 0, 2),
      0,
      2,
      1,
      3);
...
// }
  ...
  template<int Offset> struct palign_impl<Offset, Packet4i>
  {
    ...
    {
      if (Offset != 0) first = _mm_alignr_epi8(second, first, Offset * 4);
    }
  };
  ...
  template<int Offset> struct palign_impl<Offset, Packet2d>
  {
    ...
    {
      if (Offset == 1) first = _mm_castsi128_pd(_mm_alignr_epi8(_mm_castpd_si128(second), _mm_castpd_si128(first), 8));
    }
  };
#else
  // SSE2 versions
