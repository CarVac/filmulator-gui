...
namespace internal {
  ...
  template<typename Scalar> struct scalar_constant_op
  {
    ...
    template<typename PacketType> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const PacketType packetOp() const
    {
  ...
  template<typename Scalar, typename Packet> struct linspaced_op_impl<Scalar, Packet, /*IsInteger*/ false>
  {
    linspaced_op_impl(const Scalar &low, const Scalar &high, Index num_steps)
      : m_low(low), m_high(high), m_size1(num_steps == 1 ? 1 : num_steps - 1),
        m_step(num_steps == 1 ? Scalar() : (high - low) / Scalar(num_steps - 1)),
    ...
    template<typename IndexType> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const Packet packetOp(IndexType i) const
    {
      // Principle:
      ...
      if (m_flip) {
        ...
        if (i == 0) res = pinsertfirst(res, m_low);
        return res;
      } else {
        Packet pi = plset<Packet>(Scalar(i));
        Packet res = padd(pset1<Packet>(m_low), pmul(pset1<Packet>(m_step), pi));
        if (i == m_size1 - unpacket_traits<Packet>::size + 1) res = pinsertlast(res, m_high);
        return res;
      }
    }
  ...
  template<typename Scalar, typename Packet> struct linspaced_op_impl<Scalar, Packet, /*IsInteger*/ true>
  {
    linspaced_op_impl(const Scalar &low, const Scalar &high, Index num_steps)
