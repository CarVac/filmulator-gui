    ...
    RowMajor>
  {
      ...
      GemmParallelInfo<Index> *info = 0)
    {
      ...
      general_matrix_matrix_product<Index,
        ...
        ConjugateLhs,
        ColMajor>::run(cols, rows, depth, rhs, rhsStride, lhs, lhsStride, res, resStride, alpha, blocking, info);
    }
  };
      ...
      if (info) {
        ...
        for (Index k = 0; k < depth; k += kc) {
          ...
          pack_lhs(blockA + info[tid].lhs_start * actual_kc,
            lhs.getSubMapper(info[tid].lhs_start, k),
            actual_kc,
            info[tid].lhs_length);
          ...
          for (int shift = 0; shift < threads; ++shift) {
            ...
            gebp(res.getSubMapper(info[i].lhs_start, 0),
              blockA + info[i].lhs_start * actual_kc,
              blockB,
              info[i].lhs_length,
              actual_kc,
              nc,
...
#pragma omp atomic
  ...
  template<int StorageOrder,
    ...
    int MaxDepth,
    int KcFactor>
  class gemm_blocking_space<StorageOrder,
    _LhsScalar,
    _RhsScalar,
    MaxRows,
    MaxCols,
    ...
    : public level3_blocking<typename conditional<StorageOrder == RowMajor, _RhsScalar, _LhsScalar>::type,
  ...
  public:
    gemm_blocking_space(Index /*rows*/,
      Index /*cols*/,
      Index /*depth*/,
      Index /*num_threads*/,
...
namespace internal {
    ...
    : generic_product_impl_base<Lhs, Rhs, generic_product_impl<Lhs, Rhs, DenseShape, DenseShape, GemmProduct>>
  {
    ...
    template<typename Dst> static void evalTo(Dst &dst, const Lhs &lhs, const Rhs &rhs)
    {
    ...
    template<typename Dst> static void addTo(Dst &dst, const Lhs &lhs, const Rhs &rhs)
    {
      if ((rhs.rows() + dst.rows() + dst.cols()) < 20 && rhs.rows() > 0)
    ...
    template<typename Dst> static void subTo(Dst &dst, const Lhs &lhs, const Rhs &rhs)
    {
      if ((rhs.rows() + dst.rows() + dst.cols()) < 20 && rhs.rows() > 0)
    ...
    static void scaleAndAddTo(Dest &dst, const Lhs &a_lhs, const Rhs &a_rhs, const Scalar &alpha)
    {
      ...
      typedef internal::gemm_functor<Scalar,
        Index,
