...
namespace internal {
    ...
    Version>
  {
      ...
      level3_blocking<RhsScalar, LhsScalar> &blocking)
    {
      general_matrix_matrix_triangular_product<Index,
        ...
        ColMajor,
        UpLo == Lower ? Upper
                      : Lower>::run(size, depth, rhs, rhsStride, lhs, lhsStride, res, resStride, alpha, blocking);
    }
  };
    ...
    Version>
  {
      ...
      level3_blocking<LhsScalar, RhsScalar> &blocking)
    {
      ...
      if (mc > Traits::nr) mc = (mc / Traits::nr) * Traits::nr;
      ...
      for (Index k2 = 0; k2 < depth; k2 += kc) {
        ...
        for (Index i2 = 0; i2 < size; i2 += mc) {
          ...
          if (UpLo == Lower)
            gebp(
              res.getSubMapper(i2, 0), blockA, blockB, actual_mc, actual_kc, (std::min)(size, i2), alpha, -1, -1, 0, 0);
  ...
  struct tribb_kernel
  {
      ...
      const ResScalar &alpha)
    {
      ...
      for (Index j = 0; j < size; j += BlockSize) {
        ...
        if (UpLo == Upper)
          gebp_kernel(res.getSubMapper(0, j), blockA, actual_b, j, depth, actualBlockSize, alpha, -1, -1, 0, 0);
        ...
        // selfadjoint micro block
        {
          ...
          gebp_kernel(ResMapper(buffer.data(), BlockSize),
            blockA + depth * i,
            actual_b,
            actualBlockSize,
            depth,
        ...
        if (UpLo == Lower) {
          ...
          gebp_kernel(res.getSubMapper(i, j),
            blockA + depth * i,
            actual_b,
            size - i,
            depth,
...
struct general_product_to_triangular_selector<MatrixType, ProductType, UpLo, true>
{
  static void run(MatrixType &mat, const ProductType &prod, const typename MatrixType::Scalar &alpha, bool beta)
  {
    ...
    internal::gemv_static_vector_if<Scalar, Lhs::SizeAtCompileTime, Lhs::MaxSizeAtCompileTime, !UseLhsDirectly>
      static_lhs;
    ei_declare_aligned_stack_constructed_variable(Scalar,
      actualLhsPtr,
      actualLhs.size(),
...
struct general_product_to_triangular_selector<MatrixType, ProductType, UpLo, false>
{
  static void run(MatrixType &mat, const ProductType &prod, const typename MatrixType::Scalar &alpha, bool beta)
  {
    ...
    internal::general_matrix_matrix_triangular_product<Index,
      ...
      mat.data() + (SkipDiag ? (bool(IsRowMajor) != ((UpLo & Lower) == Lower) ? 1 : mat.outerStride()) : 0),
      mat.outerStride(),
      actualAlpha,
      blocking);
  }
