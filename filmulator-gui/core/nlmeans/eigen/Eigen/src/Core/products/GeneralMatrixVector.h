...
namespace internal {
   ...
   *       perform simple aligned loads for this column and aligned loads plus re-alignment for the other. (->
   * FirstAligned case)
   *       // this re-alignment is done by the palign function implemented for SSE in
   ...
   *
   * The last case (PacketSize>4) could probably be improved by generalizing the FirstAligned case, but since we do not
   * support AVX yet... One might also wonder why in the EvenAligned case we perform unaligned loads instead of using
   * the aligned-loads plus re-alignment strategy as in the FirstAligned case. The reason is that we observed that
   * unaligned loads on a 8 byte boundary are not too slow compared to unaligned loads on a 4 byte boundary.
   *
   */
    ...
    Version>
  {
    ...
    EIGEN_DONT_INLINE static void run(Index rows,
      Index cols,
      const LhsMapper &lhs,
      const RhsMapper &rhs,
      ResScalar *res,
  ...
  template<typename Index,
    ...
    bool ConjugateRhs,
    int Version>
  EIGEN_DONT_INLINE void general_matrix_vector_product<Index,
    LhsScalar,
    LhsMapper,
    ColMajor,
    ConjugateLhs,
...
#define _EIGEN_ACCUMULATE_PACKETS(Alignment0, Alignment13, Alignment2)         \
  pstore(&res[j],                                                              \
    padd(pload<ResPacket>(&res[j]),                                            \
      padd(padd(pcj.pmul(lhs0.template load<LhsPacket, Alignment0>(j), ptmp0), \
    ...
    if (ConjugateRhs) alpha = numext::conj(alpha);
    ...
    const Index alignmentStep =
      LhsPacketSize > 1 ? (LhsPacketSize - lhsStride % LhsPacketSize) & LhsPacketAlignedMask : 0;
    ...
    for (Index i = skipColumns; i < columnBound; i += columnsAtOnce) {
      RhsPacket ptmp0 = pset1<RhsPacket>(alpha * rhs(i, 0)), ptmp1 = pset1<RhsPacket>(alpha * rhs(i + offset1, 0)),
                ptmp2 = pset1<RhsPacket>(alpha * rhs(i + 2, 0)), ptmp3 = pset1<RhsPacket>(alpha * rhs(i + offset3, 0));
          ...
          case FirstAligned: {
            ...
            if (peels > 1) {
              ...
              for (; j < peeledSize; j += peels * ResPacketSize) {
                A11 = lhs1.template load<LhsPacket, Aligned>(j - 1 + LhsPacketSize);
                palign<1>(A01, A11);
                A12 = lhs2.template load<LhsPacket, Aligned>(j - 2 + LhsPacketSize);
                palign<2>(A02, A12);
                A13 = lhs3.template load<LhsPacket, Aligned>(j - 3 + LhsPacketSize);
                palign<3>(A03, A13);
                ...
                A01 = lhs1.template load<LhsPacket, Aligned>(j - 1 + 2 * LhsPacketSize);
                palign<1>(A11, A01);
                T0 = pcj.pmadd(A02, ptmp2, T0);
                A02 = lhs2.template load<LhsPacket, Aligned>(j - 2 + 2 * LhsPacketSize);
                palign<2>(A12, A02);
                T0 = pcj.pmadd(A03, ptmp3, T0);
                pstore(&res[j], T0);
                A03 = lhs3.template load<LhsPacket, Aligned>(j - 3 + 2 * LhsPacketSize);
                palign<3>(A13, A03);
            ...
            for (; j < alignedSize; j += ResPacketSize) _EIGEN_ACCUMULATE_PACKETS(Aligned, Unaligned, Unaligned);
            break;
          }
          default:
            for (Index j = alignedStart; j < alignedSize; j += ResPacketSize)
    ...
    do {
      for (Index k = start; k < end; ++k) {
        ...
        if (Vectorizable) {
          ...
          for (Index j = 0; j < alignedStart; ++j) res[j] += cj.pmul(lhs0(j), pfirst(ptmp0));
        ...
        for (Index i = alignedSize; i < size; ++i) res[i] += cj.pmul(lhs0(i), pfirst(ptmp0));
      }
      if (skipColumns) {
        start = 0;
        end = skipColumns;
...
#define _EIGEN_ACCUMULATE_PACKETS(Alignment0, Alignment13, Alignment2)            \
  {                                                                               \
    ...
    Index skipRows = 0;
    // if the data cannot be aligned (TODO add some compile time tests when possible, e.g. for floats)
    if ((sizeof(LhsScalar) != sizeof(RhsScalar)) || (lhsAlignmentOffset < 0) || (lhsAlignmentOffset == depth)
      ...
      while (
        skipRows < LhsPacketSize && alignedStart != ((lhsAlignmentOffset + alignmentStep * skipRows) % LhsPacketSize))
        ++skipRows;
      ...
      if (Vectorizable) {
        ...
        if (alignedSize > alignedStart) {
          ...
          case FirstAligned: {
            ...
            for (; j < alignedSize; j += RhsPacketSize) _EIGEN_ACCUMULATE_PACKETS(Aligned, Unaligned, Unaligned);
            break;
          }
          default:
            for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize)
    ...
    do {
      for (Index i = start; i < end; ++i) {
        ...
        // process first unaligned result's coeffs
        // FIXME this loop get vectorized by the compiler !
        for (Index j = 0; j < alignedStart; ++j) tmp0 += cj.pmul(lhs0(j), rhs(j, 0));

        if (alignedSize > alignedStart) {
          // process aligned rhs coeffs
          if (lhs0.template aligned<LhsPacket>(alignedStart))
            for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize)
              ptmp0 = pcj.pmadd(lhs0.template load<LhsPacket, Aligned>(j),
                rhs.getVectorMapper(j, 0).template load<RhsPacket, Aligned>(0),
                ptmp0);
          else
            for (Index j = alignedStart; j < alignedSize; j += RhsPacketSize)
              ptmp0 = pcj.pmadd(lhs0.template load<LhsPacket, Unaligned>(j),
                rhs.getVectorMapper(j, 0).template load<RhsPacket, Aligned>(0),
                ptmp0);
          tmp0 += predux(ptmp0);
        }
        ...
        // FIXME this loop get vectorized by the compiler !
        for (Index j = alignedSize; j < depth; ++j) tmp0 += cj.pmul(lhs0(j), rhs(j, 0));
        res[i * resIncr] += alpha * tmp0;
      }
      if (skipRows) {
        start = 0;
