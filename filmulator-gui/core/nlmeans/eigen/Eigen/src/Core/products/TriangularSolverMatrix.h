...
namespace internal {
  ...
  struct triangular_solve_matrix<Scalar, Index, Side, Mode, Conjugate, TriStorageOrder, RowMajor>
  {
      ...
      level3_blocking<Scalar, Scalar> &blocking)
    {
      triangular_solve_matrix<Scalar,
        ...
        ColMajor>::run(size, cols, tri, triStride, _other, otherStride, blocking);
    }
  };
  ...
  {
    static EIGEN_DONT_INLINE void run(Index size,
      Index otherSize,
      const Scalar *_tri,
      Index triStride,
      Scalar *_other,
  ...
  EIGEN_DONT_INLINE void
    triangular_solve_matrix<Scalar, Index, OnTheLeft, Mode, Conjugate, TriStorageOrder, ColMajor>::run(Index size,
      Index otherSize,
      const Scalar *_tri,
      Index triStride,
      Scalar *_other,
      ...
      level3_blocking<Scalar, Scalar> &blocking)
  {
    ...
    enum { SmallPanelWidth = EIGEN_PLAIN_ENUM_MAX(Traits::mr, Traits::nr), IsLower = (Mode & Lower) == Lower };
      ...
      for (Index j2 = 0; j2 < cols; j2 += subcols) {
        ...
        for (Index k1 = 0; k1 < actual_kc; k1 += SmallPanelWidth) {
          ...
          for (Index k = 0; k < actualPanelWidth; ++k) {
            ...
            Index s = TriStorageOrder == RowMajor ? (IsLower ? k2 + k1 : i + 1) : IsLower ? i + 1 : i - rs;
  ...
  template<typename Scalar, typename Index, int Mode, bool Conjugate, int TriStorageOrder>
  EIGEN_DONT_INLINE void
    triangular_solve_matrix<Scalar, Index, OnTheRight, Mode, Conjugate, TriStorageOrder, ColMajor>::run(Index size,
      Index otherSize,
      const Scalar *_tri,
      Index triStride,
      Scalar *_other,
      ...
      for (Index i2 = 0; i2 < rows; i2 += mc) {
        ...
        // triangular solver kernel
        {
          ...
          for (Index j2 = IsLower ? (actual_kc
                                      - ((actual_kc % SmallPanelWidth) ? Index(actual_kc % SmallPanelWidth)
            ...
            for (Index k = 0; k < actualPanelWidth; ++k) {
              ...
              for (Index k3 = 0; k3 < k; ++k3) {
                ...
                for (Index i = 0; i < actual_mc; ++i) r[i] -= a[i] * b;
              }
              if ((Mode & UnitDiag) == 0) {
                Scalar inv_rjj = RealScalar(1) / conj(rhs(j, j));
                for (Index i = 0; i < actual_mc; ++i) r[i] *= inv_rjj;
              }
            }

            // pack the just computed part of lhs to A
            pack_lhs_panel(blockA,
              LhsMapper(_other + absolute_j2 * otherStride + i2, otherStride),
              actualPanelWidth,
              actual_mc,
              actual_kc,
              j2);
