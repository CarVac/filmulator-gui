...
/// \internal EIGEN_GNUC_STRICT set to 1 if the compiler is really GCC and not a compatible compiler (e.g., ICC, clang,
/// mingw, etc.)
#if EIGEN_COMP_GNUC                                                                                                 \
  && !(EIGEN_COMP_CLANG || EIGEN_COMP_ICC || EIGEN_COMP_MINGW || EIGEN_COMP_PGI || EIGEN_COMP_IBM || EIGEN_COMP_ARM \
...
// Do we support r-value references?
  ...
  && (__has_feature(cxx_rvalue_references) || (defined(__cplusplus) && __cplusplus >= 201103L) \
...
#if EIGEN_MAX_CPP_VER >= 11                                                                                           \
  && ((defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)) || (defined(__GNUC__) && defined(_GLIBCXX_USE_C99)) \
...
#define EIGEN_HAS_CONSTEXPR 1
  ...
  && (__has_feature(cxx_relaxed_constexpr) || (defined(__cplusplus) && __cplusplus >= 201402L) \
...
#if EIGEN_MAX_CPP_VER >= 11                                                                                   \
  && (__has_feature(cxx_noexcept) || (__cplusplus > 201103L)                                                  \
      || ((__cplusplus >= 201103L) && (EIGEN_COMP_GNUC_STRICT || EIGEN_COMP_CLANG || EIGEN_COMP_ICC >= 1400)) \
      || EIGEN_COMP_MSVC >= 1900)
...
namespace internal {
  ...
  {
    std::cerr << "assertion failed: " << condition << " in function " << function << " at " << file << ":" << line
              << std::endl;
    abort();
  }
}// namespace internal
...
#if !EIGEN_GCC_AND_ARCH_DOESNT_WANT_STACK_ALIGNMENT && !EIGEN_GCC3_OR_OLDER && !EIGEN_COMP_SUNCC && !EIGEN_OS_QNX
...
// It takes into account both the user choice to explicitly enable/disable alignment (by settting
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE(Derived)                                                                        \
  ...
  typedef typename Base::CoeffReturnType                                                                               \
    CoeffReturnType; /*!< \brief The return type for coefficient access. \details Depending on whether the object      \
                        allows direct coefficient access (e.g. for a MatrixXd), this type is either 'const Scalar&' or \
                        simply 'Scalar' for objects that do not allow direct coefficient access. */                    \
...
// EIGEN_SIZE_MIN_PREFER_FIXED is a variant of EIGEN_SIZE_MIN_PREFER_DYNAMIC comparing MaxSizes. The difference is that
// finite values now have priority over Dynamic, so that min(3, Dynamic) gives 3. Indeed, whatever the actual value is
...
#define EIGEN_SIZE_MAX(a, b) (((int)a == Dynamic || (int)b == Dynamic) ? Dynamic : ((int)a >= (int)b) ? (int)a : (int)b)
...
#define EIGEN_MAKE_CWISE_BINARY_OP(METHOD, OPNAME)                                                                   \
  template<typename OtherDerived>                                                                                    \
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const EIGEN_CWISE_BINARY_RETURN_TYPE(Derived, OtherDerived, OPNAME)(METHOD)( \
...
#define EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, TYPEA, TYPEB)                \
  (Eigen::internal::has_ReturnType<Eigen::ScalarBinaryOpTraits<TYPEA,      \
      TYPEB,                                                               \
...
#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHERIGHT(METHOD, OPNAME)                                                       \
  ...
  EIGEN_DEVICE_FUNC inline EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE(                                             \
    const EIGEN_EXPR_BINARYOP_SCALAR_RETURN_TYPE(Derived,                                                            \
...
#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHELEFT(METHOD, OPNAME)                                                        \
  template<typename T>                                                                                               \
  EIGEN_DEVICE_FUNC inline friend EIGEN_MSVC10_WORKAROUND_BINARYOP_RETURN_TYPE(                                      \
    const EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE(                                                                    \
      typename internal::promote_scalar_arg<Scalar EIGEN_COMMA T EIGEN_COMMA EIGEN_SCALAR_BINARY_SUPPORTED(          \
        OPNAME, T, Scalar)>::type,                                                                                   \
      Derived,                                                                                                       \
      OPNAME))(METHOD)(const T &scalar, const StorageBaseType &matrix)                                               \
  {                                                                                                                  \
