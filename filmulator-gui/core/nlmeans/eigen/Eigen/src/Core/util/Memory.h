...
namespace internal {
  ...
  *****************************************************************************/
  ...
  inline void *handmade_aligned_malloc(std::size_t size)
  {
    ...
    void *aligned =
      reinterpret_cast<void *>((reinterpret_cast<std::size_t>(original) & ~(std::size_t(EIGEN_DEFAULT_ALIGN_BYTES - 1)))
  ...
  inline void *handmade_aligned_realloc(void *ptr, std::size_t size, std::size_t = 0)
  {
    ...
    if (aligned != previous_aligned) std::memmove(aligned, previous_aligned, size);
  ...
  *****************************************************************************/
  ...
  /** \internal Allocates \a size bytes. The returned pointer is guaranteed to have 16 or 32 bytes alignment depending
   * on the requirements. On allocation error, the returned pointer is null, and std::bad_alloc is thrown.
   */
  ...
  *****************************************************************************/
  ...
  template<> EIGEN_DEVICE_FUNC inline void conditional_aligned_free<false>(void *ptr) { std::free(ptr); }
  ...
  *****************************************************************************/
  ...
  /** \internal Returns the index of the first element of the array that is well aligned with respect to the requested
   * \a Alignment.
   *
   ...
   *
   * \note Otherwise, if the Alignment is larger that the scalar size, we rely on the assumptions that sizeof(Scalar) is
   * a power of 2. On the other hand, we do not assume that the array address is a multiple of sizeof(Scalar), as that
   * fails for example with Scalar=double on certain 32-bit platforms, see bug #79.
   *
...
#define EIGEN_ALLOCA _alloca
  ...
  public:
    ...
    {
      if (NumTraits<T>::RequireInitialization && m_ptr) Eigen::internal::construct_elements_of_array(m_ptr, size);
    }
...
#define EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(Scalar, Size) \
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF(                                            \
    bool(((Size) != Eigen::Dynamic) && ((sizeof(Scalar) * (Size)) % EIGEN_MAX_ALIGN_BYTES == 0)))
...
public:
  ...
  pointer allocate(size_type num, const void * /*hint*/ = 0)
  {
    ...
    // It triggered eigen/Eigen/src/Core/util/Memory.h:189:12: warning: argument 1 value '18446744073709551612' exceeds
