 ...
 */
  ...
  && (__has_feature(cxx_static_assert) || (defined(__cplusplus) && __cplusplus >= 201103L) \
...
#define EIGEN_STATIC_ASSERT_FIXED_SIZE(TYPE) \
  EIGEN_STATIC_ASSERT(                       \
    TYPE::SizeAtCompileTime != Eigen::Dynamic, YOU_CALLED_A_FIXED_SIZE_METHOD_ON_A_DYNAMIC_SIZE_MATRIX_OR_VECTOR)

// static assertion failing if the type \a TYPE is not dynamic-size
#define EIGEN_STATIC_ASSERT_DYNAMIC_SIZE(TYPE) \
  EIGEN_STATIC_ASSERT(                         \
    TYPE::SizeAtCompileTime == Eigen::Dynamic, YOU_CALLED_A_DYNAMIC_SIZE_METHOD_ON_A_FIXED_SIZE_MATRIX_OR_VECTOR)

// static assertion failing if the type \a TYPE is not a vector type of the given size
#define EIGEN_STATIC_ASSERT_VECTOR_SPECIFIC_SIZE(TYPE, SIZE) \
  EIGEN_STATIC_ASSERT(                                       \
    TYPE::IsVectorAtCompileTime &&TYPE::SizeAtCompileTime == SIZE, THIS_METHOD_IS_ONLY_FOR_VECTORS_OF_A_SPECIFIC_SIZE)
...
#define EIGEN_INTERNAL_CHECK_COST_VALUE(C) \
  EIGEN_STATIC_ASSERT(                     \
    (C) >= 0 && (C) <= HugeCost * HugeCost, EIGEN_INTERNAL_ERROR_PLEASE_FILE_A_BUG_REPORT__INVALID_COST_VALUE);
