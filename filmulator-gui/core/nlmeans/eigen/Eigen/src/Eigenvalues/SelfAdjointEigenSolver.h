...
namespace internal {
  ...
  ComputationInfo computeFromTridiagonal_impl(DiagType &diag,
    SubDiagType &subdiag,
...
public:
   ...
   */
  EIGEN_DEVICE_FUNC
  SelfAdjointEigenSolver() : m_eivec(), m_eivalues(), m_subdiag(), m_isInitialized(false) {}
  ...
  explicit SelfAdjointEigenSolver(Index size)
    : m_eivec(size, size), m_eivalues(size), m_subdiag(size > 1 ? size - 1 : 1), m_isInitialized(false)
  {}
  ...
  EIGEN_DEVICE_FUNC explicit SelfAdjointEigenSolver(const EigenBase<InputType> &matrix,
    ...
    : m_eivec(matrix.rows(), matrix.cols()), m_eivalues(matrix.cols()),
      m_subdiag(matrix.rows() > 1 ? matrix.rows() - 1 : 1), m_isInitialized(false)
  {
  ...
  int options)
{
  ...
  m_eivalues = diag;
  m_subdiag = subdiag;
  if (computeEigenvectors) { m_eivec.setIdentity(diag.size(), diag.size()); }
    ...
    MatrixType &eivec)
  {
    ...
    while (end > 0) {
      for (Index i = start; i < end; ++i)
        if (internal::isMuchSmallerThan(abs(subdiag[i]), (abs(diag[i]) + abs(diag[i + 1])), precision)
