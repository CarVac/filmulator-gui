...
public:
  ...
  EIGEN_DEVICE_FUNC inline const TransformTimeDiagonalReturnType operator*(const DiagonalBase<DiagonalDerived> &b) const
  {
    ...
    const Transform &b)
  {
    ...
    if (Mode != int(AffineCompact)) res.matrix().row(Dim) = b.matrix().row(Dim);
    return res;
  }
...
public:
  ...
  /** Concatenates two different transformations */
  template<int OtherMode, int OtherOptions>
  EIGEN_DEVICE_FUNC inline typename internal::transform_transform_product_impl<Transform,
  ...
  EIGEN_DEVICE_FUNC bool isApprox(const Transform &other,
 ...
 */
template<typename Scalar, int Dim, int Mode, int Options>
EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> &Transform<Scalar, Dim, Mode, Options>::shear(const Scalar &sx,
  const Scalar &sy)
{
...
template<typename Scalar, int Dim, int Mode, int Options>
template<typename ScalingMatrixType, typename RotationMatrixType>
EIGEN_DEVICE_FUNC void Transform<Scalar, Dim, Mode, Options>::computeScalingRotation(ScalingMatrixType *scaling,
