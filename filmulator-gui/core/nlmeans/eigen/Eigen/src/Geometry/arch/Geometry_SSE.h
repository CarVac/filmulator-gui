...
namespace internal {
  ...
  template<class Derived, class OtherDerived> struct quat_product<Architecture::SSE, Derived, OtherDerived, float>
  {
    enum {
    ...
    static inline Quaternion<float> run(const QuaternionBase<Derived> &_a, const QuaternionBase<OtherDerived> &_b)
    {
      ...
      pstoret<float, Packet4f, ResAlignment>(&res.x(),
        _mm_add_ps(_mm_sub_ps(_mm_mul_ps(a, vec4f_swizzle1(b, 3, 3, 3, 3)),
                     _mm_mul_ps(vec4f_swizzle1(a, 2, 0, 1, 0), vec4f_swizzle1(b, 1, 2, 0, 0))),
  ...
  template<class Derived> struct quat_conj<Architecture::SSE, Derived, float>
  {
    ...
    static inline Quaternion<float> run(const QuaternionBase<Derived> &q)
    {
      ...
      pstoret<float, Packet4f, ResAlignment>(
        &res.x(), _mm_xor_ps(mask, q.coeffs().template packet<traits<Derived>::Alignment>(0)));
      return res;
    }
  };
  ...
  template<class Derived, class OtherDerived> struct quat_product<Architecture::SSE, Derived, OtherDerived, double>
  {
  ...
  template<class Derived> struct quat_conj<Architecture::SSE, Derived, double>
  {
    ...
    static inline Quaternion<double> run(const QuaternionBase<Derived> &q)
    {
      ...
      pstoret<double, Packet2d, ResAlignment>(
        &res.x(), _mm_xor_pd(mask0, q.coeffs().template packet<traits<Derived>::Alignment>(0)));
      pstoret<double, Packet2d, ResAlignment>(
        &res.z(), _mm_xor_pd(mask2, q.coeffs().template packet<traits<Derived>::Alignment>(2)));
      return res;
    }
  };
