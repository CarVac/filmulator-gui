...
{
  typedef
    typename internal::hseq_side_dependent_impl<VectorsType, CoeffsType, Side>::EssentialVectorType EssentialVectorType;
...
public:
  ...
  HouseholderSequence(const VectorsType &v, const CoeffsType &h)
    : m_vectors(v), m_coeffs(h), m_trans(false), m_length(v.diagonalSize()), m_shift(0)
  {}

  /** \brief Copy constructor. */
  HouseholderSequence(const HouseholderSequence &other)
    : m_vectors(other.m_vectors), m_coeffs(other.m_coeffs), m_trans(other.m_trans), m_length(other.m_length),
  ...
  HouseholderSequence transpose() const { return HouseholderSequence(*this).setTrans(!m_trans); }
  ...
  ConjugateReturnType adjoint() const { return conjugate().setTrans(!m_trans); }
  ...
  /** \internal */
  template<typename Dest, typename Workspace> void evalTo(Dest &dst, Workspace &workspace) const
  {
  ...
  /** \internal */
  template<typename Dest, typename Workspace> inline void applyThisOnTheLeft(Dest &dst, Workspace &workspace) const
  {
    ...
    if (m_length >= BlockSize && dst.cols() > 1) {
      for (Index i = 0; i < m_length; i += BlockSize) {
        ...
        typename internal::conditional<Side == OnTheRight, Transpose<SubVectorsType>, SubVectorsType &>::type sub_vecs(
          sub_vecs1);
        Block<Dest, Dynamic, Dynamic> sub_dst(
          dst, dst.rows() - rows() + m_shift + k, 0, rows() - m_shift - k, dst.cols());
