...
namespace internal {
  ...
  EIGEN_DONT_INLINE void conjugate_gradient(const MatrixType &mat,
    const Rhs &rhs,
    Dest &x,
...
public:
  ...
  template<typename Rhs, typename Dest> void _solve_with_guess_impl(const Rhs &b, Dest &x) const
  {
    ...
    for (Index j = 0; j < b.cols(); ++j) {
      ...
      internal::conjugate_gradient(
        SelfAdjointWrapper(row_mat), b.col(j), xj, Base::m_preconditioner, m_iterations, m_error);
    }
  ...
  template<typename Rhs, typename Dest> void _solve_impl(const MatrixBase<Rhs> &b, Dest &x) const
  {
    x.setZero();
