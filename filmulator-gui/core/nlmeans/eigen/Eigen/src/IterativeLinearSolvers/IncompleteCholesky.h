 ...
 * \f$ where
 * \f$ \sigma \f$ is the initial shift value as returned and set by setInitialShift() method. The default value is \f$
 * \sigma = 10^{-3} \f$. If the factorization fails, then the shift in doubled until it succeed or a maximum of ten
...
template<typename Scalar, int _UpLo, typename OrderingType>
inline void IncompleteCholesky<Scalar, _UpLo, OrderingType>::updateList(Ref<const VectorIx> colPtr,
  Ref<VectorIx> rowIdx,
  Ref<VectorSx> vals,
  const Index &col,
  const Index &jk,
