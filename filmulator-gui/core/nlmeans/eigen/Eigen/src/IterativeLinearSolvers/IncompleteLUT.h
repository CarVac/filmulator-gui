...
namespace internal {
   ...
   **/
  template<typename VectorV, typename VectorI> Index QuickSplit(VectorV &row, VectorI &ind, Index ncut)
  {
...
public:
  IncompleteLUT()
    : m_droptol(NumTraits<Scalar>::dummy_precision()), m_fillfactor(10), m_analysisIsOk(false),
   ...
   *
   **/
  template<typename MatrixType> IncompleteLUT &compute(const MatrixType &amat)
  {
    analyzePattern(amat);
    factorize(amat);
    return *this;
