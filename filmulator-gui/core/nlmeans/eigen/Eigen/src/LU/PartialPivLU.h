...
PartialPivLU<MatrixType>::PartialPivLU(const EigenBase<InputType> &matrix)
  : m_lu(matrix.rows(), matrix.cols()), m_p(matrix.rows()), m_rowsTranspositions(matrix.rows()), m_l1_norm(0),
...
PartialPivLU<MatrixType>::PartialPivLU(EigenBase<InputType> &matrix)
  : m_lu(matrix.derived()), m_p(matrix.rows()), m_rowsTranspositions(matrix.rows()), m_l1_norm(0), m_det_p(0),
...
namespace internal {
  ...
  template<typename Scalar, int StorageOrder, typename PivIndex> struct partial_lu_impl
  {
    ...
    static Index unblocked_lu(MatrixType &lu, PivIndex *row_transpositions, PivIndex &nb_transpositions)
    {
      ...
      for (Index k = 0; k < size; ++k) {
        ...
        if (k < rows - 1) lu.bottomRightCorner(rrows, rcols).noalias() -= lu.col(k).tail(rrows) * lu.row(k).tail(rcols);
      }
      ...
      Index maxBlockSize = 256)
    {
      ...
      if (size <= 16) { return unblocked_lu(lu, row_transpositions, nb_transpositions); }
    ...
    typename TranspositionType::StorageIndex &nb_transpositions)
  {
    ...
    partial_lu_impl<typename MatrixType::Scalar,
      ...
      typename TranspositionType::StorageIndex>::blocked_lu(lu.rows(),
      lu.cols(),
      &lu.coeffRef(0, 0),
      lu.outerStride(),
