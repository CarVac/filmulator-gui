...
template<typename IndexType> struct colamd_col
{
...
template<typename IndexType> struct Colamd_Row
{
...
template<typename IndexType>
static IndexType find_ordering(IndexType n_row,
  IndexType n_col,
  IndexType Alen,
  Colamd_Row<IndexType> Row[],
  colamd_col<IndexType> Col[],
...
template<typename IndexType> static void order_children(IndexType n_col, colamd_col<IndexType> Col[], IndexType p[]);

template<typename IndexType>
static void detect_super_cols(colamd_col<IndexType> Col[],
  IndexType A[],
  IndexType head[],
  IndexType row_start,
...
template<typename IndexType> static inline IndexType clear_mark(IndexType n_row, Colamd_Row<IndexType> Row[]);
 ...
 */
template<typename IndexType> inline IndexType colamd_recommended(IndexType nnz, IndexType n_row, IndexType n_col)
{
...
static inline void colamd_set_defaults(double knobs[COLAMD_KNOBS])
{
  ...
  if (!knobs) { return; /* no knobs to initialize */ }
  for (i = 0; i < COLAMD_KNOBS; i++) { knobs[i] = 0; }
 ...
 */
template<typename IndexType>
static bool colamd(IndexType n_row,
  IndexType n_col,
  IndexType Alen,
  IndexType *A,
  IndexType *p,
  ...
  IndexType stats[COLAMD_STATS])
{
  ...
  for (i = 0; i < COLAMD_STATS; i++) { stats[i] = 0; }
  ...
  Eigen::internal::init_scoring(n_row, n_col, Row, Col, A, p, knobs, &n_row2, &n_col2, &max_deg);

  /* === Order the supercolumns =========================================== */

  ngarbage = Eigen::internal::find_ordering(n_row, n_col, Alen, Row, Col, A, p, n_col2, max_deg, 2 * nnz);
  ...
  )
{
  ...
  if (stats[COLAMD_STATUS] == COLAMD_OK_BUT_JUMBLED) {
    ...
    for (col = 0; col < n_col; col++) {
      cp = &A[p[col]];
      cp_end = &A[p[col + 1]];
      while (cp < cp_end) { A[(Row[*cp++].shared1.p)++] = col; }
    }
  }
...
)
{
  ...
  for (c = n_col - 1; c >= 0; c--) {
    ...
    if (deg > dense_col_count) {
      ...
      while (cp < cp_end) { Row[*cp++].shared1.degree--; }
      KILL_PRINCIPAL_COL(c);
    }
  }
  ...
  for (c = 0; c <= n_col; c++) { head[c] = COLAMD_EMPTY; }
  min_score = n_col;
  ...
  for (c = n_col - 1; c >= 0; c--) {
    ...
    if (COL_IS_ALIVE(c)) {
      ...
      if (next_col != COLAMD_EMPTY) { Col[next_col].shared3.prev = c; }
      head[score] = c;
  ...
  )
{
  ...
  for (k = 0; k < n_col2; /* 'k' is incremented below */) {
    ...
    while (min_score < n_col && head[min_score] == COLAMD_EMPTY) { min_score++; }
    pivot_col = head[min_score];
    COLAMD_ASSERT(pivot_col >= 0 && pivot_col <= n_col);
    next_col = Col[pivot_col].shared4.degree_next;
    head[min_score] = next_col;
    if (next_col != COLAMD_EMPTY) { Col[next_col].shared3.prev = COLAMD_EMPTY; }
    ...
    while (rp < rp_end) {
      ...
      if (next_col != COLAMD_EMPTY) { Col[next_col].shared3.prev = prev_col; }
      ...
      while (cp < cp_end) {
        ...
        /* skip if dead */
        if (ROW_IS_MARKED_DEAD(row_mark)) { continue; }
    ...
    while (rp < rp_end) {
      ...
      *new_rp++ = col;
      ...
      Col[col].shared3.prev = COLAMD_EMPTY;
      if (next_col != COLAMD_EMPTY) { Col[next_col].shared3.prev = col; }
      head[cur_score] = col;
...
)
{
  ...
  for (c = 0; c < n_col; c++) { p[Col[c].shared2.order] = c; }
}
...
)
{
  ...
  while (rp < rp_end) {
    ...
    for (super_c = first_col; super_c != COLAMD_EMPTY; super_c = Col[super_c].shared4.hash_next) {
      ...
      for (c = Col[super_c].shared4.hash_next; c != COLAMD_EMPTY; c = Col[c].shared4.hash_next) {
  ...
  )
{
  ...
  for (r = 0; r < n_row; r++) {
    if (ROW_IS_ALIVE(r)) { Row[r].shared2.mark = 0; }
  }
  return (1);
}
