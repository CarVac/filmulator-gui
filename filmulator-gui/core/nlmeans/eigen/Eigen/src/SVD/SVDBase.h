...
public:
  ...
  RealScalar threshold() const
  {
    ...
    return m_usePrescribedThreshold ? m_prescribedThreshold : diagSize * NumTraits<Scalar>::epsilon();
  }
...
protected:
   ...
   */
  SVDBase()
    : m_isInitialized(false), m_isAllocated(false), m_usePrescribedThreshold(false), m_computationOptions(0),
...
bool SVDBase<MatrixType>::allocate(Index rows, Index cols, unsigned int computationOptions)
{
  ...
  if (m_isAllocated && rows == m_rows && cols == m_cols && computationOptions == m_computationOptions) { return true; }
