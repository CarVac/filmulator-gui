...
public:
   ...
   * \sa compute()
   */
  template<typename Rhs> inline const Solve<SparseLU, Rhs> solve(const MatrixBase<Rhs> &B) const;
...
void SparseLU<MatrixType, OrderingType>::analyzePattern(const MatrixType &mat)
{
  ...
  if (m_perm_c.size()) {
    ...
    ei_declare_aligned_stack_constructed_variable(StorageIndex,
      outerIndexPtr,
      mat.cols() + 1,
  ...
  if (!m_symmetricmode) {
    ...
    if (m_perm_c.size()) { m_perm_c = post_perm * m_perm_c; }
...
template<typename MappedSupernodalType> struct SparseLUMatrixLReturnType : internal::no_assignment_operator
{
  typedef typename MappedSupernodalType::Scalar Scalar;
  explicit SparseLUMatrixLReturnType(const MappedSupernodalType &mapL) : m_mapL(mapL) {}
  Index rows() { return m_mapL.rows(); }
  Index cols() { return m_mapL.cols(); }
  template<typename Dest> void solveInPlace(MatrixBase<Dest> &X) const { m_mapL.solveInPlace(X); }
  const MappedSupernodalType &m_mapL;
};

template<typename MatrixLType, typename MatrixUType> struct SparseLUMatrixUReturnType : internal::no_assignment_operator
{
  ...
  template<typename Dest> void solveInPlace(MatrixBase<Dest> &X) const
  {
    ...
    for (Index k = m_mapL.nsuper(); k >= 0; k--) {
      ...
      if (nsupc == 1) {
        for (Index j = 0; j < nrhs; j++) { X(fsupc, j) /= m_mapL.valuePtr()[luptr]; }
      } else {
        Map<const Matrix<Scalar, Dynamic, Dynamic, ColMajor>, 0, OuterStride<>> A(
          &(m_mapL.valuePtr()[luptr]), nsupc, nsupc, OuterStride<>(lda));
        Map<Matrix<Scalar, Dynamic, Dest::ColsAtCompileTime, ColMajor>, 0, OuterStride<>> U(
          &(X(fsupc, 0)), nsupc, nrhs, OuterStride<>(n));
