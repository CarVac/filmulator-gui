...
// check whether we really need the std::deque specialization
  ...
  && !(defined(_GLIBCXX_DEQUE)   \
       && (!EIGEN_GNUC_AT_LEAST( \
         4, 1))) /* Note that before gcc-4.1 we already have: std::deque::resize(size_type,const T&). */
      ...
      Eigen::aligned_allocator_indirection<EIGEN_WORKAROUND_MSVC_STL_SUPPORT(T)>>
{
  ...
  iterator insert(const_iterator position, const value_type &x) { return deque_base::insert(position, x); }
