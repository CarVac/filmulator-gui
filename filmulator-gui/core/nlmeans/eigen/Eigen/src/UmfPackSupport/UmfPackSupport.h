...
inline void umfpack_defaults(double control[UMFPACK_CONTROL], double) { umfpack_di_defaults(control); }

inline void umfpack_defaults(double control[UMFPACK_CONTROL], std::complex<double>) { umfpack_zi_defaults(control); }
  ...
  void *Numeric)
{
  ...
  return umfpack_zi_get_numeric(
    Lp, Lj, Lx ? &lx0_real : 0, 0, Up, Ui, Ux ? &ux0_real : 0, 0, P, Q, Dx ? &dx0_real : 0, 0, do_recip, Rs, Numeric);
}
...
public:
   ...
   * \sa umfpackControl()
   */
  void umfpackReportControl() { umfpack_report_control(m_control.data(), Scalar()); }
