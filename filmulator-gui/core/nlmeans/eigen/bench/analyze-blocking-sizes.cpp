...
struct inputfile_t
{
  ...
  inputfile_t(const string &fname) : filename(fname), type(type_t::unknown)
  {
    ...
    while (getline(stream, line)) {
      ...
      if (type == type_t::unknown) { continue; }
      switch (type) {
      case type_t::all_pot_sizes: {
        ...
        if (only_cubic_sizes && !size_triple_t(product_size).is_cubic()) { continue; }
...
{
  if (preprocessed_inputfiles.empty()) { return; }
  ...
  const vector<size_t> &subset)
{
  ...
  if (!min_product_size.is_cubic() || !max_product_size.is_cubic()) { abort(); }
  if (only_cubic_sizes) {
...
void make_first_subset(size_t subset_size, vector<size_t> &out_subset, size_t set_size)
{
  ...
  for (size_t i = 0; i < subset_size; i++) { out_subset[i] = i; }
}
...
void next_subset(vector<size_t> &inout_subset, size_t set_size)
{
  ...
  for (size_t j = first_index_to_change + 1; j < inout_subset.size(); j++) { inout_subset[j] = ++p; }
}
...
bool is_number_of_subsets_feasible(size_t n, size_t p)
{
  ...
  for (size_t i = 0; i < p; i++) {
    numerator *= n - i;
    denominator *= i + 1;
    if (numerator > denominator * number_of_subsets_limit) { return false; }
  }
  return true;
}
...
size_t max_feasible_subset_size(size_t n)
{
  ...
  for (size_t p = 1; p <= n - 1; p++) {
    if (!is_number_of_subsets_feasible(n, p + 1)) { return max(p, minresult); }
  }
  return n - 1;
}
  ...
  vector<size_t> &out_subset)
{
  ...
  while (!inout_remainder.empty()) {
    ...
    for (size_t i = 0; i < candidate_indices.size(); i++) { candidate_indices[i] = i; }
    ...
    while (candidate_indices_subset_size >= 1) {
      ...
      while (true) {
        ...
        if (is_last_subset(candidate_indices_subset, candidate_indices.size())) { break; }
  ...
  vector<vector<size_t>> &out_partition)
{
  ...
  for (size_t i = 0; i < preprocessed_inputfiles.size(); i++) { remainder.push_back(i); }
...
struct partition_action_t : action_t
{
  ...
  virtual void run(const vector<string> &input_filenames) const override
  {
    ...
    while (true) {
      ...
      if (partition.size() == preprocessed_inputfiles.size() || actual_efficiency == 1.0f) { break; }
    ...
    for (auto it = partitions.begin(); it != partitions.end(); ++it) { print_partition(preprocessed_inputfiles, *it); }
  }
};
...
struct evaluate_defaults_action_t : action_t
{
  ...
  virtual void run(const vector<string> &input_filenames) const override
  {
    if (input_filenames.size() != 2) { show_usage_and_exit(); }
    ...
    for (auto it_default_sizes = inputfile_default_sizes.entries.begin();
      ...
      ++it_default_sizes) {
      if (it_default_sizes->product_size == product_size) { continue; }
      product_size = it_default_sizes->product_size;
      while (
        it_all_pot_sizes != inputfile_all_pot_sizes.entries.end() && it_all_pot_sizes->product_size != product_size) {
        ++it_all_pot_sizes;
      }
      if (it_all_pot_sizes == inputfile_all_pot_sizes.entries.end()) { break; }
      ...
      if (t.k == t.m && t.m == t.n) { cubic_results.push_back(entry); }
    }

    cout << "All results:" << endl;
    for (auto it = results.begin(); it != results.end(); ++it) { cout << *it << endl; }
    cout << endl;
    ...
    for (size_t i = 0; i < n; i++) { cout << results[i] << endl; }
    cout << endl;

    cout << "cubic results:" << endl;
    for (auto it = cubic_results.begin(); it != cubic_results.end(); ++it) { cout << *it << endl; }
    cout << endl;
...
int main(int argc, char *argv[])
{
  ...
  if (argc < 2) { show_usage_and_exit(argc, argv, available_actions); }
  ...
  if (!action) { show_usage_and_exit(argc, argv, available_actions); }
