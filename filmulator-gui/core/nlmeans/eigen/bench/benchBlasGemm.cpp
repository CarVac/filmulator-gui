  ...
  if (!(std::string(argv[1]) == "auto")) {
    ...
    for (uint k = 0; k < nbtries; ++k) {
      ...
      for (uint j = 0; j < nbloops; ++j)
        ...
        CBLAS_GEMM(
          CblasRowMajor, CblasNoTrans, CblasNoTrans, M, N, K, alpha, ma.data(), K, mb.data(), N, beta, mc.data(), N);
#else
        CBLAS_GEMM(
          CblasColMajor, CblasNoTrans, CblasNoTrans, M, N, K, alpha, ma.data(), M, mb.data(), K, beta, mc.data(), M);
#endif
      timer.stop();
    }
    if (!(std::string(argv[1]) == "auto"))
