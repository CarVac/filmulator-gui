...
template<typename T> EIGEN_DONT_INLINE typename T::Scalar sqsumNorm(T &v) { return v.norm(); }

template<typename T> EIGEN_DONT_INLINE typename T::Scalar stableNorm(T &v) { return v.stableNorm(); }

template<typename T> EIGEN_DONT_INLINE typename T::Scalar hypotNorm(T &v) { return v.hypotNorm(); }

template<typename T> EIGEN_DONT_INLINE typename T::Scalar blueNorm(T &v) { return v.blueNorm(); }

template<typename T> EIGEN_DONT_INLINE typename T::Scalar lapackNorm(T &v)
{
...
template<typename T> EIGEN_DONT_INLINE typename T::Scalar twopassNorm(T &v)
{
  typedef typename T::Scalar Scalar;
...
template<typename T> EIGEN_DONT_INLINE typename T::Scalar bl2passNorm(T &v) { return v.stableNorm(); }

template<typename T> EIGEN_DONT_INLINE typename T::Scalar divacNorm(T &v)
{
  int n = v.size() / 2;
...
template<typename T> EIGEN_DONT_INLINE typename T::Scalar pblueNorm(const T &v)
{
  ...
  if (nmax <= 0) {
    ...
    ibeta = std::numeric_limits<Scalar>::radix;// NumTraits<Scalar>::Base;                    // base for floating-point
                                               // numbers
    it = std::numeric_limits<Scalar>::digits;// NumTraits<Scalar>::Mantissa;                // number of base-beta
                                             // digits in mantissa
