...
template<typename Transformation, typename Data> EIGEN_DONT_INLINE void transform(const Transformation &t, Data &data)
{
...
template<typename Scalar, typename Data> EIGEN_DONT_INLINE void transform(const Quaternion<Scalar> &t, Data &data)
{
...
template<typename T> struct get_dim
{
  enum { Dim = T::Dim };
};
template<typename S, int R, int C, int O, int MR, int MC> struct get_dim<Matrix<S, R, C, O, MR, MC>>
{
  enum { Dim = R };
};

template<typename Transformation, int N> struct bench_impl
{
...
template<typename Transformation> struct bench_impl<Transformation, 0>
{
  static EIGEN_DONT_INLINE void run(const Transformation &) {}
};

template<typename Transformation> EIGEN_DONT_INLINE void bench(const std::string &msg, const Transformation &t)
{
  cout << msg << " ";
