...
template<int S> struct map_size
{
  enum { ret = S };
};
template<> struct map_size<10>
{
  enum { ret = 20 };
};
template<> struct map_size<11>
{
  enum { ret = 50 };
};
template<> struct map_size<12>
{
  enum { ret = 100 };
};
template<> struct map_size<13>
{
  enum { ret = 300 };
};
...
template<int M, int N, int K, typename Scalar, int Mode> EIGEN_DONT_INLINE void bench_prod()
{
...
template<int M, int N> struct loop_on_k<M, N, END>
{
  static void run() {}
};
...
template<int N, int K> struct loop_on_m<END, N, K>
{
  static void run() {}
};
...
template<int M, int K, typename Scalar, int Mode> struct loop_on_n<M, END, K, Scalar, Mode>
{
  static void run() {}
};
