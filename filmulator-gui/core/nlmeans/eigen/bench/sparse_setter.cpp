...
EIGEN_DONT_INLINE Scalar *setinnerrand_eigen(const Coordinates &coords, const Values &vals)
{
  ...
  for (int i = 0; i < coords.size(); ++i) { mat.insert(coords[i].x(), coords[i].y()) = vals[i]; }
  mat.finalize();
  CHECK_MEM;
  return 0;
}
...
EIGEN_DONT_INLINE Scalar *setrand_eigen_dynamic(const Coordinates &coords, const Values &vals)
{
  ...
  for (int i = 0; i < coords.size(); ++i) { mat.coeffRef(coords[i].x(), coords[i].y()) += vals[i]; }
  mat.finalize();
  CHECK_MEM;
...
EIGEN_DONT_INLINE Scalar *setrand_eigen_sumeq(const Coordinates &coords, const Values &vals)
{
  ...
  for (int j = 0; j < KK; ++j) {
    ...
    mat.reserve(n);
    for (int i = j * n; i < (j + 1) * n; ++i) { aux.insert(coords[i].x(), coords[i].y()) += vals[i]; }
    aux.finalize();
    mat += aux;
  }
  return &mat.coeffRef(coords[0].x(), coords[0].y());
...
EIGEN_DONT_INLINE Scalar *setrand_eigen_compact(const Coordinates &coords, const Values &vals)
{
  ...
  for (int i = 0; i < coords.size(); ++i) { setter.coeffRef(coords[i].x(), coords[i].y()) += vals[i]; }
...
template<class T1, class T2> bool kv_pair_less(const std::pair<T1, T2> &x, const std::pair<T1, T2> &y)
{
  return x.first < y.first;
}


template<class I, class T> void csr_sort_indices(const I n_row, const I Ap[], I Aj[], T Ax[])
{
...
template<class I, class T> void csr_sum_duplicates(const I n_row, const I n_col, I Ap[], I Aj[], T Ax[])
{
  I nnz = 0;
  I row_end = 0;
  for (I i = 0; i < n_row; i++) {
...
EIGEN_DONT_INLINE Scalar *setrand_scipy(const Coordinates &coords, const Values &vals)
{
  ...
  coo_tocsr<Scalar>(
    SIZE, SIZE, coords.size(), coords, vals, mat._outerIndexPtr(), mat._innerIndexPtr(), mat._valuePtr());
...
EIGEN_DONT_INLINE Scalar *setrand_ublas_mapped(const Coordinates &coords, const Values &vals)
{
  ...
  for (int i = 0; i < coords.size(); ++i) { aux(coords[i].x(), coords[i].y()) += vals[i]; }
  CHECK_MEM;
