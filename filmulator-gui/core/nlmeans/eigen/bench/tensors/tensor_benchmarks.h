...
public:
  ...
  void memcpy(int num_iters)
  {
    ...
    for (int iter = 0; iter < num_iters; ++iter) { device_.memcpy(c_, a_, m_ * m_ * sizeof(T)); }
  ...
  void typeCasting(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { B.device(device_) = A.template cast<T>(); }
    // Record the number of values copied per second
  ...
  void random(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = C.random(); }
  ...
  void rowChip(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = B.chip(iter % k_, 0); }
  ...
  void colChip(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = B.chip(iter % n_, 1); }
    // Record the number of values copied from the rhs chip to the lhs.
    finalizeBenchmark(static_cast<int64_t>(n_) * num_iters);
  }
  ...
  void shuffling(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { B.device(device_) = A.shuffle(shuffle); }
  ...
  void padding(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { B.device(device_) = A.pad(paddings); }
  ...
  void striding(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { B.device(device_) = A.stride(strides); }
    // Record the number of values copied from the padded tensor A each second
    finalizeBenchmark(static_cast<int64_t>(m_) * k_ * num_iters);
  }
  ...
  void broadcasting(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = A.broadcast(broadcast); }
  ...
  void algebraicFunc(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = A.rsqrt() + B.sqrt() * B.square(); }
  ...
  void transcendentalFunc(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = A.exp() + B.log(); }
    // Record the number of FLOP executed per second (assuming one operation
    // per value)
    finalizeBenchmark(static_cast<int64_t>(m_) * m_ * num_iters);
  }
  ...
  void rowReduction(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = B.sum(sum_along_dim); }
    // Record the number of FLOP executed per second (assuming one operation
    // per value)
  ...
  void fullReduction(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = B.sum(); }
    // Record the number of FLOP executed per second (assuming one operation
    // per value)
    finalizeBenchmark(static_cast<int64_t>(k_) * n_ * num_iters);
  }
  ...
  void contraction(int num_iters)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = A.contract(B, dims); }
  ...
  void convolution(int num_iters, int kernel_x, int kernel_y)
  {
    ...
    StartBenchmarkTiming();
    for (int iter = 0; iter < num_iters; ++iter) { C.device(device_) = A.convolve(B, dims); }
    ...
    finalizeBenchmark(
      static_cast<int64_t>(2) * (m_ - kernel_x + 1) * (n_ - kernel_y + 1) * kernel_x * kernel_y * num_iters);
  }
...
private:
  ...
  inline void finalizeBenchmark(int64_t num_items)
  {
    ...
    if (Eigen::internal::is_same<Device, Eigen::GpuDevice>::value) { device_.synchronize(); }
#endif
    StopBenchmarkTiming();
