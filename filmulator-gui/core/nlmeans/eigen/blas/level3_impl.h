...
int EIGEN_BLAS_FUNC(gemm)(const char *opa,
  const char *opb,
  const int *m,
  const int *n,
  const int *k,
  ...
  const int *ldc)
{
  ...
  static const functype func[12] = { // array index: NOTR  | (NOTR << 2)
...
int EIGEN_BLAS_FUNC(trsm)(const char *side,
  const char *uplo,
  const char *opa,
  const char *diag,
  const int *m,
...
int EIGEN_BLAS_FUNC(trmm)(const char *side,
  const char *uplo,
  const char *opa,
  const char *diag,
  const int *m,
...
int EIGEN_BLAS_FUNC(symm)(const char *side,
  const char *uplo,
  const int *m,
  const int *n,
  const RealScalar *palpha,
  ...
  const int *ldc)
{
  ...
  if (SIDE(*side) == LEFT)
    if (UPLO(*uplo) == UP)
      internal::
        product_selfadjoint_matrix<Scalar, DenseIndex, RowMajor, true, false, ColMajor, false, false, ColMajor>::run(
          *m, *n, a, *lda, b, *ldb, c, *ldc, alpha, blocking);
    else if (UPLO(*uplo) == LO)
      internal::
        product_selfadjoint_matrix<Scalar, DenseIndex, ColMajor, true, false, ColMajor, false, false, ColMajor>::run(
          *m, *n, a, *lda, b, *ldb, c, *ldc, alpha, blocking);
    else
      return 0;
  else if (SIDE(*side) == RIGHT)
    if (UPLO(*uplo) == UP)
      internal::
        product_selfadjoint_matrix<Scalar, DenseIndex, ColMajor, false, false, RowMajor, true, false, ColMajor>::run(
          *m, *n, b, *ldb, a, *lda, c, *ldc, alpha, blocking);
    else if (UPLO(*uplo) == LO)
      internal::
        product_selfadjoint_matrix<Scalar, DenseIndex, ColMajor, false, false, ColMajor, true, false, ColMajor>::run(
          *m, *n, b, *ldb, a, *lda, c, *ldc, alpha, blocking);
    else
      return 0;
  else
...
int EIGEN_BLAS_FUNC(syrk)(const char *uplo,
  const char *op,
  const int *n,
  const int *k,
  const RealScalar *palpha,
...
int EIGEN_BLAS_FUNC(syr2k)(const char *uplo,
  const char *op,
  const int *n,
  const int *k,
  const RealScalar *palpha,
...
// c = alpha*a*b + beta*c  for side = 'L'or'l'
// c = alpha*b*a + beta*c  for side = 'R'or'r
int EIGEN_BLAS_FUNC(hemm)(const char *side,
  const char *uplo,
  const int *m,
  const int *n,
  const RealScalar *palpha,
...
int EIGEN_BLAS_FUNC(herk)(const char *uplo,
  const char *op,
  const int *n,
  const int *k,
  const RealScalar *palpha,
...
int EIGEN_BLAS_FUNC(her2k)(const char *uplo,
  const char *op,
  const int *n,
  const int *k,
  const RealScalar *palpha,
