...
template<typename MatrixType> void comparisons(const MatrixType &m)
{
  ...
  VERIFY_IS_APPROX(
    ((m1.array().abs() + 1) > RealScalar(0.1)).matrix().rowwise().count(), VectorOfIndices::Constant(rows, cols));
}
...
template<typename VectorType> void lpNorm(const VectorType &v)
{
  ...
  VERIFY_IS_APPROX(
    numext::pow(u.template lpNorm<5>(), typename VectorType::RealScalar(5)), u.array().abs().pow(5).sum());
}
...
template<int> void regression_bug_654()
{
...
template<int> void regrrssion_bug_1410()
{
  const Matrix4i M;
  const Array4i A;
