...
template<typename MatrixType> void basicStuff(const MatrixType &m)
{
  ...
  sm2.setZero();
  for (typename MatrixType::Index i = 0; i < rows; ++i) sm2.col(i) = sm1.row(i);
  VERIFY_IS_APPROX(sm2, sm1.transpose());

  sm2.setZero();
  for (typename MatrixType::Index i = 0; i < rows; ++i) sm2.col(i).noalias() = sm1.row(i);
  VERIFY_IS_APPROX(sm2, sm1.transpose());
...
template<typename Scalar> void fixedSizeMatrixConstruction()
{
  Scalar raw[4];
