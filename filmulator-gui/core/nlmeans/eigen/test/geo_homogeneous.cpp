...
template<typename Scalar, int Size> void homogeneous(void)
{
  ...
  */
  ...
  VERIFY_IS_APPROX(
    (t2.lazyProduct(pts.colwise().homogeneous())).colwise().hnormalized(), (t2 * pts1).colwise().hnormalized());

  VERIFY_IS_APPROX(
    (v0.transpose().homogeneous().lazyProduct(t2)).hnormalized(), (v0.transpose().homogeneous() * t2).hnormalized());
  ...
  VERIFY_IS_APPROX(
    (t2.template triangularView<Lower>() * v0.homogeneous()).eval(), (t2.template triangularView<Lower>() * hv0));
}
