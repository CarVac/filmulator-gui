...
template<typename Scalar, int Options> void quaternion(void)
{
  ...
  */
  ...
  if (abs(aa.angle()) > 5 * test_precision<Scalar>() && (aa.axis() - v1.normalized()).norm() < Scalar(1.99)
...
template<typename Scalar> void mapQuaternion(void)
{
  ...
  if (internal::packet_traits<Scalar>::Vectorizable) VERIFY_RAISES_ASSERT((MQuaternionA(array3unaligned)));
#endif
