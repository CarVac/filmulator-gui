...
template<typename T> EIGEN_DONT_INLINE void dont_over_optimize(T &x)
{
...
template<typename Scalar, int Mode, int Options> void transformations()
{
  ...
  */
  ...
  // AngleAxis
  VERIFY_IS_APPROX(
    AngleAxisx(a, v1.normalized()).toRotationMatrix(), Quaternionx(AngleAxisx(a, v1.normalized())).toRotationMatrix());
