...
inline bool test_isApprox(const short &a, const short &b) { return internal::isApprox(a, b, test_precision<short>()); }
...
inline bool test_isApprox(const long &a, const long &b) { return internal::isApprox(a, b, test_precision<long>()); }
...
inline bool test_isApprox(const int &a, const int &b) { return internal::isApprox(a, b, test_precision<int>()); }
...
inline bool test_isApprox(const float &a, const float &b) { return internal::isApprox(a, b, test_precision<float>()); }
...
inline bool test_isApprox(const half &a, const half &b) { return internal::isApprox(a, b, test_precision<half>()); }
  ...
  const EigenBase<T2> &b)
{
  ...
  return sqrt(
    RealScalar((ea - eb).cwiseAbs2().sum()) / RealScalar((std::min)(eb.cwiseAbs2().sum(), ea.cwiseAbs2().sum())));
}
...
template<typename S, int D> S test_relative_error(const Translation<S, D> &a, const Translation<S, D> &b)
{
...
template<typename S, int D> S test_relative_error(const AlignedBox<S, D> &a, const AlignedBox<S, D> &b)
{
...
template<typename T1, typename T2>
typename NumTraits<typename NumTraits<T1>::Real>::NonInteger test_relative_error(const T1 &a,
  const T2 &b,
...
template<typename T> T test_relative_error(const Rotation2D<T> &a, const Rotation2D<T> &b)
{
  return test_relative_error(a.angle(), b.angle());
}

template<typename T> T test_relative_error(const AngleAxis<T> &a, const AngleAxis<T> &b)
{
...
template<typename Type1, typename Type2> inline bool verifyIsApprox(const Type1 &a, const Type2 &b)
{
...
template<typename Derived> inline bool test_isUnitary(const MatrixBase<Derived> &m)
{
  return m.isUnitary(test_precision<typename internal::traits<Derived>::Scalar>());
}

// Forward declaration to avoid ICC warning
template<typename T, typename U> bool test_is_equal(const T &actual, const U &expected, bool expect_equal = true);

template<typename T, typename U> bool test_is_equal(const T &actual, const U &expected, bool expect_equal)
{
  if ((actual == expected) == expect_equal) return true;
  // false:
...
template<typename MatrixType> void createRandomPIMatrixOfRank(Index desired_rank, Index rows, Index cols, MatrixType &m)
{
...
// Forward declaration to avoid ICC warning
template<typename PermutationVectorType> void randomPermutationVector(PermutationVectorType &v, Index size);
template<typename PermutationVectorType> void randomPermutationVector(PermutationVectorType &v, Index size)
{
...
template<typename T> bool isPlusInf(const T &x) { return x > NumTraits<T>::highest(); }

template<typename T> bool isMinusInf(const T &x) { return x < NumTraits<T>::lowest(); }

}// end namespace Eigen

template<typename T> struct GetDifferentType;

template<> struct GetDifferentType<float>
{
  typedef double type;
};
template<> struct GetDifferentType<double>
{
  typedef float type;
};
