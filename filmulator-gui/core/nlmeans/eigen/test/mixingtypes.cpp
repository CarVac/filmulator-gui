...
template<int SizeAtCompileType> void raise_assertion(Index size = SizeAtCompileType)
{
...
template<int SizeAtCompileType> void mixingtypes(int size = SizeAtCompileType)
{
  ...
  rcd = mcd;
  VERIFY_IS_APPROX(
    rcd.noalias() += md + mcd * md, mcd + (md.template cast<CD>().eval()) + mcd * (md.template cast<CD>().eval()));
