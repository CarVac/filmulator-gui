...
template<int N, typename XprType> void use_n_times(const XprType &xpr)
{
...
template<int N, typename ReferenceType, typename XprType> bool verify_eval_type(const XprType &, const ReferenceType &)
{
  typedef typename internal::nested_eval<XprType, N>::type EvalType;
  return internal::is_same<typename internal::remove_all<EvalType>::type,
...
template<typename MatrixType> void run_nesting_ops_1(const MatrixType &_m)
{
  ...
  VERIFY_IS_APPROX(
    (m.transpose() * m).diagonal().array().abs().sum(), (m.transpose() * m).diagonal().array().abs().sum());
