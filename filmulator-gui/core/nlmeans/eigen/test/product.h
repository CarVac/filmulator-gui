...
template<typename MatrixType> void product(const MatrixType &m)
{
  ...
  */
  ...
  if (rows != cols) VERIFY_RAISES_ASSERT(m3 = m1 * m1);
  ...
  // vector at runtime (see bug 1166)
  {
    ...
    VERIFY_IS_APPROX(
      res2.block(0, 0, 1, cols).noalias() = m1.block(0, 0, 1, cols) * square2, (ref2.row(0) = m1.row(0) * square2));
  }
  ...
  // vector.block() (see bug 1283)
  {
    ...
    VERIFY_IS_APPROX(
      w2.block(0, 0, 1, cols).noalias() = vc2.block(0, 0, cols, 1).transpose() * square2, vc2.transpose() * square2);
    ...
    VERIFY_IS_APPROX(
      w2.block(0, 0, 1, cols).noalias() = square2.block(0, 0, 1, cols) * square2, vc2.transpose() * square2);
