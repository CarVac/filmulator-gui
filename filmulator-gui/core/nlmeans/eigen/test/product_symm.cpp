...
template<typename Scalar, int Size, int OtherSize> void symm(int size = Size, int othersize = OtherSize)
{
  ...
  VERIFY_IS_EQUAL(m1, m3);
  VERIFY_IS_APPROX(
    rhs12 += (s1 * m2).template selfadjointView<Upper>() * (s2 * rhs1), rhs13 += (s1 * m1) * (s2 * rhs1));
  ...
  rhs13 = rhs12;
  VERIFY_IS_APPROX(
    rhs12 -= (s1 * m2).template selfadjointView<Lower>() * (s2 * rhs3), rhs13 -= (s1 * m1) * (s2 * rhs3));
...
void test_product_symm()
{
  for (int i = 0; i < g_repeat; i++) {
    CALL_SUBTEST_1((symm<float, Dynamic, Dynamic>(
      internal::random<int>(1, EIGEN_TEST_MAX_SIZE), internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_2((symm<double, Dynamic, Dynamic>(
      internal::random<int>(1, EIGEN_TEST_MAX_SIZE), internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_3((symm<std::complex<float>, Dynamic, Dynamic>(
      internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2), internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2))));
    CALL_SUBTEST_4((symm<std::complex<double>, Dynamic, Dynamic>(
      internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2), internal::random<int>(1, EIGEN_TEST_MAX_SIZE / 2))));
