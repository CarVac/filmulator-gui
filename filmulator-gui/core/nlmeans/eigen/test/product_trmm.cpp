...
template<typename T> int get_random_size()
{
  ...
  int otherCols = OtherCols == Dynamic ? get_random_size<Scalar>() : OtherCols)
{
  ...
  VERIFY_IS_APPROX(
    ge_sx.noalias() = (s2 * ge_left) * (s1 * mat).template triangularView<Mode>(), (s2 * ge_left) * s1tri);
  ...
  VERIFY_IS_APPROX(
    ge_xs = (s1 * mat).transpose().template triangularView<Mode>() * ge_left.adjoint(), s1triTr * ge_left.adjoint());
