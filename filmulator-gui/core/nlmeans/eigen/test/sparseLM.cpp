...
template<typename Scalar> struct sparseGaussianTest : SparseFunctor<Scalar, int>
{
  ...
  int df(const VectorType &uv, JacobianType &fjac)
  {
    ...
    for (int col = 0; col < half; col++) {
      for (int row = 0; row < m; row++) {
        ...
        coeff = coeff * coeff;
        if (coeff < 1. && coeff > 0.) { fjac.coeffRef(row, col) = -(1 - coeff) * (1 - coeff); }
      }
    }
    ...
    for (int col = 0; col < half; col++) {
      for (int row = 0; row < m; row++) {
        coeff = (m_x(row) - col) / v(col);
        coeff = coeff * coeff;
        if (coeff < 1. && coeff > 0.) { fjac.coeffRef(row, col + half) = -4 * (u(col) / v(col)) * coeff * (1 - coeff); }
      }
    }
    return 0;
  }
...
template<typename T> void test_sparseLM_T()
{
