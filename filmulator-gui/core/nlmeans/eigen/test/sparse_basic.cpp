...
template<typename SparseMatrixType> void sparse_basic(const SparseMatrixType &ref)
{
  ...
  // test insert (inner random)
  {
    ...
    for (Index j = 0; j < cols; ++j) {
      for (Index k = 0; k < nnz; ++k) {
        ...
        if (m1.coeff(i, j) == Scalar(0)) m2.insert(i, j) = m1(i, j) = internal::random<Scalar>();
      }
    }

    if (call_reserve && !SparseMatrixType::IsRowMajor) { VERIFY(g_realloc_count == 0); }
  ...
  // test basic computations
  {
    ...
    VERIFY_IS_APPROX(m3 - refM4, refM3 - refM4);
    VERIFY_IS_APPROX(
      (RealScalar(0.5) * refM4 + RealScalar(0.5) * m3).eval(), RealScalar(0.5) * refM4 + RealScalar(0.5) * refM3);
    VERIFY_IS_APPROX(
      (RealScalar(0.5) * refM4 + m3 * RealScalar(0.5)).eval(), RealScalar(0.5) * refM4 + RealScalar(0.5) * refM3);
    VERIFY_IS_APPROX(
      (RealScalar(0.5) * refM4 + m3.cwiseProduct(m3)).eval(), RealScalar(0.5) * refM4 + refM3.cwiseProduct(refM3));
    ...
    // and/or
    {
      ...
      if (mb1.coeffs().all() && mb2.coeffs().all()) {
        VERIFY_IS_EQUAL(
          mb3.nonZeros(), (refM1.real().template cast<bool>() && refM2.real().template cast<bool>()).count());
      }
    }
  }
  ...
  // test reverse iterators
  {
    ...
    for (Index j = 0; j < m2.outerSize(); ++j) {
      ...
      for (typename SparseMatrixType::ReverseInnerIterator it(m2, j); it; --it) {
        VERIFY_IS_APPROX(
          std::abs(ref_value[ref_value.size() - count_forward + count_reverse]) + 1, std::abs(it.value()) + 1);
...
template<typename SparseMatrixType> void big_sparse_triplet(Index rows, Index cols, double density)
{
  typedef typename SparseMatrixType::StorageIndex StorageIndex;
  typedef typename SparseMatrixType::Scalar Scalar;
