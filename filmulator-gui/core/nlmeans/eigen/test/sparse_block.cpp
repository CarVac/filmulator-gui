...
template<typename T>
typename Eigen::internal::enable_if<(T::Flags & RowMajorBit) == RowMajorBit, typename T::RowXpr>::type innervec(T &A,
  Index i)
{
  return A.row(i);
}
...
template<typename T>
typename Eigen::internal::enable_if<(T::Flags & RowMajorBit) == 0, typename T::ColXpr>::type innervec(T &A, Index i)
{
  return A.col(i);
}
...
template<typename SparseMatrixType> void sparse_block(const SparseMatrixType &ref)
{
  ...
  // test innerVector()
  {
    ...
    for (Index j = 0; j < (std::min)(outer, inner); ++j) {
      ...
      if (j > 0) VERIFY(j == numext::real(m3.innerVector(j).lastCoeff()));
    }
    m3.makeCompressed();
    for (Index j = 0; j < (std::min)(outer, inner); ++j) {
      VERIFY(j == numext::real(m3.innerVector(j).nonZeros()));
  ...
  // test innerVectors()
  {
    ...
    if (SparseMatrixType::IsRowMajor)
      VERIFY_IS_APPROX(
        m2.innerVectors(j0, n0) + m2.innerVectors(j1, n0), refMat2.middleRows(j0, n0) + refMat2.middleRows(j1, n0));
    else
