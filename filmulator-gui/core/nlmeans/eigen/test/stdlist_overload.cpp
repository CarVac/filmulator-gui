...
template<class Container, class Position> typename Container::iterator get(Container &c, Position position)
{
...
template<class Container, class Position, class Value> void set(Container &c, Position position, const Value &value)
{
  typename Container::iterator it = c.begin();
  std::advance(it, position);
  *it = value;
...
template<typename MatrixType> void check_stdlist_matrix(const MatrixType &m)
{
  ...
  *itw = *itv;
  ...
  for (unsigned int i = 23; i < v.size(); ++i) { VERIFY((*get(v, i)) == (*get(w, (i - 23) % w.size()))); }
}

template<typename TransformType> void check_stdlist_transform(const TransformType &)
{
  ...
  *itw = *itv;
  ...
  for (int i = 0; i < 30 || ((ref == &(*get(w, 0))) && i < 300); ++i) v.push_back(*get(w, i % w.size()));
  for (unsigned int i = 23; i < v.size(); ++i) { VERIFY(get(v, i)->matrix() == get(w, (i - 23) % w.size())->matrix()); }
}

template<typename QuaternionType> void check_stdlist_quaternion(const QuaternionType &)
{
  ...
  *itw = *itv;
  ...
  for (int i = 0; i < 30 || ((ref == &(*get(w, 0))) && i < 300); ++i) v.push_back(*get(w, i % w.size()));
  for (unsigned int i = 23; i < v.size(); ++i) { VERIFY(get(v, i)->coeffs() == get(w, (i - 23) % w.size())->coeffs()); }
}
