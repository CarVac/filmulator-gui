...
template<typename SvdType, typename MatrixType> void svd_check_full(const MatrixType &m, const SvdType &svd)
{
  Index rows = m.rows();
  Index cols = m.cols();
...
void svd_compare_to_full(const MatrixType &m, unsigned int computationOptions, const SvdType &referenceSvd)
{
  ...
  if (computationOptions & (ComputeFullV | ComputeThinV)) {
    ...
    VERIFY_IS_APPROX(
      svd.matrixV().leftCols(diagSize) * svd.singularValues().asDiagonal() * svd.matrixV().leftCols(diagSize).adjoint(),
  ...
  if (computationOptions & (ComputeFullU | ComputeThinU)) {
    ...
    VERIFY_IS_APPROX(svd.matrixU().leftCols(diagSize) * svd.singularValues().cwiseAbs2().asDiagonal()
      ...
      referenceSvd.matrixU().leftCols(diagSize) * referenceSvd.singularValues().cwiseAbs2().asDiagonal()
...
template<typename MatrixType> void svd_min_norm(const MatrixType &m, unsigned int computationOptions)
{
  typedef typename MatrixType::Scalar Scalar;
  Index cols = m.cols();
...
template<typename SvdType, typename MatrixType> void svd_inf_nan()
{
  SvdType svd;
  typedef typename MatrixType::Scalar Scalar;
...
template<typename> void svd_underoverflow()
{
#if defined __INTEL_COMPILER
...
template<typename MatrixType> void svd_all_trivial_2x2(void (*cb)(const MatrixType &, bool))
{
  MatrixType M;
...
template<typename> void svd_preallocate()
{
