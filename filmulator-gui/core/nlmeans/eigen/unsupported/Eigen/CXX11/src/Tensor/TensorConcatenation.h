...
namespace internal {
  ...
  template<typename Axis, typename LhsXprType, typename RhsXprType>
  struct nested<TensorConcatenationOp<Axis, LhsXprType, RhsXprType>,
    1,
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorConcatenationOp &operator=(const OtherDerived &other)
  {
...
struct TensorEvaluator<const TensorConcatenationOp<Axis, LeftArgType, RightArgType>, Device>
{
  ...
  {
    const double compute_cost = NumDims
    ...
    return (lhs_size / (lhs_size + rhs_size)) * m_leftImpl.costPerCoeff(vectorized)
           + (rhs_size / (lhs_size + rhs_size)) * m_rightImpl.costPerCoeff(vectorized)
  ...
  : public TensorEvaluator<const TensorConcatenationOp<Axis, LeftArgType, RightArgType>, Device>
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(XprType &op, const Device &device) : Base(op, device)
  {
