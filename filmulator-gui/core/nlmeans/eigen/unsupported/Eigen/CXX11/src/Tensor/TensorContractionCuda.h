...
__device__ EIGEN_STRONG_INLINE void EigenContractionKernelInternal(const LhsMapper lhs,
  const RhsMapper rhs,
...
__global__ void __launch_bounds__(512) EigenContractionKernel(const LhsMapper lhs,
  const RhsMapper rhs,
  const OutputMapper output,
  const Index m_size,
  const Index n_size,
...
__device__ EIGEN_STRONG_INLINE void EigenFloatContractionKernelInternal16x16(const LhsMapper lhs,
  const RhsMapper rhs,
  const OutputMapper output,
  ...
  const Index base_n)
{
  ...
  float4 results[4];
  for (int i = 0; i < 4; i++) { results[i].x = results[i].y = results[i].z = results[i].w = 0; }


#define prefetch_lhs(reg, row, col)                                  \
  if (!CHECK_LHS_BOUNDARY) {                                         \
    if (col < k_size) { reg = lhs.loadPacket<Unaligned>(row, col); } \
  } else {                                                           \
    if (col < k_size) {                                              \
      if (row + 3 < m_size) {                                        \
...
#undef add_vals
  ...
  if (!CHECK_LHS_BOUNDARY && !CHECK_RHS_BOUNDARY) {
    ...
    if (lhs_vert + 3 < m_size) {
      ...
      for (int i = 0; i < 4; i++) { output(lhs_vert, horiz_base + i) = results[i].x; }
    }
...
template<typename Index,
  ...
  bool CHECK_LHS_BOUNDARY,
  bool CHECK_RHS_BOUNDARY>
__device__ EIGEN_STRONG_INLINE void EigenFloatContractionKernelInternal(const LhsMapper lhs,
  const RhsMapper rhs,
  const OutputMapper output,
  float2 lhs_shmem2[][32],
  float2 rhs_shmem2[][8],
  const Index m_size,
  const Index n_size,
  const Index k_size,
  ...
  const Index base_n)
{
  ...
  float4 results[8];
  for (int i = 0; i < 8; i++) { results[i].x = results[i].y = results[i].z = results[i].w = 0; }
...
#pragma unroll
  ...
  if (!CHECK_LHS_BOUNDARY && !CHECK_RHS_BOUNDARY) {
    ...
    if (lhs_vert + 3 < m_size) {
      ...
      for (int i = 0; i < 8; i++) { output(lhs_vert, horiz_base + i) = results[i].x; }
    }
  } else if (!CHECK_LHS_BOUNDARY) {
    // CHECK BOUNDARY_B
    for (int i = 0; i < 8; i++) {
...
__global__ void __launch_bounds__(256) EigenFloatContractionKernel(const LhsMapper lhs,
  const RhsMapper rhs,
  const OutputMapper output,
  const Index m_size,
  const Index n_size,
...
template<typename Index, typename LhsMapper, typename RhsMapper, typename OutputMapper>
__global__ void __launch_bounds__(256) EigenFloatContractionKernel16x16(const LhsMapper lhs,
  const RhsMapper rhs,
  const OutputMapper output,
  const Index m_size,
  const Index n_size,
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType>, GpuDevice>
  : public TensorContractionEvaluatorBase<
      TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType>, GpuDevice>>
{
  ...
  {
    static void Run(const LhsMapper &lhs,
      const RhsMapper &rhs,
      const OutputMapper &output,
      Index m,
      Index n,
      ...
      const GpuDevice &device)
    {
      ...
      LAUNCH_CUDA_KERNEL((EigenContractionKernel<Scalar, Index, LhsMapper, RhsMapper, OutputMapper>),
        num_blocks,
        block_size,
        0,
        device,
  ...
  struct LaunchKernels<float, float, Index, LhsMapper, RhsMapper, OutputMapper>
  {
      ...
      const GpuDevice &device)
    {
      if (m < 768 || n < 768) {
        ...
        LAUNCH_CUDA_KERNEL((EigenFloatContractionKernel16x16<Index, LhsMapper, RhsMapper, OutputMapper>),
          num_blocks,
          block_size,
          0,
          device,
        ...
        LAUNCH_CUDA_KERNEL((EigenFloatContractionKernel<Index, LhsMapper, RhsMapper, OutputMapper>),
          num_blocks,
          block_size,
          0,
          device,
  ...
  void evalTyped(Scalar *buffer) const
  {
    ...
    LaunchKernels<LhsScalar, RhsScalar, Index, LhsMapper, RhsMapper, OutputMapper>::Run(
      lhs, rhs, output, m, n, k, this->m_device);
  }
};
