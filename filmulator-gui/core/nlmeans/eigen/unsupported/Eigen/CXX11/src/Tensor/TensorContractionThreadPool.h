...
namespace internal {
  ...
  template<typename LhsScalar, typename LhsMapper, typename Index> struct packLhsArg
  {
    LhsScalar *blockA;
    const LhsMapper &lhs;
    const Index m_start;
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType>, ThreadPoolDevice>
  : public TensorContractionEvaluatorBase<
      TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType>, ThreadPoolDevice>>
{
  ...
  typedef typename internal::conditional<static_cast<int>(Layout) == static_cast<int>(ColMajor),
    LeftArgType,
  ...
  static const int LDims = internal::array_size<typename TensorEvaluator<EvalLeftArgType, Device>::Dimensions>::value;
  static const int RDims = internal::array_size<typename TensorEvaluator<EvalRightArgType, Device>::Dimensions>::value;
  ...
  {
    typedef internal::TensorContractionInputMapper<LhsScalar,
      Index,
      internal::Lhs,
      LeftEvaluator,
      left_nocontract_t,
      contract_t,
      internal::packet_traits<LhsScalar>::size,
      lhs_inner_dim_contiguous,
      false,
      Unaligned>
      LhsMapper;
    typedef internal::TensorContractionInputMapper<RhsScalar,
      Index,
      internal::Rhs,
      RightEvaluator,
      right_nocontract_t,
      contract_t,
      internal::packet_traits<RhsScalar>::size,
    ...
    typedef internal::
      gemm_pack_lhs<LhsScalar, Index, typename LhsMapper::SubMapper, Traits::mr, Traits::LhsProgress, ColMajor>
        LhsPacker;
    typedef internal::gemm_pack_rhs<RhsScalar, Index, typename RhsMapper::SubMapper, Traits::nr, ColMajor> RhsPacker;
    typedef internal::gebp_kernel<LhsScalar, RhsScalar, Index, OutputMapper, Traits::mr, Traits::nr, false, false>
      GebpKernel;
    ...
    Index bm, bn, bk;
    if (shard_by_col) {
      internal::TensorContractionBlocking<LhsMapper, RhsMapper, Index, internal::ShardByCol> blocking(k, m, n, 2);
      bm = blocking.mc();
      bn = blocking.nc();
      bk = blocking.kc();
    } else {
      internal::TensorContractionBlocking<LhsMapper, RhsMapper, Index, internal::ShardByRow> blocking(k, m, n, 2);
      bm = blocking.mc();
      bn = blocking.nc();
      bk = blocking.kc();
    }
    ...
    if (shard_by_col) {
      internal::TensorContractionBlocking<LhsMapper, RhsMapper, Index, internal::ShardByCol> blocking(
        k, m, n, num_threads);
      bm = blocking.mc();
      bn = blocking.nc();
      bk = blocking.kc();
    } else {
      internal::TensorContractionBlocking<LhsMapper, RhsMapper, Index, internal::ShardByRow> blocking(
        k, m, n, num_threads);
      bm = blocking.mc();
      bn = blocking.nc();
      bk = blocking.kc();
    ...
    LhsMapper lhs(this->m_leftImpl,
      this->m_left_nocontract_strides,
      this->m_i_strides,
      this->m_left_contracting_strides,
      this->m_k_strides);

    RhsMapper rhs(this->m_rightImpl,
      this->m_right_nocontract_strides,
      this->m_j_strides,
      this->m_right_contracting_strides,
      this->m_k_strides);

    Context<LhsPacker, RhsPacker, GebpKernel, LhsMapper, RhsMapper, OutputMapper>(this->m_device,
      num_threads,
      lhs,
      rhs,
      buffer,
  ...
  public:
    Context(const Device &device,
      ...
      bool shard_by_col,
      bool parallel_pack)
      : device_(device), lhs_(lhs), rhs_(rhs), buffer_(buffer), output_(buffer, tm), num_threads_(num_threads),
        shard_by_col_(shard_by_col), parallel_pack_(parallel_pack), m_(tm), n_(tn), k_(tk), bm_(bm), bn_(bn), bk_(bk),
        nm_(nm), nn_(nn), nk_(nk), gm_(gm), gn_(gn), nm0_(nm0), nn0_(nn0)
    {
      for (Index x = 0; x < P; x++) {
      ...
      size_t lhs_size = divup<size_t>(bm_ * bk_ * sizeof(LhsScalar), align) * align;
      size_t rhs_size = divup<size_t>(bn_ * bk_ * sizeof(RhsScalar), align) * align;
      packed_mem_ = static_cast<char *>(
        internal::aligned_malloc((nm0_ * lhs_size + nn0_ * rhs_size) * std::min<size_t>(nk_, P - 1)));
  ...
  private:
    ...
    void pack_lhs(Index m, Index k)
    {
      ...
      for (Index m1 = m * gm_; m1 < mend; m1++)
        LhsPacker()(packed_lhs_[k % (P - 1)][m1], lhs_.getSubMapper(m1 * bm_, k * bk_), bk(k), bm(m1));
    ...
    void pack_rhs(Index n, Index k)
    {
      ...
      for (Index n1 = n * gn_; n1 < nend; n1++) {
        ...
        RhsPacker()(packed_rhs_[k % (P - 1)][n1], rhs_.getSubMapper(k * bk_, n1 * bn_), bk(k), bn(n1));
      }
    ...
    void enqueue_packing_helper(Index start, Index end, Index k, bool rhs)
    {
      if (end - start == 1) {
        ...
        device_.enqueueNoNotification([=]() { enqueue_packing_helper(start, mid, k, rhs); });
      }
    }
    ...
    bool shard_by_col) const
  {
    ...
    double new_parallelism = static_cast<double>(new_tasks) / (divup<int>(new_tasks, num_threads) * num_threads);
    Index old_tasks = divup(nm0, oldgm) * divup(nn0, oldgn);
    double old_parallelism = static_cast<double>(old_tasks) / (divup<int>(old_tasks, num_threads) * num_threads);
...
#define CEIL_DIV(a, b) (((a) + (b) - 1) / (b))
    ...
    */
    ...
    MaxSizeVector<Notification *> kernel_notifications(num_kernel_notifications, nullptr);
    ...
    for (Index k_block_idx = 0; k_block_idx < k_blocks; k_block_idx++) {
      ...
      for (Index m_block_idx = 0; m_block_idx < m_blocks; m_block_idx += numBlockAs) {
        ...
        for (Index mt_block_idx = m_block_idx; mt_block_idx < m_block_idx + num_blocks; mt_block_idx++) {
          ...
          lhs_notifications[blockAId] = this->m_device.enqueue(&Self::packLhs<packLArg, LhsPacker>, arg);
        }
    ...
    for (size_t i = 0; i < lhs_notifications.size(); ++i) { delete lhs_notifications[i]; }

    // deallocate all of the memory for both A and B's
    for (size_t i = 0; i < blockAs.size(); i++) { this->m_device.deallocate(blockAs[i]); }
    for (size_t i = 0; i < blockBs.size(); i++) { this->m_device.deallocate(blockBs[i]); }
...
#undef CEIL_DIV
   ...
   */
  template<typename packLArg, typename LhsPacker> static void packLhs(const packLArg arg)
  {
   ...
   */
  template<typename packRKArg, typename RhsPacker, typename GebpKernel> static void packRhsAndKernel(packRKArg arg)
  {
    if (arg.need_to_pack) {
      RhsPacker pack_rhs;
  ...
  TensorOpCost contractionCost(Index m, Index n, Index bm, Index bn, Index bk, bool shard_by_col, bool prepacked) const
  {
