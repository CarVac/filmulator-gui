 ...
 */
namespace internal {
  template<typename TargetType, typename XprType> struct traits<TensorConversionOp<TargetType, XprType>>
  {
  ...
  template<typename TargetType, typename XprType> struct eval<TensorConversionOp<TargetType, XprType>, Eigen::Dense>
  {
    typedef const TensorConversionOp<TargetType, XprType> &type;
  };

  template<typename TargetType, typename XprType>
  struct nested<TensorConversionOp<TargetType, XprType>,
    1,
...
struct PacketConverter
{
  ...
  template<int LoadMode, typename Index> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TgtPacket packet(Index index) const
  {
...
struct TensorEvaluator<const TensorConversionOp<TargetType, ArgType>, Device>
{
  ...
  enum { IsAligned = false, PacketAccess = true, Layout = TensorEvaluator<ArgType, Device>::Layout, RawAccess = false };
  ...
  {
    return ConversionSubExprEval<internal::is_same<TargetType, SrcType>::value,
      TensorEvaluator<ArgType, Device>,
      Scalar>::run(m_impl, data);
  }
...
protected:
  template<int LoadMode, bool ActuallyVectorize> struct PacketConv
  {
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType run(const TensorEvaluator<ArgType, Device> &impl,
      Index index)
    {
      internal::scalar_cast_op<SrcType, TargetType> converter;
  ...
  template<int LoadMode> struct PacketConv<LoadMode, true>
  {
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType run(const TensorEvaluator<ArgType, Device> &impl,
      Index index)
    {
      ...
      PacketConverter<TensorEvaluator<ArgType, Device>,
        PacketSourceType,
        PacketReturnType,
        SrcCoeffRatio,
        TgtCoeffRatio>
