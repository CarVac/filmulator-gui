...
namespace internal {
  ...
  template<typename Index, typename InputDims, int NumKernelDims, int Layout> class IndexMapper
  {
  public:
    ...
    EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Index mapCudaInputPlaneToTensorInputOffset(Index p) const
    {
      ...
      if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
        ...
        if (NumKernelDims < NumDims) { limit = NumDims - NumKernelDims - 1; }
...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, Device>
{
      ...
      m_kernelArg(op.kernelExpression()), m_kernel(NULL), m_local_kernel(false), m_device(device)
  {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      m_inputStride[0] = 1;
      for (int i = 1; i < NumDims; ++i) { m_inputStride[i] = m_inputStride[i - 1] * input_dims[i - 1]; }
    } else {
      m_inputStride[NumDims - 1] = 1;
      for (int i = NumDims - 2; i >= 0; --i) { m_inputStride[i] = m_inputStride[i + 1] * input_dims[i + 1]; }
    }
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      m_outputStride[0] = 1;
      for (int i = 1; i < NumDims; ++i) { m_outputStride[i] = m_outputStride[i - 1] * m_dimensions[i - 1]; }
    } else {
      ...
      for (int i = NumDims - 2; i >= 0; --i) { m_outputStride[i] = m_outputStride[i + 1] * m_dimensions[i + 1]; }
    }
  }
  ...
  void evalTo(typename XprType::Scalar *buffer)
  {
    ...
    for (int i = 0; i < dimensions().TotalSize(); ++i) { buffer[i] += coeff(i); }
    cleanup();
  }
  ...
  template<int LoadMode> EIGEN_DEVICE_FUNC PacketReturnType packet(const Index index) const
  {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorOpCost costPerCoeff(bool vectorized) const
  {
    ...
    return TensorOpCost(0, 0, firstIndex_compute_cost, vectorized, PacketSize)
           + kernel_size
               * (m_inputImpl.costPerCoeff(vectorized) + m_kernelImpl.costPerCoeff(vectorized)
...
__global__ void EigenConvolutionKernel1D(InputEvaluator eval,
...
__global__ void EigenConvolutionKernel2D(InputEvaluator eval,
...
#pragma unroll
      for (int i = threadIdx.x; i < num_x_input; i += blockDim.x) {
        const int tensor_index =
          plane_input_offset + indexMapper.mapCudaInputKernelToTensorInputOffset(i + first_x, j + first_y);
...
#pragma unroll
        ...
        const int tensor_index =
          plane_output_offset + indexMapper.mapCudaOutputKernelToTensorOutputOffset(i + first_x, j + first_y);
        buffer[tensor_index] = result;
      }
    }
...
__global__ void EigenConvolutionKernel3D(InputEvaluator eval,
    ...
    for (int k = threadIdx.z; k < num_z_input; k += blockDim.z) {
      for (int j = threadIdx.y; j < num_y_input; j += blockDim.y) {
        for (int i = threadIdx.x; i < num_x_input; i += blockDim.x) {
          const int tensor_index =
            plane_input_offset
            + indexMapper.mapCudaInputKernelToTensorInputOffset(i + first_x, j + first_y, k + first_z);
    ...
    for (int k = threadIdx.z; k < num_z_output; k += blockDim.z) {
      for (int j = threadIdx.y; j < num_y_output; j += blockDim.y) {
        for (int i = threadIdx.x; i < num_x_output; i += blockDim.x) {
          ...
          const int tensor_index =
            plane_output_offset
            + indexMapper.mapCudaOutputKernelToTensorOutputOffset(i + first_x, j + first_y, k + first_z);
          buffer[tensor_index] = result;
        }
      }
    }
...
struct TensorEvaluator<const TensorConvolutionOp<Indices, InputArgType, KernelArgType>, GpuDevice>
{
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType &op, const GpuDevice &device)
    : m_inputImpl(op.inputExpression(), device), m_kernelArg(op.kernelExpression()),
      m_kernelImpl(op.kernelExpression(), device), m_indices(op.indices()), m_buf(NULL), m_kernel(NULL),
      m_local_kernel(false), m_device(device)
  {
  ...
  static unsigned int ceil(unsigned int num, unsigned int denom)
  {
    ...
    if (num > rounded_toward_zero * denom) { return rounded_toward_zero + 1; }
  ...
  void executeEval(Scalar *data) const
  {
    ...
    case 1: {
      ...
      switch (kernel_size) {
      case 4: {
        LAUNCH_CUDA_KERNEL((EigenConvolutionKernel1D<TensorEvaluator<InputArgType, GpuDevice>, Index, InputDims, 4>),
          num_blocks,
          block_size,
          shared_mem,
          m_device,
      ...
      case 7: {
        LAUNCH_CUDA_KERNEL((EigenConvolutionKernel1D<TensorEvaluator<InputArgType, GpuDevice>, Index, InputDims, 7>),
          num_blocks,
          block_size,
          shared_mem,
          m_device,
    ...
    case 2: {
      ...
      // num_blocks.z: " << num_blocks.z << " maxX: " << maxX << " maxY: " << maxY << " maxP: " << maxP << " shared_mem:
      // " << shared_mem << " in stream " << m_device.stream() << endl;
      ...
      default: {
        LAUNCH_CUDA_KERNEL(
          (EigenConvolutionKernel2D<TensorEvaluator<InputArgType, GpuDevice>, Index, InputDims, Dynamic, Dynamic>),
          num_blocks,
          block_size,
          shared_mem,
          m_device,
    ...
    case 3: {
      ...
      const int maxX = numext::mini<int>(128,
        numext::mini<int>(maxSharedMem / (sizeof(Scalar) * kernel_size_y * kernel_size_z) - kernel_size_x + 1, numX));
      const int maxY = numext::mini<int>(128,
        numext::mini<int>(
          maxSharedMem / (sizeof(Scalar) * (maxX + kernel_size_x - 1) * kernel_size_z) - kernel_size_y + 1, numY));
      const int maxZ = numext::mini<int>(128,
        numext::mini<int>(
          maxSharedMem / (sizeof(Scalar) * (maxX + kernel_size_x - 1) * (maxY + kernel_size_y - 1)) - kernel_size_z + 1,
          numZ));
      ...
      LAUNCH_CUDA_KERNEL((EigenConvolutionKernel3D<TensorEvaluator<InputArgType, GpuDevice>, Index, InputDims>),
        num_blocks,
        block_size,
        shared_mem,
        m_device,
    ...
    default: {
      EIGEN_STATIC_ASSERT(
        (NumKernelDims >= 1 && NumKernelDims <= 3), THIS_METHOD_IS_ONLY_FOR_OBJECTS_OF_A_SPECIFIC_SIZE);
    }
    }
  }
  ...
  template<int LoadMode> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packet(const Index index) const
  {
    eigen_assert(m_buf);
    eigen_assert(index < m_dimensions.TotalSize());
