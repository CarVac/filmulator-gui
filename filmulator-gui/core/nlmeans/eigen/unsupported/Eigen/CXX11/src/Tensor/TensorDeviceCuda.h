  ...
  if (!m_devicePropInitialized) {
    ...
    if (first) {
      ...
      if (status != cudaSuccess) {
        std::cerr << "Failed to get the number of CUDA devices: " << cudaGetErrorString(status) << std::endl;
      ...
      for (int i = 0; i < num_devices; ++i) {
        ...
        if (status != cudaSuccess) {
          std::cerr << "Failed to initialize CUDA device #" << i << ": " << cudaGetErrorString(status) << std::endl;
          assert(status == cudaSuccess);
        }
      }
...
public:
  ...
  {
    if (scratch_) { deallocate(scratch_); }
  }

  const cudaStream_t &stream() const { return *stream_; }
  const cudaDeviceProp &deviceProperties() const { return m_deviceProperties[device_]; }
  ...
  {
    if (scratch_ == NULL) { scratch_ = allocate(kCudaScratchSize + sizeof(unsigned int)); }
    return scratch_;
  }
...
struct GpuDevice
{
  ...
  EIGEN_STRONG_INLINE const cudaStream_t &stream() const { return stream_->stream(); }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void memcpy(void *dst, const void *src, size_t n) const
  {
    ...
    cudaError_t err = cudaMemcpyAsync(dst, src, n, cudaMemcpyDeviceToDevice, stream_->stream());
    EIGEN_UNUSED_VARIABLE(err)
    assert(err == cudaSuccess);
#else
  ...
  {
    cudaError_t err = cudaMemcpyAsync(dst, src, n, cudaMemcpyHostToDevice, stream_->stream());
    EIGEN_UNUSED_VARIABLE(err)
    assert(err == cudaSuccess);
  }
  ...
  {
    cudaError_t err = cudaMemcpyAsync(dst, src, n, cudaMemcpyDeviceToHost, stream_->stream());
    EIGEN_UNUSED_VARIABLE(err)
    assert(err == cudaSuccess);
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void synchronize() const
  {
    ...
    if (err != cudaSuccess) {
      std::cerr << "Error detected in CUDA stream: " << cudaGetErrorString(err) << std::endl;
      assert(err == cudaSuccess);
    }
#else
  ...
  EIGEN_STRONG_INLINE int getNumCudaMultiProcessors() const { return stream_->deviceProperties().multiProcessorCount; }
  EIGEN_STRONG_INLINE int maxCudaThreadsPerBlock() const { return stream_->deviceProperties().maxThreadsPerBlock; }
