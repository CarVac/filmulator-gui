...
struct SyclDevice
{
  ...
  {
    return (
      get_sycl_buffer<T>(num_bytes, ptr)->template get_access<AcMd, cl::sycl::access::target::global_buffer>(cgh));
  }

  template<typename T>
  inline std::pair<std::map<const void *, std::shared_ptr<void>>::iterator, bool> add_sycl_buffer(const T *ptr,
    size_t num_bytes) const
  {
    using Type = cl::sycl::buffer<T, 1>;
    std::pair<std::map<const void *, std::shared_ptr<void>>::iterator, bool> ret =
      buffer_map.insert(std::pair<const void *, std::shared_ptr<void>>(ptr,
        std::shared_ptr<void>(
          new Type(cl::sycl::range<1>(num_bytes)), [](void *dataMem) { delete static_cast<Type *>(dataMem); })));
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void *allocate(size_t) const { return internal::aligned_malloc(8); }
  ...
  {
    auto host_acc =
      (static_cast<cl::sycl::buffer<T, 1> *>(add_sycl_buffer(dst, n).first->second.get()))
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void memcpyDeviceToHost(T *dst, const T *src, size_t n) const
  {
    ...
    if (it != buffer_map.end()) {
      auto host_acc = (static_cast<cl::sycl::buffer<T, 1> *>(it->second.get()))
