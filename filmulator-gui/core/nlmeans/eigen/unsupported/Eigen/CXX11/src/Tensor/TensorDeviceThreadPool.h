...
{
public:
  Barrier(unsigned int count) : state_(count << 1), notified_(false) { eigen_assert(((count << 1) >> 1) == count); }
...
template<typename SyncType> static EIGEN_STRONG_INLINE void wait_until_ready(SyncType *n)
{
  if (n) { n->Wait(); }
}
...
struct ThreadPoolDevice
{
  ...
  EIGEN_STRONG_INLINE void *allocate(size_t num_bytes) const { return internal::aligned_malloc(num_bytes); }

  EIGEN_STRONG_INLINE void deallocate(void *buffer) const { internal::aligned_free(buffer); }

  EIGEN_STRONG_INLINE void memcpy(void *dst, const void *src, size_t n) const { ::memcpy(dst, src, n); }
  ...
  template<class Function, class... Args> EIGEN_STRONG_INLINE Notification *enqueue(Function &&f, Args &&...args) const
  {
  ...
  {
    pool_->Schedule(std::bind(&FunctionWrapperWithBarrier<Function, Args...>::run, b, f, args...));
  }
    ...
    std::function<void(Index, Index)> f) const
  {
    ...
    for (Index prev_block_count = block_count; max_efficiency < 1.0 && prev_block_count > 1;) {
      ...
      if (coarser_efficiency + 0.01 >= max_efficiency) {
        ...
        if (max_efficiency < coarser_efficiency) { max_efficiency = coarser_efficiency; }
      }
    }
