...
namespace internal {
  ...
  struct fixed_size_tensor_index_linearization_helper
  {
    ...
    EIGEN_DEVICE_FUNC static inline Index run(array<Index, NumIndices> const &indices, const Dimensions &dimensions)
    {
      return array_get<RowMajor ? n - 1 : (NumIndices - n)>(indices) + dget < RowMajor ? n - 1 : (NumIndices - n),
             Dimensions > ::value
                            * fixed_size_tensor_index_linearization_helper<Index, NumIndices, n - 1, RowMajor>::run(
                              indices, dimensions);
    }
  };
  ...
  {
    template<typename Dimensions>
    EIGEN_DEVICE_FUNC static inline Index run(array<Index, NumIndices> const &, const Dimensions &)
    {
      return 0;
    }
  };

  template<typename Index, std::size_t n> struct fixed_size_tensor_index_extraction_helper
  {
    template<typename Dimensions>
    EIGEN_DEVICE_FUNC static inline Index run(const Index index, const Dimensions &dimensions)
    {
  ...
  {
    template<typename Dimensions> EIGEN_DEVICE_FUNC static inline Index run(const Index, const Dimensions &)
    {
      return 0;
    }
  };
...
template<typename std::ptrdiff_t... Indices> struct Sizes : internal::numeric_list<std::ptrdiff_t, Indices...>
{
...
template<std::size_t n> struct non_zero_size
{
...
template<std::size_t V1 = 0, std::size_t V2 = 0, std::size_t V3 = 0, std::size_t V4 = 0, std::size_t V5 = 0>
struct Sizes
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE size_t rank() const { return count; }

  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE size_t TotalSize() { return internal::arg_prod<Base>::value; }
...
template<typename DenseIndex, int NumDims> struct DSizes : array<DenseIndex, NumDims>
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE size_t rank() const { return NumDims; }
  ...
  template<typename... IndexTypes>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE explicit DSizes(DenseIndex firstDimension,
  ...
  EIGEN_DEVICE_FUNC
    DSizes(const DenseIndex i0, const DenseIndex i1, const DenseIndex i2, const DenseIndex i3, const DenseIndex i4)
  {
...
namespace internal {
  ...
  struct tensor_vsize_index_linearization_helper
  {
      ...
      std::vector<DenseIndex> const &dimensions)
    {
      return array_get<RowMajor ? n : (NumIndices - n - 1)>(indices)
             + array_get<RowMajor ? n : (NumIndices - n - 1)>(dimensions)
                 * tensor_vsize_index_linearization_helper<Index, NumIndices, n - 1, RowMajor>::run(
                   indices, dimensions);
    }
  };
...
namespace internal {
  ...
  template<std::size_t n, std::size_t V1, std::size_t V2, std::size_t V3, std::size_t V4, std::size_t V5>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE std::size_t array_get(const Sizes<V1, V2, V3, V4, V5> &)
  {
  ...
  template<typename Dims1, typename Dims2, size_t n, size_t m> struct sizes_match_below_dim
  {
    static EIGEN_DEVICE_FUNC inline bool run(Dims1 &, Dims2 &) { return false; }
  };
  template<typename Dims1, typename Dims2, size_t n> struct sizes_match_below_dim<Dims1, Dims2, n, n>
  {
  ...
  {
    static EIGEN_DEVICE_FUNC inline bool run(Dims1 &, Dims2 &) { return true; }
  };

}// end namespace internal


template<typename Dims1, typename Dims2> EIGEN_DEVICE_FUNC bool dimensions_match(Dims1 &dims1, Dims2 &dims2)
{
  return internal::
    sizes_match_below_dim<Dims1, Dims2, internal::array_size<Dims1>::value, internal::array_size<Dims2>::value>::run(
      dims1, dims2);
}
