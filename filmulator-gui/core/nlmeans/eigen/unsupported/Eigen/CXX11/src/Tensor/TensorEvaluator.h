...
template<typename Derived, typename Device> struct TensorEvaluator
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const Derived &m, const Device &device)
    : m_data(const_cast<typename internal::traits<Derived>::template MakePointer<Scalar>::Type>(m.data())),
      m_dims(m.dimensions()), m_device(device), m_impl(m)
  {}
  ...
  {
    return TensorOpCost(sizeof(CoeffReturnType), 0, 0, vectorized, internal::unpacket_traits<PacketReturnType>::size);
  }

  EIGEN_DEVICE_FUNC typename internal::traits<Derived>::template MakePointer<Scalar>::Type data() const
  {
    return m_data;
  }
...
template<typename Derived, typename Device> struct TensorEvaluator<const Derived, Device>
{
  ...
  EIGEN_DEVICE_FUNC typename internal::traits<Derived>::template MakePointer<const Scalar>::Type data() const
  {
    return m_data;
  }
...
struct TensorEvaluator<const TensorCwiseBinaryOp<BinaryOp, LeftArgType, RightArgType>, Device>
{
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType &op, const Device &device)
    : m_functor(op.functor()), m_leftImpl(op.lhsExpression(), device), m_rightImpl(op.rhsExpression(), device)
  {
  ...
  template<int LoadMode> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packet(Index index) const
  {
    return m_functor.packetOp(
      m_leftImpl.template packet<LoadMode>(index), m_rightImpl.template packet<LoadMode>(index));
  }
...
struct TensorEvaluator<const TensorCwiseTernaryOp<TernaryOp, Arg1Type, Arg2Type, Arg3Type>, Device>
{
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType &op, const Device &device)
    : m_functor(op.functor()), m_arg1Impl(op.arg1Expression(), device), m_arg2Impl(op.arg2Expression(), device),
...
struct TensorEvaluator<const TensorSelectOp<IfArgType, ThenArgType, ElseArgType>, Device>
{
  ...
  enum {
    ...
    PacketAccess = TensorEvaluator<ThenArgType, Device>::PacketAccess
                   & TensorEvaluator<ElseArgType, Device>::PacketAccess & internal::packet_traits<Scalar>::HasBlend,
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType &op, const Device &device)
    : m_condImpl(op.ifExpression(), device), m_thenImpl(op.thenExpression(), device),
  ...
  template<int LoadMode> EIGEN_DEVICE_FUNC PacketReturnType packet(Index index) const
  {
    ...
    return internal::pblend(
      select, m_thenImpl.template packet<LoadMode>(index), m_elseImpl.template packet<LoadMode>(index));
  }
