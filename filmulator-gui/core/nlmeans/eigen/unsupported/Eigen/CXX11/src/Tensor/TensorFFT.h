...
{
  template<typename T> EIGEN_DEVICE_FUNC T operator()(const T &val) const { return val; }
};

template<> struct MakeComplex<true>
{
  template<typename T> EIGEN_DEVICE_FUNC std::complex<T> operator()(const T &val) const
  {
...
{
  template<typename T> EIGEN_DEVICE_FUNC std::complex<T> operator()(const std::complex<T> &val) const { return val; }
};
...
struct TensorEvaluator<const TensorFFTOp<FFT, ArgType, FFTResultType, FFTDir>, Device>
{
  ...
  template<int LoadMode> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE PacketReturnType packet(Index index) const
  {
...
private:
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void evalToBuf(OutputScalar *data)
  {
    ...
    for (size_t i = 0; i < m_fft.size(); ++i) {
      ...
      if (!is_power_of_two) {
        ...
        if (line_len > 1) {
          ...
          const ComplexScalar pos_j_base = ComplexScalar(std::cos(pi_over_len), std::sin(pi_over_len));
          pos_j_base_powered[1] = pos_j_base;
          if (line_len > 2) {
            const ComplexScalar pos_j_base_sq = pos_j_base * pos_j_base;
            for (int j = 2; j < line_len + 1; ++j) {
              pos_j_base_powered[j] =
                pos_j_base_powered[j - 1] * pos_j_base_powered[j - 1] / pos_j_base_powered[j - 2] * pos_j_base_sq;
            }
          }
        }
      }
      ...
      for (Index partial_index = 0; partial_index < m_size / line_len; ++partial_index) {
        ...
        if (stride == 1) {
          ...
          for (int j = 0; j < line_len; ++j, offset += stride) { line_buf[j] = buf[offset]; }
        }
    ...
    if (!write_to_out) {
      for (Index i = 0; i < m_size; ++i) { data[i] = PartOf<FFTResultType>()(buf[i]); }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    processDataLineCooleyTukey(ComplexScalar *line_buf, Index line_len, Index log_len)
  {
    ...
    const ComplexScalar *pos_j_base_powered)
  {
    ...
    for (Index i = n; i < m; ++i) { a[i] = ComplexScalar(0, 0); }
    ...
    for (Index i = n; i < m - n; ++i) { b[i] = ComplexScalar(0, 0); }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static void scramble_FFT(ComplexScalar *data, Index n)
  {
    ...
    Index j = 1;
    for (Index i = 1; i < n; ++i) {
      if (j > i) { std::swap(data[j - 1], data[i - 1]); }
      Index m = n >> 1;
      while (m >= 2 && j > m) {
        j -= m;
        m >>= 1;
  ...
  template<int Dir> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void butterfly_2(ComplexScalar *data)
  {
  ...
  template<int Dir> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void butterfly_4(ComplexScalar *data)
  {
    ComplexScalar tmp[4];
    tmp[0] = data[0] + data[1];
    tmp[1] = data[0] - data[1];
  ...
  template<int Dir> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void butterfly_8(ComplexScalar *data)
  {
