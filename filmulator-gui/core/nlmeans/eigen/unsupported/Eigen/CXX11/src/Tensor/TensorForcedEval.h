...
namespace internal {
  ...
  template<typename XprType, template<class> class MakePointer_>
  struct nested<TensorForcedEvalOp<XprType, MakePointer_>,
    1,
    typename eval<TensorForcedEvalOp<XprType, MakePointer_>>::type>
  {
...
struct TensorEvaluator<const TensorForcedEvalOp<ArgType, MakePointer_>, Device>
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(CoeffReturnType *)
  {
    ...
    internal::TensorExecutor<const EvalTo, typename internal::remove_const<Device>::type, PacketAccess>::run(
      evalToTmp, m_device);
    return true;
  }
