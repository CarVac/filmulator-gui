...
namespace internal {
   ...
   */
  template<typename Scalar> struct scalar_mod_op
  {
    EIGEN_DEVICE_FUNC scalar_mod_op(const Scalar &divisor) : m_divisor(divisor) {}
    EIGEN_DEVICE_FUNC inline Scalar operator()(const Scalar &a) const { return a % m_divisor; }
    const Scalar m_divisor;
  };
  template<typename Scalar> struct functor_traits<scalar_mod_op<Scalar>>
  {
   ...
   */
  template<typename Scalar> struct scalar_mod2_op
  {
    EIGEN_EMPTY_STRUCT_CTOR(scalar_mod2_op);
    EIGEN_DEVICE_FUNC inline Scalar operator()(const Scalar &a, const Scalar &b) const { return a % b; }
  };
  template<typename Scalar> struct functor_traits<scalar_mod2_op<Scalar>>
  {
    enum { Cost = scalar_div_cost<Scalar, false>::value, PacketAccess = false };
  };

  template<typename Scalar> struct scalar_fmod_op
  {
  ...
  template<typename Scalar> struct functor_traits<scalar_fmod_op<Scalar>>
  {
    enum {
  ...
  template<typename T> struct scalar_sigmoid_op
  {
    ...
    template<typename Packet> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet packetOp(const Packet &x) const
    {
  ...
  template<typename T> struct functor_traits<scalar_sigmoid_op<T>>
  {
    enum {
      Cost = NumTraits<T>::AddCost * 2 + NumTraits<T>::MulCost * 6,
      PacketAccess =
        packet_traits<T>::HasAdd && packet_traits<T>::HasDiv && packet_traits<T>::HasNegate && packet_traits<T>::HasExp
    };
  };


  template<typename Reducer, typename Device> struct reducer_traits
  {
  ...
  template<typename T> struct SumReducer
  {
    ...
    template<typename Packet> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet initializePacket() const
    {
      return pset1<Packet>(initialize());
    }
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T finalize(const T accum) const { return accum; }
    template<typename Packet> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet finalizePacket(const Packet &vaccum) const
    {
      return vaccum;
    }
    template<typename Packet>
  ...
  template<typename T, typename Device> struct reducer_traits<SumReducer<T>, Device>
  {
    enum { Cost = NumTraits<T>::AddCost, PacketAccess = PacketType<T, Device>::HasAdd };
  };
  ...
  template<typename T> struct MeanReducer
  {
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T finalize(const T accum) const { return accum / scalarCount_; }
    template<typename Packet> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet finalizePacket(const Packet &vaccum) const
    {
  ...
  template<typename T, typename Device> struct reducer_traits<MeanReducer<T>, Device>
  {
    enum { Cost = NumTraits<T>::AddCost, PacketAccess = PacketType<T, Device>::HasAdd };
  };


  template<typename T, bool IsMax = true, bool IsInteger = true> struct MinMaxBottomValue
  {
    EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE T bottom_value() { return Eigen::NumTraits<T>::lowest(); }
  };
  template<typename T> struct MinMaxBottomValue<T, true, false>
  {
    EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE T bottom_value() { return -Eigen::NumTraits<T>::infinity(); }
  };
  template<typename T> struct MinMaxBottomValue<T, false, true>
  {
    EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE T bottom_value() { return Eigen::NumTraits<T>::highest(); }
  };
  template<typename T> struct MinMaxBottomValue<T, false, false>
  {
    EIGEN_DEVICE_FUNC static EIGEN_STRONG_INLINE T bottom_value() { return Eigen::NumTraits<T>::infinity(); }
  };
  ...
  template<typename T, typename Device> struct reducer_traits<MaxReducer<T>, Device>
  {
    enum { Cost = NumTraits<T>::AddCost, PacketAccess = PacketType<T, Device>::HasMax };
  };
  ...
  template<typename T, typename Device> struct reducer_traits<MinReducer<T>, Device>
  {
    enum { Cost = NumTraits<T>::AddCost, PacketAccess = PacketType<T, Device>::HasMin };
  };
  ...
  template<typename T, typename Device> struct reducer_traits<ProdReducer<T>, Device>
  {
    enum { Cost = NumTraits<T>::MulCost, PacketAccess = PacketType<T, Device>::HasMul };
  };
  ...
  template<typename Device> struct reducer_traits<AndReducer, Device>
  {
    enum { Cost = 1, PacketAccess = false };
  };
  ...
  template<typename Device> struct reducer_traits<OrReducer, Device>
  {
    enum { Cost = 1, PacketAccess = false };
  };
  ...
  template<typename T, typename Device> struct reducer_traits<ArgMaxTupleReducer<T>, Device>
  {
  ...
  template<typename T, typename Device> struct reducer_traits<ArgMinTupleReducer<T>, Device>
  {
    enum { Cost = NumTraits<T>::AddCost, PacketAccess = false };
  };


  template<typename T, typename Index, size_t NumDims> class GaussianGenerator
  {
  public:
    static const bool PacketAccess = false;
    ...
    EIGEN_DEVICE_FUNC GaussianGenerator(const array<T, NumDims> &means, const array<T, NumDims> &std_devs)
      : m_means(means)
    {
      for (size_t i = 0; i < NumDims; ++i) { m_two_sigmas[i] = std_devs[i] * std_devs[i] * 2; }
    }
  ...
  template<typename T, typename Index, size_t NumDims> struct functor_traits<GaussianGenerator<T, Index, NumDims>>
  {
    enum {
      Cost =
