...
namespace internal {
  ...
  template<DenseIndex Rows, DenseIndex Cols, typename XprType>
  struct nested<TensorImagePatchOp<Rows, Cols, XprType>,
    1,
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorImagePatchOp(const XprType &expr,
    ...
    : m_xpr(expr), m_patch_rows(patch_rows), m_patch_cols(patch_cols), m_row_strides(row_strides),
      m_col_strides(col_strides), m_in_row_strides(in_row_strides), m_in_col_strides(in_col_strides),
      m_row_inflate_strides(row_inflate_strides), m_col_inflate_strides(col_inflate_strides), m_padding_explicit(false),
      m_padding_top(0), m_padding_bottom(0), m_padding_left(0), m_padding_right(0), m_padding_type(padding_type),
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorImagePatchOp(const XprType &expr,
    ...
    : m_xpr(expr), m_patch_rows(patch_rows), m_patch_cols(patch_cols), m_row_strides(row_strides),
      m_col_strides(col_strides), m_in_row_strides(in_row_strides), m_in_col_strides(in_col_strides),
      m_row_inflate_strides(row_inflate_strides), m_col_inflate_strides(col_inflate_strides), m_padding_explicit(true),
      m_padding_top(padding_top), m_padding_bottom(padding_bottom), m_padding_left(padding_left),
      m_padding_right(padding_right), m_padding_type(PADDING_VALID), m_padding_value(padding_value)
  {}
...
struct TensorEvaluator<const TensorImagePatchOp<Rows, Cols, ArgType>, Device>
{
    ...
    : m_impl(op.expression(), device)
  {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      for (int i = 4; i < NumDims; ++i) { m_dimensions[i] = input_dims[i - 1]; }
    } else {
      // RowMajor
      // NumDims-1: depth
      ...
      for (int i = NumDims - 5; i >= 0; --i) { m_dimensions[i] = input_dims[i]; }
    }
  ...
  template<int LoadMode> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packet(Index index) const
  {
    ...
    if (patchIndex != indices[1] / m_fastPatchStride) { return packetWithPossibleZero(index); }
