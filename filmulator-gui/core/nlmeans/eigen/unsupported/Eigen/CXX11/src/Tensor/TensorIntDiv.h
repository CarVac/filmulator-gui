...
namespace internal {
  ...
  namespace {
    ...
    template<typename T>
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE typename internal::enable_if<sizeof(T) == 4, int>::type count_leading_zeros(
      const T val)
    {
#ifdef __CUDA_ARCH__
      return __clz(val);
    ...
    template<typename T>
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE typename internal::enable_if<sizeof(T) == 8, int>::type count_leading_zeros(
      const T val)
    {
#ifdef __CUDA_ARCH__
      return __clzll(val);
    ...
    template<typename T> struct DividerTraits
    {
      typedef typename UnsignedTraits<T>::type type;
      static const int N = sizeof(T) * 8;
    };

    template<typename T> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint32_t muluh(const uint32_t a, const T b)
    {
    ...
    template<typename T> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint64_t muluh(const uint64_t a, const T b)
    {
#if defined(__CUDA_ARCH__)
    ...
    template<int N, typename T> struct DividerHelper
    {
      static EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE uint32_t computeMultiplier(const int log_div, const T divider)
      {
        ...
        return static_cast<uint32_t>(
          (static_cast<uint64_t>(1) << (N + log_div)) / divider - (static_cast<uint64_t>(1) << N) + 1);
      }
    };

    template<typename T> struct DividerHelper<64, T>
    {
  ...
  template<typename T, bool div_gt_one = false> struct TensorIntDivisor
  {
  public:
  ...
  template<> class TensorIntDivisor<int32_t, true>
  {
  public:
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorIntDivisor()
    {
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T operator/(const T &numerator,
