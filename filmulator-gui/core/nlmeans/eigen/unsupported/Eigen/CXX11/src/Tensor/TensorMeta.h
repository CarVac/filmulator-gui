...
template<typename T, typename X, typename Y> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE T divup(const X x, const Y y)
{
  return static_cast<T>((x + y - 1) / y);
}

template<typename T> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE T divup(const T x, const T y)
{
  return static_cast<T>((x + y - 1) / y);
}
...
template<typename Scalar, typename Device> struct PacketType : internal::packet_traits<Scalar>
{
...
template<> struct PacketType<half, GpuDevice>
{
  typedef half2 type;
...
template<typename T> struct PacketType<T, SyclDevice>
{
  typedef T type;
...
public:
  ...
  EIGEN_CONSTEXPR EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Tuple() : first(), second() {}

  EIGEN_CONSTEXPR EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Tuple(const U &f, const V &s) : first(f), second(s) {}
...
namespace internal {
  ...
  EIGEN_CONSTEXPR EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE array<Index, sizeof...(Is)> customIndices2Array(IndexType &idx,
    numeric_list<Index, Is...>)
  {
    return { idx[Is]... };
  }
  ...
  template<typename B, typename D> struct is_base_of
  {
    ...
    template<typename BB, typename DD> struct Host
    {
      operator BB *() const;
      operator DD *();
    };
