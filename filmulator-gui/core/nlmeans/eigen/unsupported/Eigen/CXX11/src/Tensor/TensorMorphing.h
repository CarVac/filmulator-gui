...
namespace internal {
  ...
  template<typename NewDimensions, typename XprType>
  struct nested<TensorReshapingOp<NewDimensions, XprType>,
    1,
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorReshapingOp &operator=(const OtherDerived &other)
  {
...

{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType &op, const Device &device) : Base(op, device) {}
...
namespace internal {
  ...
  template<typename StartIndices, typename Sizes, typename XprType>
  struct nested<TensorSlicingOp<StartIndices, Sizes, XprType>,
    1,
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorSlicingOp(const XprType &expr,
...
struct TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>, Device>
{
    ...
    : m_impl(op.expression(), device), m_device(device), m_dimensions(op.sizes()), m_offsets(op.startIndices())
  {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      m_inputStrides[0] = 1;
      for (int i = 1; i < NumDims; ++i) { m_inputStrides[i] = m_inputStrides[i - 1] * input_dims[i - 1]; }
      ...
      for (int i = NumDims - 2; i >= 0; --i) { m_inputStrides[i] = m_inputStrides[i + 1] * input_dims[i + 1]; }
    ...
    if (!NumTraits<typename internal::remove_const<Scalar>::type>::RequireInitialization && data && m_impl.data()) {
      ...
      if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
        for (int i = 0; i < NumDims; ++i) {
          ...
          if (dimensions()[i] != m_impl.dimensions()[i]) { break; }
        }
      } else {
        ...
        for (int i = 0; i < NumDims; ++i) {
          if (m_dimensions[i] != m_impl.dimensions()[i]) {
            ...
            for (int j = i + 1; j < NumDims; ++j) {
              if (m_dimensions[j] > 1) { return NULL; }
  ...
  : public TensorEvaluator<const TensorSlicingOp<StartIndices, Sizes, ArgType>, Device>
{
  ...
  template<int StoreMode> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void writePacket(Index index, const PacketReturnType &x)
  {
    ...
    if (inputIndices[1] - inputIndices[0] == packetSize - 1) {
      ...
      for (int i = 1; i < packetSize - 1; ++i) { this->coeffRef(index + i) = values[i]; }
    }
  }
};
...
namespace internal {
  ...
  template<typename StartIndices, typename StopIndices, typename Strides, typename XprType>
  struct nested<TensorStridingSlicingOp<StartIndices, StopIndices, Strides, XprType>,
    1,
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorStridingSlicingOp(const XprType &expr,
  ...
  template<typename OtherDerived>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorStridingSlicingOp &operator=(const OtherDerived &other)
  {
...
struct TensorEvaluator<const TensorStridingSlicingOp<StartIndices, StopIndices, Strides, ArgType>, Device>
{
    ...
    : m_impl(op.expression(), device), m_device(device), m_strides(op.strides())
  {
    ...
    m_block_total_size_max = numext::maxi(static_cast<std::size_t>(1), device.lastLevelCacheSize() / sizeof(Scalar));
  }
