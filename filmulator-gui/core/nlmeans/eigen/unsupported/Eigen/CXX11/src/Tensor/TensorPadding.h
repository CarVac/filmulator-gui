...
struct TensorEvaluator<const TensorPaddingOp<PaddingDimensions, ArgType>, Device>
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType coeff(Index index) const
  {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      for (int i = NumDims - 1; i > 0; --i) {
        ...
        if (isPaddingAtIndexForDim(idx, i)) { return m_paddingValue; }
        inputIndex += (idx - m_padding[i].first) * m_inputStrides[i];
        index -= idx * m_outputStrides[i];
      }
      if (isPaddingAtIndexForDim(index, 0)) { return m_paddingValue; }
      ...
      if (isPaddingAtIndexForDim(index, NumDims - 1)) { return m_paddingValue; }
  ...
  {
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) { return packetColMajor(index); }
