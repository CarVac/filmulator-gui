 ...
 */
namespace internal {
  template<typename PatchDim, typename XprType> struct traits<TensorPatchOp<PatchDim, XprType>> : public traits<XprType>
  {
  ...
  template<typename PatchDim, typename XprType> struct eval<TensorPatchOp<PatchDim, XprType>, Eigen::Dense>
  {
...
struct TensorEvaluator<const TensorPatchOp<PatchDim, ArgType>, Device>
{
    ...
    : m_impl(op.expression(), device)
  {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      ...
      m_outputStrides[0] = 1;
      for (int i = 1; i < NumDims; ++i) { m_outputStrides[i] = m_outputStrides[i - 1] * m_dimensions[i - 1]; }
    } else {
      for (int i = 0; i < NumDims - 1; ++i) {
      ...
      for (int i = NumDims - 2; i >= 0; --i) { m_outputStrides[i] = m_outputStrides[i + 1] * m_dimensions[i + 1]; }
    }
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorOpCost costPerCoeff(bool vectorized) const
  {
    const double compute_cost =
      NumDims * (TensorOpCost::DivCost<Index>() + TensorOpCost::MulCost<Index>() + 2 * TensorOpCost::AddCost<Index>());
