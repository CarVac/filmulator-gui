...
namespace internal {
  ...
  template<typename T> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T RandomToTypeUniform(uint64_t *state)
  {
  ...
  template<> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Eigen::half RandomToTypeUniform<Eigen::half>(uint64_t *state)
  {
    Eigen::half result;
  ...
  template<> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE float RandomToTypeUniform<float>(uint64_t *state)
  {
    typedef union {
      uint32_t raw;
      float fp;
  ...
  template<> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE double RandomToTypeUniform<double>(uint64_t *state)
  {
    typedef union {
      uint64_t raw;
      double dp;
  ...
  {
    return std::complex<float>(RandomToTypeUniform<float>(state), RandomToTypeUniform<float>(state));
  }
  template<>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE std::complex<double> RandomToTypeUniform<std::complex<double>>(uint64_t *state)
  {
    return std::complex<double>(RandomToTypeUniform<double>(state), RandomToTypeUniform<double>(state));
  }
  ...
  public:
    ...
    template<typename Index> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T operator()(Index i) const
    {
    ...
    template<typename Packet, typename Index> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet packetOp(Index i) const
    {
      const int packetSize = internal::unpacket_traits<Packet>::size;
      EIGEN_ALIGN_MAX T values[packetSize];
      uint64_t local_state = m_state + i;
      for (int j = 0; j < packetSize; ++j) { values[j] = RandomToTypeUniform<T>(&local_state); }
      m_state = local_state;
  ...
  template<typename Scalar> struct functor_traits<UniformRandomGenerator<Scalar>>
  {
    enum {
  ...
  template<typename T> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T RandomToTypeNormal(uint64_t *state)
  {
  ...
  {
    return std::complex<float>(RandomToTypeNormal<float>(state), RandomToTypeNormal<float>(state));
  }
  template<>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE std::complex<double> RandomToTypeNormal<std::complex<double>>(uint64_t *state)
  {
    return std::complex<double>(RandomToTypeNormal<double>(state), RandomToTypeNormal<double>(state));
  }
  ...
  public:
    ...
    template<typename Packet, typename Index> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet packetOp(Index i) const
    {
      ...
      EIGEN_ALIGN_MAX T values[packetSize];
      uint64_t local_state = m_state + i;
      for (int j = 0; j < packetSize; ++j) { values[j] = RandomToTypeNormal<T>(&local_state); }
      m_state = local_state;
      return internal::pload<Packet>(values);
    }
  ...
  template<typename Scalar> struct functor_traits<NormalRandomGenerator<Scalar>>
  {
    enum {
