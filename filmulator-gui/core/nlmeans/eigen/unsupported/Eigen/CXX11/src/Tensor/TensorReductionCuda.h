...
namespace internal {
  ...
  template<typename Type> __device__ inline Type atomicExchCustom(Type *address, Type val)
  {
  ...
  __global__ void FullReductionKernel(Reducer reducer,
    const Self input,
    Index num_coeffs,
    ...
    unsigned int *semaphore)
  {
    ...
    if (gridDim.x == 1) {
      if (first_index == 0) { *output = reducer.initialize(); }
    } else {
      if (threadIdx.x == 0) {
...
#pragma unroll
    ...
    if ((threadIdx.x & (warpSize - 1)) == 0) { atomicReduce(output, accum, reducer); }
  ...
  __global__ void
    ReductionInitFullReduxKernelHalfFloat(Reducer reducer, const Self input, Index num_coeffs, half2 *scratch)
  {
  ...
  __global__ void ReductionInitKernelHalfFloat(Reducer reducer, const Self input, Index num_coeffs, half *output)
  {
    ...
    if (thread_id == 0 && num_coeffs % 2 != 0) { output[num_coeffs - 1] = reducer.initialize(); }
  }

  template<int BlockSize, int NumPerThread, typename Self, typename Reducer, typename Index>
  __global__ void
    FullReductionKernelHalfFloat(Reducer reducer, const Self input, Index num_coeffs, half *output, half2 *scratch)
  {
...
#pragma unroll
    ...
    if ((threadIdx.x & (warpSize - 1)) == 0) { atomicReduce(scratch, accum, reducer); }
  ...
  template<typename Op> __global__ void ReductionCleanupKernelHalfFloat(Op &reducer, half *output, half2 *scratch)
  {
  ...
  struct FullReductionLauncher<Self,
    Op,
    OutputType,
    PacketAccess,
    typename internal::enable_if<internal::is_same<float, OutputType>::value
                                   || internal::is_same<double, OutputType>::value,
      void>::type>
  {
    static void
      run(const Self &self, Op &reducer, const GpuDevice &device, OutputType *output, typename Self::Index num_coeffs)
    {
      ...
      if (num_blocks > 1) { semaphore = device.semaphore(); }
  ...
  {
    static void
      run(const Self &self, Op &reducer, const GpuDevice &device, half *output, typename Self::Index num_coeffs)
    {
      typedef typename Self::Index Index;
      ...
      if (num_blocks > 1) {
        LAUNCH_CUDA_KERNEL((ReductionCleanupKernelHalfFloat<Op>), 1, 1, 0, device, reducer, output, scratch);
      }
    }
  };
#endif
  ...
  template<typename Self, typename Op, bool Vectorizable> struct FullReducer<Self, Op, GpuDevice, Vectorizable>
  {
    ...
    static void run(const Self &self, Op &reducer, const GpuDevice &device, OutputType *output)
    {
      ...
      FullReductionLauncher<Self, Op, OutputType, reducer_traits<Op, GpuDevice>::PacketAccess>::run(
  ...
  __global__ void InnerReductionKernel(Reducer reducer,
    const Self input,
...
#pragma unroll
        ...
        if ((threadIdx.x & (warpSize - 1)) == 0) { atomicReduce(&(output[row]), reduced_val, reducer); }
      }
    }
#else
    assert(0 && "Shouldn't be called on unsupported device");
  ...
  template<int NumPerThread, typename Self, typename Reducer, typename Index>
  __global__ void InnerReductionKernelHalfFloat(Reducer reducer,
    const Self input,
    Index num_coeffs_to_reduce,
    Index num_preserved_coeffs,
    half *output)
  {
    ...
    if (gridDim.x == 1) {
      ...
      if (i < num_preserved_coeffs) { output[i] = reducer.initialize(); }
      __syncthreads();
    }
...
#pragma unroll
            for (int k = 0; k < unroll_times; ++k) {
              ...
              reducer.reducePacket(
                input.m_impl.template packet<Unaligned>(row * num_coeffs_to_reduce + col), &reduced_val1);
              reducer.reducePacket(
                input.m_impl.template packet<Unaligned>((row + 1) * num_coeffs_to_reduce + col), &reduced_val2);
            }
          }
        }
...
#pragma unroll
  ...
  {
    static EIGEN_DEVICE_FUNC bool
      run(const Self &, Op &, const GpuDevice &, OutputType *, typename Self::Index, typename Self::Index)
    {
  ...
  // Specialization for float and double
  template<typename Self, typename Op, typename OutputType, bool PacketAccess>
  struct InnerReductionLauncher<Self,
    Op,
    OutputType,
    PacketAccess,
    typename internal::enable_if<internal::is_same<float, OutputType>::value
      ...
      void>::type>
  {
      ...
      typename Self::Index num_preserved_vals)
    {
      ...
      if (num_blocks > 1) {
        ...
        const int max_blocks = device.getNumCudaMultiProcessors() * device.maxCudaThreadsPerMultiProcessor() / 1024;
        const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
  ...
  template<typename Self, typename Op> struct InnerReductionLauncher<Self, Op, Eigen::half, true>
  {
      ...
      typename Self::Index num_preserved_vals)
    {
      ...
      if (num_blocks > 1) {
        ...
        const int num_blocks = numext::mini<int>(max_blocks, dyn_blocks);
        LAUNCH_CUDA_KERNEL(
          (ReductionInitKernelHalfFloat<Self, Op, Index>), 1, 1, 0, device, reducer, self, num_preserved_vals, output);
      }
  ...
  template<typename Self, typename Op> struct InnerReducer<Self, Op, GpuDevice>
  {
      ...
      typename Self::Index num_preserved_vals)
    {
      ...
      const Index num_coeffs = array_prod(self.m_impl.dimensions());
      // Don't crash when we're called with an input tensor of size 0.
      if (num_coeffs == 0) { return true; }
      // It's faster to use the usual code.
      if (num_coeffs_to_reduce <= 128) { return true; }
  ...
  template<int NumPerThread, typename Self, typename Reducer, typename Index>
  __global__ void OuterReductionKernel(Reducer reducer,
    const Self input,
    Index num_coeffs_to_reduce,
    Index num_preserved_coeffs,
    typename Self::CoeffReturnType *output)
  ...
  template<typename Self, typename Op> struct OuterReducer<Self, Op, GpuDevice>
  {
      ...
      typename Self::Index num_preserved_vals)
    {
      ...
      // It's faster to use the usual code.
      if (num_coeffs_to_reduce <= 32) { return true; }
