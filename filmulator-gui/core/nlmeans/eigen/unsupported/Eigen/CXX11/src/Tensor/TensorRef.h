...
namespace internal {
  ...
  template<typename Dimensions, typename Scalar> class TensorLazyBaseEvaluator
  {
  public:
...
public:
  ...
  template<typename Expression> EIGEN_STRONG_INLINE TensorRef &operator=(const Expression &expr)
  {
    unrefEvaluator();
    m_evaluator = new internal::TensorLazyEvaluator<Dimensions, Expression, DefaultDevice>(expr, DefaultDevice());
    m_evaluator->incrRefCount();
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const Scalar coeff(const array<Index, NumIndices> &indices) const
  {
    ...
    if (PlainObjectType::Options & RowMajor) {
      index += indices[0];
      for (size_t i = 1; i < NumIndices; ++i) { index = index * dims[i] + indices[i]; }
    } else {
      index += indices[NumIndices - 1];
      for (int i = NumIndices - 2; i >= 0; --i) { index = index * dims[i] + indices[i]; }
    }
...
private:
  EIGEN_STRONG_INLINE void unrefEvaluator()
  {
    if (m_evaluator) {
      ...
      if (m_evaluator->refCount() == 0) { delete m_evaluator; }
    }
  }
...
template<typename Derived, typename Device> struct TensorEvaluator<const TensorRef<Derived>, Device>
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const TensorRef<Derived> &m, const Device &) : m_ref(m) {}
...
struct TensorEvaluator<TensorRef<Derived>, Device> : public TensorEvaluator<const TensorRef<Derived>, Device>
{
  ...
  enum { IsAligned = false, PacketAccess = false, RawAccess = false };
