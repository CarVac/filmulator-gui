...
namespace internal {
  ...
  template<typename Op, typename XprType> struct traits<TensorScanOp<Op, XprType>> : public traits<XprType>
  {
  ...
  template<typename Op, typename XprType> struct eval<TensorScanOp<Op, XprType>, Eigen::Dense>
  {
...
template<typename Self, typename Reducer, typename Device> struct ScanLauncher;

// Eval as rvalue
template<typename Op, typename ArgType, typename Device> struct TensorEvaluator<const TensorScanOp<Op, ArgType>, Device>
{
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType &op, const Device &device)
    : m_impl(op.expression(), device), m_device(device), m_exclusive(op.exclusive()), m_accumulator(op.accumulator()),
      m_size(m_impl.dimensions()[op.axis()]), m_stride(1), m_output(NULL)
  {
    ...
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      for (int i = 0; i < op.axis(); ++i) { m_stride = m_stride * dims[i]; }
    } else {
      for (int i = NumDims - 1; i > op.axis(); --i) { m_stride = m_stride * dims[i]; }
    }
  }
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType *data() const { return m_output; }
...
template<typename Self, typename Reducer> struct ScanLauncher<Self, Reducer, GpuDevice>
{
