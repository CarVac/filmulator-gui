...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorShufflingOp &operator=(const OtherDerived &other)
  {
...
struct TensorEvaluator<const TensorShufflingOp<Shuffle, ArgType>, Device>
{
    ...
    : m_impl(op.expression(), device)
  {
    ...
    for (int i = 0; i < NumDims; ++i) { m_dimensions[i] = input_dims[shuffle[i]]; }
    ...
    for (int i = 0; i < NumDims; ++i) { m_inputStrides[i] = inputStrides[shuffle[i]]; }
  }
  ...
  : public TensorEvaluator<const TensorShufflingOp<Shuffle, ArgType>, Device>
{
  ...
  enum { IsAligned = false, PacketAccess = (internal::packet_traits<Scalar>::size > 1), RawAccess = false };
