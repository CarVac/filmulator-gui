...
public:
  ...
  {
    if (NumIndices_ == 0) { m_data = internal::conditional_aligned_new_auto<T, (Options_ & DontAlign) == 0>(1); }
  }
  ...
  {
    internal::conditional_aligned_delete_auto<T, (Options_ & DontAlign) == 0>(
      m_data, internal::array_prod(m_dimensions));
  }
