...
namespace internal {
  ...
  template<typename Strides, typename XprType> struct eval<TensorStridingOp<Strides, XprType>, Eigen::Dense>
  {
...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorStridingOp &operator=(const OtherDerived &other)
  {
...
struct TensorEvaluator<const TensorStridingOp<Strides, ArgType>, Device>
{
  ...
  {
    double compute_cost =
      (NumDims - 1) * (TensorOpCost::AddCost<Index>() + TensorOpCost::MulCost<Index>() + TensorOpCost::DivCost<Index>())
