...
namespace TensorSycl {
  namespace internal {
    ...
    template<typename Expr> struct ConvertToDeviceExpression;

    template<template<class...> class NonOpCategory, bool IsConst, typename... Args> struct NonOpConversion
    {
    ...
    {
      typedef typename GetType<IsConst,
        NonOpCategory<typename ConvertToDeviceExpression<Args>::Type, MakeGlobalPointer>>::Type Type;
    };
...
#define KERNELBROKERCONVERT(CVQual, Res, ExprNode)                                               \
  ...
  struct ConvertToDeviceExpression<CVQual ExprNode<Expr>> : DeviceConvertor<ExprNode, Res, Expr> \
  {                                                                                              \
  };
...
#define KERNELBROKERCONVERTREDUCTION(CVQual)                                                                        \
  ...
  {                                                                                                                 \
    typedef CVQual TensorReductionOp<OP, Dim, typename ConvertToDeviceExpression<subExpr>::Type, MakeGlobalPointer> \
      Type;                                                                                                         \
  };
