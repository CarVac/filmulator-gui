...
namespace TensorSycl {
  namespace internal {
    ...
    template<typename PtrType, size_t N, typename... Params> struct EvalToLHSConstructor
    {
      PtrType expr;
    ...
    template<typename OrigExpr, typename IndexExpr, typename... Params> struct ExprConstructor;
...
#define TENSORMAP(CVQual)                                                                                            \
  ...
  struct ExprConstructor<                                                                                            \
    CVQual TensorMap<Tensor<Scalar_, NumIndices_, Options_, IndexType_>, Options2_, MakeGlobalPointer>,              \
    CVQual                                                                                                           \
      PlaceHolder<CVQual TensorMap<Tensor<Scalar_, NumIndices_, Options_, IndexType_>, Options3_, MakePointer_>, N>, \
    Params...>                                                                                                       \
  {                                                                                                                  \
...
#define UNARYCATEGORY(CVQual)                                                                                 \
  template<template<class, class> class UnaryCategory,                                                        \
    typename OP,                                                                                              \
    typename OrigRHSExpr,                                                                                     \
    typename RHSExpr,                                                                                         \
    typename... Params>                                                                                       \
...
#define BINARYCATEGORY(CVQual)                                                                             \
  template<template<class, class, class> class BinaryCategory,                                             \
    typename OP,                                                                                           \
    typename OrigLHSExpr,                                                                                  \
    typename OrigRHSExpr,                                                                                  \
    typename LHSExpr,                                                                                      \
    typename RHSExpr,                                                                                      \
    typename... Params>                                                                                    \
  struct ExprConstructor<CVQual BinaryCategory<OP, OrigLHSExpr, OrigRHSExpr>,                              \
    CVQual BinaryCategory<OP, LHSExpr, RHSExpr>,                                                           \
    Params...>                                                                                             \
  {                                                                                                        \
...
#define TERNARYCATEGORY(CVQual)                                                                                  \
  template<template<class, class, class, class> class TernaryCategory,                                           \
    typename OP,                                                                                                 \
    typename OrigArg1Expr,                                                                                       \
    typename OrigArg2Expr,                                                                                       \
    typename OrigArg3Expr,                                                                                       \
    typename Arg1Expr,                                                                                           \
    typename Arg2Expr,                                                                                           \
    typename Arg3Expr,                                                                                           \
    typename... Params>                                                                                          \
  struct ExprConstructor<CVQual TernaryCategory<OP, OrigArg1Expr, OrigArg2Expr, OrigArg3Expr>,                   \
    CVQual TernaryCategory<OP, Arg1Expr, Arg2Expr, Arg3Expr>,                                                    \
    Params...>                                                                                                   \
  {                                                                                                              \
    ...
    typedef CVQual                                                                                               \
      TernaryCategory<OP, typename my_arg1_type::Type, typename my_arg2_type::Type, typename my_arg3_type::Type> \
        Type;                                                                                                    \
    ...
    template<typename FuncDetector>                                                                              \
    ExprConstructor(FuncDetector &funcD, const utility::tuple::Tuple<Params...> &t)                              \
      : arg1Expr(funcD.arg1Expr, t), arg2Expr(funcD.arg2Expr, t), arg3Expr(funcD.arg3Expr, t),                   \
...
#define SELECTOP(CVQual)                                                                                               \
  template<typename OrigIfExpr,                                                                                        \
    typename OrigThenExpr,                                                                                             \
    typename OrigElseExpr,                                                                                             \
    typename IfExpr,                                                                                                   \
    typename ThenExpr,                                                                                                 \
    typename ElseExpr,                                                                                                 \
    typename... Params>                                                                                                \
  struct ExprConstructor<CVQual TensorSelectOp<OrigIfExpr, OrigThenExpr, OrigElseExpr>,                                \
    CVQual TensorSelectOp<IfExpr, ThenExpr, ElseExpr>,                                                                 \
    Params...>                                                                                                         \
  {                                                                                                                    \
    ...
    typedef CVQual TensorSelectOp<typename my_if_type::Type, typename my_then_type::Type, typename my_else_type::Type> \
      Type;                                                                                                            \
    my_if_type ifExpr;                                                                                                 \
    ...
    template<typename FuncDetector>                                                                                    \
    ExprConstructor(FuncDetector &funcD, const utility::tuple::Tuple<Params...> &t)                                    \
      : ifExpr(funcD.ifExpr, t), thenExpr(funcD.thenExpr, t), elseExpr(funcD.elseExpr, t),                             \
...
#define ASSIGN(CVQual)                                                                                         \
  ...
  struct ExprConstructor<CVQual TensorAssignOp<OrigLHSExpr, OrigRHSExpr>,                                      \
    CVQual TensorAssignOp<LHSExpr, RHSExpr>,                                                                   \
    Params...>                                                                                                 \
  {                                                                                                            \
    typedef ExprConstructor<OrigLHSExpr, LHSExpr, Params...> my_left_type;                                     \
    typedef ExprConstructor<OrigRHSExpr, RHSExpr, Params...> my_right_type;                                    \
...
#define FORCEDEVAL(CVQual)                                                                           \
  ...
  struct ExprConstructor<CVQual TensorForcedEvalOp<OrigExpr, MakeGlobalPointer>,                     \
    CVQual PlaceHolder<CVQual TensorForcedEvalOp<DevExpr>, N>,                                       \
    Params...>                                                                                       \
  {                                                                                                  \
    typedef CVQual TensorMap<Tensor<typename TensorForcedEvalOp<DevExpr, MakeGlobalPointer>::Scalar, \
                               TensorForcedEvalOp<DevExpr, MakeGlobalPointer>::NumDimensions,        \
                               0,                                                                    \
...
#define SYCLREDUCTIONEXPR(CVQual)                                                                            \
  ...
  struct ExprConstructor<CVQual TensorReductionOp<OP, Dim, OrigExpr, MakeGlobalPointer>,                     \
    CVQual PlaceHolder<CVQual TensorReductionOp<OP, Dim, DevExpr>, N>,                                       \
    Params...>                                                                                               \
  {                                                                                                          \
    static const size_t NumIndices =                                                                         \
      ValueCondition<TensorReductionOp<OP, Dim, DevExpr, MakeGlobalPointer>::NumDimensions == 0,             \
        1,                                                                                                   \
        TensorReductionOp<OP, Dim, DevExpr, MakeGlobalPointer>::NumDimensions>::Res;                         \
    typedef CVQual TensorMap<Tensor<typename TensorReductionOp<OP, Dim, DevExpr, MakeGlobalPointer>::Scalar, \
                               NumIndices,                                                                   \
                               0,                                                                            \
                               typename TensorReductionOp<OP, Dim, DevExpr>::Index>,                         \
      0,                                                                                                     \
      MakeGlobalPointer>                                                                                     \
      Type;                                                                                                  \
    Type expr;                                                                                               \
