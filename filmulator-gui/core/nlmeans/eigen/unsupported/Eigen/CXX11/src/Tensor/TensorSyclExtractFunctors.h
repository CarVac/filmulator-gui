...
namespace TensorSycl {
  namespace internal {
    ...
    {
      FunctorExtractor<TensorEvaluator<RHSExpr, Dev>> rhsExpr;
      FunctorExtractor(const TensorEvaluator<const TensorEvalToOp<RHSExpr>, Dev> &expr) : rhsExpr(expr.impl()) {}
    };
    ...
    {
      template<typename InDim> static inline Dim getDim(InDim dims) { return dims; }
    };

    template<typename Dim> struct DimConstr<Dim, 0>
    {
      template<typename InDim> static inline Dim getDim(InDim dims) { return Dim(dims.TotalSize()); }
    };
    ...
    struct FunctorExtractor<TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>, Device>>
    {
      ...
      typedef typename Eigen::internal::conditional<Evaluator::NumOutputDims == 0,
        DSizes<typename Evaluator::Index, 1>,
    ...
    template<typename Evaluator> auto inline extractFunctors(const Evaluator &evaluator) -> FunctorExtractor<Evaluator>
    {
