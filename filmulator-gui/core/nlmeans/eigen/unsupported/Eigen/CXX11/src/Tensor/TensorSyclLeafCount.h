...
namespace TensorSycl {
  namespace internal {
    ...
    template<typename Expr> struct LeafCount;
    ...
    template<typename Arg, typename... Args> struct CategoryCount<Arg, Args...>
    {
    ...
    // const TensorCwiseUnaryOp, const TensorCwiseNullaryOp, const TensorCwiseBinaryOp, const TensorCwiseTernaryOp, and
    ...
    template<typename Expr> struct LeafCount<const TensorForcedEvalOp<Expr>>
    {
      static const size_t Count = 1;
    };

    /// specialisation of the \ref LeafCount struct when the node type is TensorForcedEvalOp
    template<typename Expr> struct LeafCount<TensorForcedEvalOp<Expr>> : LeafCount<const TensorForcedEvalOp<Expr>>
    {
    };

    /// specialisation of the \ref LeafCount struct when the node type is const TensorEvalToOp
    template<typename Expr> struct LeafCount<const TensorEvalToOp<Expr>>
    {
    ...
    template<typename OP, typename Dim, typename Expr> struct LeafCount<const TensorReductionOp<OP, Dim, Expr>>
    {
      static const size_t Count = 1;
    };
    ...
    template<typename Expr> struct LeafCount<TensorEvalToOp<Expr>> : LeafCount<const TensorEvalToOp<Expr>>
    {
    };
