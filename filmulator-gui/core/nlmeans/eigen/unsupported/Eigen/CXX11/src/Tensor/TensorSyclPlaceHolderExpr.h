...
namespace TensorSycl {
  namespace internal {
    ...
    template<typename Scalar, size_t N> struct PlaceHolder
    {
    ...
    template<typename Expr, size_t N> struct PlaceHolderExpression;

    template<size_t N, typename... Args> struct CalculateIndex;

    template<size_t N, typename Arg> struct CalculateIndex<N, Arg>
    {
      typedef typename PlaceHolderExpression<Arg, N>::Type ArgType;
      typedef utility::tuple::Tuple<ArgType> ArgsTuple;
    };

    template<size_t N, typename Arg1, typename Arg2> struct CalculateIndex<N, Arg1, Arg2>
    {
    ...
    template<size_t N, typename Arg1, typename Arg2, typename Arg3> struct CalculateIndex<N, Arg1, Arg2, Arg3>
    {
    ...
    template<template<class...> class Category, class OP, class TPL> struct CategoryHelper;
...
#define OPEXPRCATEGORY(CVQual)                                                                             \
  ...
  {                                                                                                        \
    typedef CVQual                                                                                         \
      typename CategoryHelper<Category, OP, typename CalculateIndex<N, SubExpr...>::ArgsTuple>::Type Type; \
  };
...
#define SELECTEXPR(CVQual)                                                           \
  ...
  {                                                                                  \
    typedef CVQual typename CategoryHelper<TensorSelectOp,                           \
      NoOP,                                                                          \
      typename CalculateIndex<N, IfExpr, ThenExpr, ElseExpr>::ArgsTuple>::Type Type; \
  };
...
#define ASSIGNEXPR(CVQual)                                                                                         \
  ...
  {                                                                                                                \
    typedef CVQual                                                                                                 \
      typename CategoryHelper<TensorAssignOp, NoOP, typename CalculateIndex<N, LHSExpr, RHSExpr>::ArgsTuple>::Type \
        Type;                                                                                                      \
  };
...
#define TENSORMAPEXPR(CVQual)                                                                                       \
  ...
  struct PlaceHolderExpression<                                                                                     \
    CVQual TensorMap<Tensor<Scalar_, NumIndices_, Options_, IndexType_>, Options2_, MakePointer_>,                  \
    N>                                                                                                              \
  {                                                                                                                 \
    typedef CVQual                                                                                                  \
      PlaceHolder<CVQual TensorMap<Tensor<Scalar_, NumIndices_, Options_, IndexType_>, Options2_, MakePointer_>, N> \
        Type;                                                                                                       \
  };
...
#define FORCEDEVAL(CVQual)                                                                           \
  template<typename Expr, size_t N> struct PlaceHolderExpression<CVQual TensorForcedEvalOp<Expr>, N> \
  {                                                                                                  \
...
#define EVALTO(CVQual)                                                                           \
  template<typename Expr, size_t N> struct PlaceHolderExpression<CVQual TensorEvalToOp<Expr>, N> \
  {                                                                                              \
...
#undef SYCLREDUCTION
    ...
    template<typename Expr> struct createPlaceHolderExpression
    {
