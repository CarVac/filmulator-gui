...
namespace tuple {
  ...
  template<typename T> struct StaticIf<true, T>
  {
    typedef T type;
  };
  ...
  template<size_t k, class T, class... Ts> struct ElemTypeHolder<k, Tuple<T, Ts...>>
  {
...
#define TERMINATE_CONDS_TUPLE_GET(CVQual)                                                                              \
  ...
  typename StaticIf<k == 0, CVQual typename ElemTypeHolder<0, Tuple<Ts...>>::type &>::type get(CVQual Tuple<Ts...> &t) \
  {                                                                                                                    \
...
#define RECURSIVE_TUPLE_GET(CVQual)                                                                \
  ...
  typename StaticIf<k != 0, CVQual typename ElemTypeHolder<k, Tuple<T, Ts...>>::type &>::type get( \
    CVQual Tuple<T, Ts...> &t)                                                                     \
  {                                                                                                \
...
#undef RECURSIVE_TUPLE_GET
  ...
  /// \return Tuple<Args...>
  template<typename... Args> Tuple<Args...> make_tuple(Args... args) { return Tuple<Args...>(args...); }
  ...
  /// \tparam Args the type of the arguments to construct the tuple from
  /// \return size_t
  template<typename... Args> static constexpr size_t size(Tuple<Args...> &) { return sizeof...(Args); }
...
// FIXME Doxygen has problems with recursive inheritance
  ...
  /// \tparam N represents sizeof..(elemens)- sizeof...(Is)
  /// \tparam Is... are the list of generated index so far
  template<size_t MIN, size_t N, size_t... Is> struct RangeBuilder : public RangeBuilder<MIN, N - 1, N - 1, Is...>
  {
  };
  ...
  template<size_t MIN, size_t MAX> struct IndexRange : RangeBuilder<MIN, MAX>::type
  {
  };
  ...
  /// \param a the new elements going to be added to the tuple
  /// \return Tuple<Args..., T>
  template<typename... Args, typename T> Tuple<Args..., T> append(Tuple<Args...> t, T a)
  {
