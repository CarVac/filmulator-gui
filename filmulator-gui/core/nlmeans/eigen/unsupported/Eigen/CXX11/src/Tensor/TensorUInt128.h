...
namespace internal {
  ...
  template<uint64_t n> struct static_val
  {
    ...
    template<typename T> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static_val(const T &v) { eigen_assert(v == n); }
  };


  template<typename HIGH = uint64_t, typename LOW = uint64_t> struct TensorUInt128
  {
    HIGH high;
    LOW low;
    ...
    template<typename T> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE explicit TensorUInt128(const T &x) : high(0), low(x)
    {
    ...
    const TensorUInt128<HR, LR> &rhs)
  {
    ...
    if (result.low < rhs.low) { result.high += 1; }
    return result;
  }
  ...
  {
    TensorUInt128<uint64_t, uint64_t> result(lhs.high - rhs.high, lhs.low - rhs.low);
    if (result.low > lhs.low) { result.high -= 1; }
    return result;
  }
