...
public:
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorVolumePatchOp(const XprType &expr,
    ...
    : m_xpr(expr), m_patch_planes(patch_planes), m_patch_rows(patch_rows), m_patch_cols(patch_cols),
      ...
      m_col_inflate_strides(col_inflate_strides), m_padding_explicit(false), m_padding_top_z(0), m_padding_bottom_z(0),
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorVolumePatchOp(const XprType &expr,
    ...
    : m_xpr(expr), m_patch_planes(patch_planes), m_patch_rows(patch_rows), m_patch_cols(patch_cols),
      ...
      m_in_plane_strides(in_plane_strides), m_in_row_strides(in_row_strides), m_in_col_strides(in_col_strides),
      m_plane_inflate_strides(plane_inflate_strides), m_row_inflate_strides(row_inflate_strides),
      m_col_inflate_strides(col_inflate_strides), m_padding_explicit(true), m_padding_top_z(padding_top_z),
      m_padding_bottom_z(padding_bottom_z), m_padding_top(padding_top), m_padding_bottom(padding_bottom),
      m_padding_left(padding_left), m_padding_right(padding_right), m_padding_type(PADDING_VALID),
      m_padding_value(padding_value)
  {}
...
struct TensorEvaluator<const TensorVolumePatchOp<Planes, Rows, Cols, ArgType>, Device>
{
  ...
  template<int LoadMode> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE PacketReturnType packet(Index index) const
  {
    ...
    if (inputCols[0] != inputCols[1]) { return packetWithPossibleZero(index); }
    ...
    if (inputRows[0] != inputRows[1]) { return packetWithPossibleZero(index); }
