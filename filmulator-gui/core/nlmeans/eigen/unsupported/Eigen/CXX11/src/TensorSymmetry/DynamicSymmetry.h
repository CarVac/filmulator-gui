...
public:
  ...
  inline DynamicSGroup(const DynamicSGroup &o)
    : m_numIndices(o.m_numIndices), m_elements(o.m_elements), m_generators(o.m_generators),
  ...
  template<typename Gen_> inline void add(Gen_) { add(Gen_::One, Gen_::Two, Gen_::Flags); }
  ...
  inline RV apply(const std::array<Index, N> &idx, RV initial, Args &&...args) const
  {
    ...
    for (std::size_t i = 0; i < size(); i++)
      initial = Op::run(h_permute(i, idx, typename internal::gen_numeric_list<int, N>::type()),
        m_elements[i].flags,
        initial,
  ...
  inline internal::tensor_symmetry_value_setter<Tensor_, DynamicSGroup>
    operator()(Tensor_ &tensor, typename Tensor_::Index firstIndex, IndexTypes... otherIndices) const
  {
    ...
    return operator()(
      tensor, std::array<typename Tensor_::Index, Tensor_::NumIndices>{ { firstIndex, otherIndices... } });
  }

  template<typename Tensor_>
  inline internal::tensor_symmetry_value_setter<Tensor_, DynamicSGroup> operator()(Tensor_ &tensor,
    std::array<typename Tensor_::Index, Tensor_::NumIndices> const &indices) const
  {
...
private:
  ...
  inline std::array<Index, N>
    h_permute(std::size_t which, const std::array<Index, N> &idx, internal::numeric_list<int, n...>) const
  {
    return std::array<Index, N>{ { idx[n >= m_numIndices ? n : m_elements[which].representation[n]]... } };
  }

  template<typename Index> inline std::vector<Index> h_permute(std::size_t which, std::vector<Index> idx) const
  {
...
template<typename... Gen> class DynamicSGroupFromTemplateArgs : public DynamicSGroup
{
public:
...
private:
  template<typename Gen1, typename... GenNext> inline void add_all(internal::type_list<Gen1, GenNext...>)
  {
    add(Gen1());
