...
namespace internal {
  ...
  template<int... nn> struct tensor_static_symgroup_permutate<numeric_list<int, nn...>>
  {
    ...
    template<typename T> constexpr static inline std::array<T, N> run(const std::array<T, N> &indices)
    {
      return { { indices[nn]... } };
    }
  };
  ...
  template<typename first, typename... next> struct tensor_static_symgroup_do_apply<internal::type_list<first, next...>>
  {
    ...
    static inline RV run(const std::vector<Index> &idx, RV initial, Args &&...args)
    {
      eigen_assert(idx.size() >= SGNumIndices
                   && "Can only apply symmetry group to objects that have at least the required amount of indices.");
      initial = Op::run(tensor_static_symgroup_index_permute(idx, typename first::indices()),
        first::flags,
        initial,
        std::forward<Args>(args)...);
      return tensor_static_symgroup_do_apply<internal::type_list<next...>>::template run<Op, RV, SGNumIndices>(
...
template<typename... Gen> class StaticSGroup
{
  ...
  typedef internal::group_theory::enumerate_group_elements<internal::tensor_static_symgroup_multiply,
...
public:
  ...
  inline internal::tensor_symmetry_value_setter<Tensor_, StaticSGroup<Gen...>> operator()(Tensor_ &tensor,
