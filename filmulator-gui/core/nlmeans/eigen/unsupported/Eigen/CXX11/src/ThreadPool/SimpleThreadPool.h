...
template<typename Environment> class SimpleThreadPoolTempl : public ThreadPoolInterface
{
public:
  ...
  {
    {
      ...
      while (!pending_.empty()) { empty_.wait(l); }
      exiting_ = true;
  ...
  void WorkerLoop(int thread_id)
  {
    ...
    while (!exiting_) {
      if (pending_.empty()) {
        ...
        pending_.pop_front();
        if (pending_.empty()) { empty_.notify_all(); }
      }
      if (t.f) {
        mu_.unlock();
        env_.ExecuteTask(t);
