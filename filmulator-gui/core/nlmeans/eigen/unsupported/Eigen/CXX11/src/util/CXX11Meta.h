...
namespace internal {
  ...
  {
    constexpr static std::size_t count = sizeof...(nn) + 1;
    constexpr static T first_value = n;
  };
  ...
  template<int n> struct h_skip
  {
    ...
    constexpr static inline typename h_skip_helper_numeric<T, n, ii...>::type helper(numeric_list<T, ii...>)
    {
      return typename h_skip_helper_numeric<T, n, ii...>::type();
    }
    template<typename... tt>
    constexpr static inline typename h_skip_helper_type<n, tt...>::type helper(type_list<tt...>)
    {
  ...
  struct sum_op
  {
    template<typename A, typename B> EIGEN_DEVICE_FUNC constexpr static inline auto run(A a, B b) -> decltype(a + b)
    {
      return a + b;
    }
  ...
  {
    template<typename A> constexpr static inline auto run(A a) -> decltype(a >= 0) { return a >= 0; }
  };
  ...
  // does...
  template<typename... Ts> constexpr inline decltype(reduce<product_op, Ts...>::run((*((Ts *)0))...)) arg_prod(Ts... ts)
  {
    return reduce<product_op, Ts...>::run(ts...);
  }

  template<typename... Ts> constexpr inline decltype(reduce<sum_op, Ts...>::run((*((Ts *)0))...)) arg_sum(Ts... ts)
  {
  ...
  template<typename Array, int... n> constexpr inline Array h_array_reverse(Array arr, numeric_list<int, n...>)
  {
    return { { array_get<sizeof...(n) - n - 1>(arr)... } };
  }

  template<typename T, std::size_t N> constexpr inline array<T, N> array_reverse(array<T, N> arr)
  {
  ...
  {
    EIGEN_DEVICE_FUNC constexpr static inline auto run(array<T, N> arr, T identity)
  ...
  {
    EIGEN_DEVICE_FUNC constexpr static inline T run(const array<T, N> &arr, T) { return array_get<0>(arr); }
  };

  template<typename Reducer, typename T> struct h_array_reduce<Reducer, T, 0>
  {
    EIGEN_DEVICE_FUNC constexpr static inline T run(const array<T, 0> &, T identity) { return identity; }
  };

  template<typename Reducer, typename T, std::size_t N>
  EIGEN_DEVICE_FUNC constexpr inline auto array_reduce(const array<T, N> &arr, T identity)
  ...
  EIGEN_DEVICE_FUNC constexpr inline auto array_sum(const array<T, N> &arr)
  ...
  template<typename T, std::size_t N>
  EIGEN_DEVICE_FUNC constexpr inline auto array_prod(const array<T, N> &arr)
  ...
  constexpr inline array<decltype(Op::run(A(), B())), N>
  ...
  constexpr inline auto h_array_zip_and_reduce(array<A, N> a, array<B, N> b, numeric_list<int, n...>)
  ...
  constexpr inline auto array_zip_and_reduce(array<A, N> a, array<B, N> b)
  ...
  constexpr inline auto h_array_apply_and_reduce(array<A, N> arr, numeric_list<int, n...>)
  ...
  constexpr inline auto array_apply_and_reduce(array<A, N> a)
  ...
  {
    template<typename t, int... ii> constexpr static inline array<t, n> run(t v, numeric_list<int, ii...>)
    {
      return { { typename id_numeric<int, ii, t>::type(v)... } };
    }
  };

  template<int n, typename t> constexpr array<t, n> repeat(t v)
  {
