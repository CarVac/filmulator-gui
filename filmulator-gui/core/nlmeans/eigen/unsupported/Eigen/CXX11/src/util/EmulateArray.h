...
#if EIGEN_HAS_VARIADIC_TEMPLATES
  EIGEN_DEVICE_FUNC array(std::initializer_list<T> l) : dummy() { eigen_assert(l.size() == 0); }
#endif
...
namespace internal {
  template<std::size_t I, class T, std::size_t N> EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T &array_get(array<T, N> &a)
  {
    return a[I];
  }
...
#define STD_GET_ARR_HACK std::template get<I, T, N>(a)
  ...
  template<std::size_t I, class T, std::size_t N> constexpr inline T &array_get(std::array<T, N> &a)
  {
  ...
  template<std::size_t I, class T, std::size_t N> constexpr inline T const &array_get(std::array<T, N> const &a)
  {
