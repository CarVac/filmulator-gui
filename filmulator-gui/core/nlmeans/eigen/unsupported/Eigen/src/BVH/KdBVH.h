...
public:
   ...
   * a Volume. */
  template<typename Iter> KdBVH(Iter begin, Iter end)
  {
    init(begin, end, 0, 0);
   ...
   * BVH */
  template<typename OIter, typename BIter> KdBVH(OIter begin, OIter end, BIter boxBegin, BIter boxEnd)
  {
...
private:
  ...
  std::vector<int>
    children;// children of x are children[2x] and children[2x+1], indices bigger than boxes.size() index into objects.
  VolumeList boxes;
  ObjectList objects;
};
