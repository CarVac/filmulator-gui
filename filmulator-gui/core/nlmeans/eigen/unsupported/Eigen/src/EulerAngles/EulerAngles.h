...
public:
   ...
   */
  template<typename Derived> EulerAngles(const MatrixBase<Derived> &m) { *this = m; }
   ...
   */
  template<typename Derived> EulerAngles(const RotationBase<Derived, 3> &rot) { *this = rot; }
  ...
  static EulerAngles FromRotation(const MatrixBase<Derived> &m)
  {
    ...
    EulerAngles e;
    System::template CalcEulerAngles<PositiveRangeAlpha, PositiveRangeBeta, PositiveRangeGamma, _Scalar>(e, m);
    return e;
  }
  ...
  template<typename Derived> EulerAngles &operator=(const MatrixBase<Derived> &m)
  {
    EIGEN_STATIC_ASSERT_MATRIX_SPECIFIC_SIZE(Derived, 3, 3)
  ...
  template<typename Derived> EulerAngles &operator=(const RotationBase<Derived, 3> &rot)
  {
  ...
  Matrix3 toRotationMatrix() const { return static_cast<QuaternionType>(*this).toRotationMatrix(); }
...
namespace internal {
  template<typename _Scalar, class _System> struct traits<EulerAngles<_Scalar, _System>>
  {
    typedef _Scalar Scalar;
  };
}// namespace internal
