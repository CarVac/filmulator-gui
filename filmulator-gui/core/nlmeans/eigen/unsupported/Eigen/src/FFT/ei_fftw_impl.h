...
namespace internal {
  ...
  template<typename T> inline T *fftw_cast(const T *p) { return const_cast<T *>(p); }
  ...
  template<typename T> struct fftw_plan
  {
  };

  template<> struct fftw_plan<float>
  {
    ...
    {
      if (m_plan == NULL) m_plan = fftwf_plan_dft_c2r_1d(nfft, src, dst, FFTW_ESTIMATE | FFTW_PRESERVE_INPUT);
  ...
  template<> struct fftw_plan<double>
  {
    ...
    inline void inv(scalar_type *dst, complex_type *src, int nfft)
    {
      if (m_plan == NULL) m_plan = fftw_plan_dft_c2r_1d(nfft, src, dst, FFTW_ESTIMATE | FFTW_PRESERVE_INPUT);
  ...
  template<> struct fftw_plan<long double>
  {
    ...
    inline void inv(scalar_type *dst, complex_type *src, int nfft)
    {
      if (m_plan == NULL) m_plan = fftwl_plan_dft_c2r_1d(nfft, src, dst, FFTW_ESTIMATE | FFTW_PRESERVE_INPUT);
  ...
  template<typename _Scalar> struct fftw_impl
  {
