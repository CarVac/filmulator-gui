...
namespace internal {
  ...
  template<typename _MatrixType, typename _Preconditioner> struct traits<DGMRES<_MatrixType, _Preconditioner>>
  {
...
public:
  ...
  explicit DGMRES(const EigenBase<MatrixDerived> &A)
    : Base(A.derived()), m_restart(30), m_neig(0), m_r(0), m_maxNeig(5), m_isDeflAllocated(false),
  ...
  /** \internal */
  template<typename Rhs, typename Dest> void _solve_with_guess_impl(const Rhs &b, Dest &x) const
  {
    bool failed = false;
  ...
  /** \internal */
  template<typename Rhs, typename Dest> void _solve_impl(const Rhs &b, MatrixBase<Dest> &x) const
  {
    x = b;
...
protected:
  ...
  Index dgmresCycle(const MatrixType &mat,
  ...
  Index dgmresComputeDeflationData(const MatrixType &mat,
    const Preconditioner &precond,
    const Index &it,
    StorageIndex &neig) const;
  // Apply deflation to a vector
  template<typename RhsType, typename DestType> Index dgmresApplyDeflation(const RhsType &In, DestType &Out) const;
...
template<typename _MatrixType, typename _Preconditioner>
template<typename Rhs, typename Dest>
void DGMRES<_MatrixType, _Preconditioner>::dgmres(const MatrixType &mat,
  const Rhs &rhs,
  Dest &x,
  const Preconditioner &precond) const
{
  ...
  while (nbIts < m_iterations && m_info == NoConvergence) {
    ...
    if (nbIts < m_iterations && m_info == NoConvergence) r0 = rhs - mat * x;
  }
}
...
template<typename _MatrixType, typename _Preconditioner>
template<typename Dest>
Index DGMRES<_MatrixType, _Preconditioner>::dgmresCycle(const MatrixType &mat,
  const Preconditioner &precond,
  Dest &x,
  DenseVector &r0,
  RealScalar &beta,
  ...
  Index &nbIts) const
{
  ...
  while (m_info == NoConvergence && it < m_restart && nbIts < m_iterations) {
    ...
    for (Index i = 1; i <= it; ++i) { m_H.col(it).applyOnTheLeft(i - 1, i, gr[i - 1].adjoint()); }
...
template<typename _MatrixType, typename _Preconditioner>
Index DGMRES<_MatrixType, _Preconditioner>::dgmresComputeDeflationData(const MatrixType &mat,
  const Preconditioner &precond,
  const Index &it,
  StorageIndex &neig) const
{
  // First, find the Schur form of the Hessenberg matrix H
  typename internal::conditional<NumTraits<Scalar>::IsComplex, ComplexSchur<DenseMatrix>, RealSchur<DenseMatrix>>::type
    schurofH;
  bool computeU = true;
  ...
  if (!m_lambdaN) { m_lambdaN = (std::max)(modulEig.maxCoeff(), m_lambdaN); }
  ...
  Sr.setZero();
  for (Index j = 0; j < nbrEig; j++) { Sr.col(j) = schurofH.matrixU().col(perm(it - j - 1)); }
