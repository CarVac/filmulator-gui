...
template<typename _Scalar> class IncompleteLU : public SparseSolverBase<IncompleteLU<_Scalar>>
{
protected:
...
public:
  ...
  template<typename MatrixType> IncompleteLU(const MatrixType &mat) { compute(mat); }

  Index rows() const { return m_lu.rows(); }
  Index cols() const { return m_lu.cols(); }

  template<typename MatrixType> IncompleteLU &compute(const MatrixType &mat)
  {
    m_lu = mat;
    int size = mat.cols();
    Vector diag(size);
