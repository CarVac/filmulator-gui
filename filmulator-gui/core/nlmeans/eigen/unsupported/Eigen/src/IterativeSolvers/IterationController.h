...
public:
  ...
  void setCallback(void (*t)(const IterationController &)) { m_callback = t; }
  ...
  template<typename VectorType> bool converged(const VectorType &v) { return converged(v.squaredNorm()); }
  ...
  template<typename VectorType> bool finished(const MatrixBase<VectorType> &v)
  {
