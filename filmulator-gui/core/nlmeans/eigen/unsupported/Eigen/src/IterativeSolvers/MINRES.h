...
namespace internal {
  ...
  EIGEN_DONT_INLINE void minres(const MatrixType &mat,
    const Rhs &rhs,
    Dest &x,
    ...
    typename Dest::RealScalar &tol_error)
  {
    ...
    while (iters < maxIters) {
      ...
      if (residualNorm2 < threshold2) { break; }
...
public:
  ...
  template<typename Rhs, typename Dest> void _solve_with_guess_impl(const Rhs &b, Dest &x) const
  {
    ...
    typedef
      typename internal::conditional<TransposeInput, Transpose<const ActualMatrixType>, ActualMatrixType const &>::type
        RowMajorWrapper;
    ...
    for (int j = 0; j < b.cols(); ++j) {
      ...
      internal::minres(SelfAdjointWrapper(row_mat), b.col(j), xj, Base::m_preconditioner, m_iterations, m_error);
    }
