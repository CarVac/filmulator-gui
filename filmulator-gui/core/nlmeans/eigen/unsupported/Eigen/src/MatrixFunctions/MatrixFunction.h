...
namespace internal {
  ...
  void matrix_function_compute_map(const EivalsType &eivals, const ListOfClusters &clusters, VectorType &eivalToCluster)
  {
    ...
    for (typename ListOfClusters::const_iterator cluster = clusters.begin(); cluster != clusters.end(); ++cluster) {
      for (Index i = 0; i < eivals.rows(); ++i) {
        if (std::find(cluster->begin(), cluster->end(), i) != cluster->end()) { eivalToCluster[i] = clusterIndex; }
      }
      ++clusterIndex;
    }
  }
  ...
  void matrix_function_compute_permutation(const DynVectorType &blockStart,
  ...
  void matrix_function_compute_block_atomic(const MatrixType &T,
    AtomicType &atomic,
  ...
  void matrix_function_compute_above_diagonal(const MatrixType &T,
    const VectorType &blockStart,
    const VectorType &clusterSize,
    MatrixType &fT)
  {
