...
namespace internal {
  ...
  template<typename MatrixType> void matrix_log_compute_2x2(const MatrixType &A, MatrixType &result)
  {
    ...
    if (y == Scalar(0)) {
      ...
      int unwindingNumber =
        static_cast<int>(ceil((imag(logA11 - logA00) - RealScalar(EIGEN_PI)) / RealScalar(2 * EIGEN_PI)));
  ...
  {
    const float maxNormForPade[] = {
      2.5111573934555054e-1 /* degree = 3 */, 4.0535837411880493e-1, 5.3149729967117310e-1
    };
