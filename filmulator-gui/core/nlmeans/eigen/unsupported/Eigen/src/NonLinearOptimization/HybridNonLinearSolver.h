 ...
 *
 */
template<typename FunctorType, typename Scalar = double> class HybridNonLinearSolver
{
public:
  ...
  {
    Parameters()
      : factor(Scalar(100.)), maxfev(1000), xtol(std::sqrt(NumTraits<Scalar>::epsilon())), nb_of_subdiagonals(-1),
        nb_of_superdiagonals(-1), epsfcn(Scalar(0.))
    {}
    Scalar factor;
...
HybridNonLinearSolverSpace::Status HybridNonLinearSolver<FunctorType, Scalar>::solveOneStep(FVectorType &x)
{
  ...
  while (true) {
    ...
    if (ratio < Scalar(.1)) {
      ...
      if (abs(ratio - 1.) <= Scalar(.1)) { delta = pnorm / Scalar(.5); }
    }
