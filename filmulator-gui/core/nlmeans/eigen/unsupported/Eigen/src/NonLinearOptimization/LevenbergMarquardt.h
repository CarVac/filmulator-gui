 ...
 */
template<typename FunctorType, typename Scalar = double> class LevenbergMarquardt
{
...
public:
  ...
  {
    Parameters()
      : factor(Scalar(100.)), maxfev(400), ftol(sqrt_epsilon()), xtol(sqrt_epsilon()), gtol(Scalar(0.)),
        epsfcn(Scalar(0.))
    {}
    Scalar factor;
  ...
  static LevenbergMarquardtSpace::Status
    lmdif1(FunctorType &functor, FVectorType &x, Index *nfev, const Scalar tol = sqrt_epsilon());
...
LevenbergMarquardtSpace::Status LevenbergMarquardt<FunctorType, Scalar>::minimizeInit(FVectorType &x)
{
  ...
  /*     check the input parameters for errors. */
  if (n <= 0 || m < n || parameters.ftol < 0. || parameters.xtol < 0. || parameters.gtol < 0. || parameters.maxfev <= 0
      || parameters.factor <= 0.)
  ...
  if (fnorm != 0.)
    for (Index j = 0; j < n; ++j)
      if (wa2[permutation.indices()[j]] != 0.)
        gnorm =
          (std::max)(gnorm, abs(fjac.col(j).head(j + 1).dot(qtf.head(j + 1) / fnorm) / wa2[permutation.indices()[j]]));
