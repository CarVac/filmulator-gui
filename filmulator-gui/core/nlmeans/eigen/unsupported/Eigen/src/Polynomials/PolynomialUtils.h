 ...
 *   \f$ |x| \le 1 \f$
 */
template<typename Polynomials, typename T> inline T poly_eval_horner(const Polynomials &poly, const T &x)
{
  T val = poly[poly.size() - 1];
  for (DenseIndex i = poly.size() - 2; i >= 0; --i) { val = val * x + poly[i]; }
  return val;
}
 ...
 * \param[in] x : the value to evaluate the polynomial at.
 */
template<typename Polynomials, typename T> inline T poly_eval(const Polynomials &poly, const T &x)
{
  ...
  if (numext::abs2(x) <= Real(1)) {
    ...
    T val = poly[0];
    T inv_x = T(1) / x;
    for (DenseIndex i = 1; i < poly.size(); ++i) { val = val * inv_x + poly[i]; }
...
inline typename NumTraits<typename Polynomial::Scalar>::Real cauchy_max_bound(const Polynomial &poly)
{
  ...
  for (DenseIndex i = 0; i < poly.size() - 1; ++i) { cb += abs(poly[i] * inv_leading_coeff); }
  return cb + Real(1);
}
...
inline typename NumTraits<typename Polynomial::Scalar>::Real cauchy_min_bound(const Polynomial &poly)
{
  ...
  if (poly.size() - 1 == i) { return Real(1); }

  const Scalar inv_min_coeff = Scalar(1) / poly[i];
  Real cb(1);
  for (DenseIndex j = i + 1; j < poly.size(); ++j) { cb += abs(poly[j] * inv_min_coeff); }
  return Real(1) / cb;
}
