 ...
 *
 */
template<typename MatrixType> class SkylineInplaceLU
{
protected:
...
void SkylineInplaceLU<MatrixType>::compute()
{
  ...
  for (Index row = 0; row < rows; row++) {
    ...
    const Index &col = row;
    for (typename MatrixType::InnerLowerIterator lIt(m_lu, col); lIt; ++lIt) { lIt.valueRef() /= pivot; }
...
template<typename MatrixType> void SkylineInplaceLU<MatrixType>::computeRowMajor()
{
  const size_t rows = m_lu.rows();
  const size_t cols = m_lu.cols();
