 ...
 */
namespace internal {
  template<typename _Scalar, int _Options> struct traits<SkylineMatrix<_Scalar, _Options>>
  {
...
public:
    ...
    if (IsRowMajor) {
      if (outer < inner)// upper matrix
      {
        ...
        if (outer < minOuterIndex)// The value does not yet exist
        {
          ...
          for (Index innerIdx = cols(); innerIdx > inner; innerIdx--) { m_colStartIndex[innerIdx] += bandIncrement; }
      ...
      if (outer > inner)// lower matrix
      {
        ...
        if (inner < minInnerIndex)// The value does not yet exist
        {
          ...
          for (Index innerIdx = rows(); innerIdx > outer; innerIdx--) { m_rowStartIndex[innerIdx] += bandIncrement; }
      ...
      if (outer > inner)// upper matrix
      {
        ...
        if (outer > maxOuterIndex)// The value does not yet exist
        {
          ...
          memset(
            this->_upperPtr() + m_rowStartIndex[inner] + previousProfile + 1, 0, (bandIncrement - 1) * sizeof(Scalar));
      ...
      if (outer < inner)// lower matrix
      {
        ...
        if (inner > maxInnerIndex)// The value does not yet exist
        {
          ...
          memset(
            this->_lowerPtr() + m_colStartIndex[outer] + previousProfile + 1, 0, (bandIncrement - 1) * sizeof(Scalar));
  ...
  template<typename OtherDerived> inline SkylineMatrix &operator=(const SkylineMatrixBase<OtherDerived> &other)
  {
  ...
  friend std::ostream &operator<<(std::ostream &s, const SkylineMatrix &m)
  {
    ...
    EIGEN_DBG_SKYLINE(std::cout << "upper elements : " << std::endl;
    ...
    for (Index rowIdx = 0; rowIdx < m.rows(); rowIdx++) {
      for (Index colIdx = 0; colIdx < m.cols(); colIdx++) { s << m.coeff(rowIdx, colIdx) << "\t"; }
      s << std::endl;
    }
    return s;
  }
...
template<typename Scalar, int _Options> class SkylineMatrix<Scalar, _Options>::InnerUpperIterator
{
public:
  InnerUpperIterator(const SkylineMatrix &mat, Index outer)
    : m_matrix(mat), m_outer(outer),
      m_id(_Options == RowMajor ? mat.m_colStartIndex[outer] : mat.m_rowStartIndex[outer] + 1), m_start(m_id),
...
template<typename Scalar, int _Options> class SkylineMatrix<Scalar, _Options>::InnerLowerIterator
{
public:
  InnerLowerIterator(const SkylineMatrix &mat, Index outer)
    : m_matrix(mat), m_outer(outer),
      m_id(_Options == RowMajor ? mat.m_rowStartIndex[outer] : mat.m_colStartIndex[outer] + 1), m_start(m_id),
