 ...
 */
template<typename _Scalar, int _BlockAtCompileTime = Dynamic, int _Options = ColMajor, typename _StorageIndex = int>
class BlockSparseMatrix;
...
namespace internal {
  ...
  template<typename BlockSparseMatrixT> struct traits<BlockSparseMatrixView<BlockSparseMatrixT>>
  {
    typedef Ref<
  ...
  template<typename Iterator, bool IsColMajor> struct TripletComp
  {
...
template<typename BlockSparseMatrixT> class BlockSparseMatrixView : public SparseMatrixBase<BlockSparseMatrixT>
{
public:
...
template<typename BlockSparseMatrixT, typename VectorType> class BlockVectorView
{
public:
  enum {
  ...
  typedef Ref<const Matrix<typename BlockSparseMatrixT::Scalar,
    (RowsAtCompileTime == 1) ? 1 : BlockSize,
...
// Proxy to view a regular vector as a block vector
template<typename BlockSparseMatrixT, typename VectorType> class BlockVectorReturn
{
public:
  enum {
    ColsAtCompileTime = VectorType::ColsAtCompileTime,
...
template<typename Lhs, typename Rhs> class BlockSparseTimeDenseProduct;
...
public:
  ...
  template<typename Dest> void scaleAndAddTo(Dest &dest, const typename Rhs::Scalar &alpha) const
  {
    ...
    internal::sparse_time_dense_product(
      BlockSparseMatrixView<Lhs>(m_lhs), BlockVectorView<Lhs, Rhs>(m_lhs, m_rhs), tmpDest, alpha);
  }
...
public:
  ...
  typedef
    typename internal::ref_selector<BlockSparseMatrix<_Scalar, _BlockAtCompileTime, _Options, _StorageIndex>>::type
      Nested;
  ...
  typedef Matrix<RealScalar, _BlockAtCompileTime, _BlockAtCompileTime, IsColMajor ? ColMajor : RowMajor>
    BlockRealScalar;
  typedef
    typename internal::conditional<_BlockAtCompileTime == Dynamic, Scalar, BlockScalar>::type BlockScalarReturnType;
  typedef BlockSparseMatrix<Scalar, BlockSize, IsColMajor ? ColMajor : RowMajor, StorageIndex> PlainObject;

public:
  // Default constructor
  BlockSparseMatrix()
    : m_innerBSize(0), m_outerBSize(0), m_innerOffset(0), m_outerOffset(0), m_nonzerosblocks(0), m_values(0),
      m_blockPtr(0), m_indices(0), m_outerIndex(0), m_blockSize(BlockSize)
  {}
  ...
  BlockSparseMatrix(Index brow, Index bcol)
    : m_innerBSize(IsColMajor ? brow : bcol), m_outerBSize(IsColMajor ? bcol : brow), m_innerOffset(0),
      m_outerOffset(0), m_nonzerosblocks(0), m_values(0), m_blockPtr(0), m_indices(0), m_outerIndex(0),
  ...
  BlockSparseMatrix(const BlockSparseMatrix &other)
    : m_innerBSize(other.m_innerBSize), m_outerBSize(other.m_outerBSize), m_nonzerosblocks(other.m_nonzerosblocks),
      m_nonzeros(other.m_nonzeros), m_blockPtr(0), m_blockSize(other.m_blockSize)
  {
   ...
   *
   */
  template<typename MatrixType> inline BlockSparseMatrix(const MatrixType &spmat) : m_blockSize(BlockSize)
  {
  ...
  {
    eigen_assert(
      (m_innerBSize != 0 && m_outerBSize != 0) && "Trying to assign to a zero-size matrix, call resize() first");
  ...
  template<typename MatrixType> void setBlockStructure(const MatrixType &blockPattern)
  {
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      for (; it; ++it) { nzBlockIdx.push_back(it.index()); }
  ...
  {
    eigen_assert(
      (m_innerBSize != 0 && m_outerBSize != 0) && "TRYING TO RESERVE ZERO-SIZE MATRICES, CALL resize() first");
  ...
  template<typename InputIterator> void setFromTriplets(const InputIterator &begin, const InputIterator &end)
  {
    ...
    for (InputIterator it(begin); it != end; ++it) {
      ...
      eigen_assert(
        (it->value().rows() == it->value().cols() && (it->value().rows() == m_blockSize)) || (m_blockSize == Dynamic));
    ...
    for (StorageIndex bj = 0; bj < m_outerBSize; ++bj) {
      ...
      if (m_blockSize == Dynamic) { m_blockPtr[m_outerIndex[bj + 1]] = m_blockPtr[m_outerIndex[bj]] + nz_outer(bj); }
    }
    ...
    for (InputIterator it(begin); it != end; ++it) {
      ...
      if (m_blockSize == Dynamic) { m_blockPtr[block_id(outer) + 1] = m_blockPtr[block_id(outer)] + block_size; }
      block_id(outer)++;
    }
  ...
  template<typename VecType> BlockSparseTimeDenseProduct<BlockSparseMatrix, VecType> operator*(const VecType &lhs) const
  {
  ...
  Index blockPtr(Index id) const
  {
    ...
    // return blockDynIdx(id, typename internal::conditional<(BlockSize==Dynamic), internal::true_type,
...
public:
  InnerIterator(const BlockSparseMatrix &mat, Index outer)
    : m_mat(mat), m_outerB(mat.outerToBlock(outer)), m_outer(outer), itb(mat, mat.outerToBlock(outer)),
