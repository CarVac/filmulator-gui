...
public:
  ...
  void setZero()
  {
    for (Index j = 0; j < outerSize(); ++j) m_data[j].clear();
  }
  ...
  {
    Index res = 0;
    for (Index j = 0; j < outerSize(); ++j) res += m_data[j].size();
    return res;
  }
  ...
  void reserve(Index reserveSize = 1000)
  {
    if (outerSize() > 0) {
      ...
      for (Index j = 0; j < outerSize(); ++j) { m_data[j].reserve(reserveSizePerVector); }
    }
  }
  ...
  {
    for (Index j = 0; j < outerSize(); ++j) m_data[j].prune(reference, epsilon);
  }
  ...
  void resize(Index rows, Index cols)
  {
    ...
    setZero();
    if (Index(m_data.size()) != outerSize) { m_data.resize(outerSize); }
  }
  ...
  void resizeAndKeepData(Index rows, Index cols)
  {
    ...
    if (m_data.size() != outerSize) { m_data.resize(outerSize); }
  }

  /** The class DynamicSparseMatrix is deprectaed */
  EIGEN_DEPRECATED inline DynamicSparseMatrix() : m_innerSize(0), m_data(0)
  {
  ...
  /** The class DynamicSparseMatrix is deprectaed */
  EIGEN_DEPRECATED inline DynamicSparseMatrix(Index rows, Index cols) : m_innerSize(0)
  {
#ifdef EIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
    EIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
#endif
  ...
  EIGEN_DEPRECATED explicit inline DynamicSparseMatrix(const SparseMatrixBase<OtherDerived> &other) : m_innerSize(0)
  {
#ifdef EIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
    EIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
#endif
  ...
  inline DynamicSparseMatrix(const DynamicSparseMatrix &other) : Base(), m_innerSize(0)
  {
#ifdef EIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
    EIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
#endif
...
public:
  InnerIterator(const DynamicSparseMatrix &mat, Index outer) : Base(mat.m_data[outer]), m_outer(outer) {}
...
public:
  ReverseInnerIterator(const DynamicSparseMatrix &mat, Index outer) : Base(mat.m_data[outer]), m_outer(outer) {}
