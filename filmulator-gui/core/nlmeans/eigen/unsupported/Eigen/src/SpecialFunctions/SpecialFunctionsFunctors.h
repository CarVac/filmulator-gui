...
namespace internal {
  ...
  template<typename Scalar> struct functor_traits<scalar_igamma_op<Scalar>>
  {
    enum {
      // Guesstimate
  ...
  template<typename Scalar> struct functor_traits<scalar_igammac_op<Scalar>>
  {
    enum {
      // Guesstimate
      Cost = 20 * NumTraits<Scalar>::MulCost + 10 * NumTraits<Scalar>::AddCost,
  ...
  template<typename Scalar> struct functor_traits<scalar_betainc_op<Scalar>>
  {
    enum {
      // Guesstimate
  ...
  template<typename Scalar> struct functor_traits<scalar_lgamma_op<Scalar>>
  {
    enum {
      // Guesstimate
  ...
  template<typename Scalar> struct functor_traits<scalar_digamma_op<Scalar>>
  {
    enum {
      // Guesstimate
      Cost = 10 * NumTraits<Scalar>::MulCost + 5 * NumTraits<Scalar>::AddCost,
  ...
  template<typename Scalar> struct functor_traits<scalar_zeta_op<Scalar>>
  {
    enum {
      // Guesstimate
      Cost = 10 * NumTraits<Scalar>::MulCost + 5 * NumTraits<Scalar>::AddCost,
  ...
  template<typename Scalar> struct scalar_polygamma_op
  {
    ...
    typedef typename packet_traits<Scalar>::type Packet;
    EIGEN_DEVICE_FUNC inline Packet packetOp(const Packet &n, const Packet &x) const
    {
      return internal::ppolygamma(n, x);
    }
  };
  template<typename Scalar> struct functor_traits<scalar_polygamma_op<Scalar>>
  {
    enum {
      // Guesstimate
      Cost = 10 * NumTraits<Scalar>::MulCost + 5 * NumTraits<Scalar>::AddCost,
  ...
  template<typename Scalar> struct functor_traits<scalar_erf_op<Scalar>>
  {
    enum {
      // Guesstimate
      Cost = 10 * NumTraits<Scalar>::MulCost + 5 * NumTraits<Scalar>::AddCost,
  ...
  template<typename Scalar> struct functor_traits<scalar_erfc_op<Scalar>>
  {
    enum {
      // Guesstimate
      Cost = 10 * NumTraits<Scalar>::MulCost + 5 * NumTraits<Scalar>::AddCost,
