 ...
 **/
template<typename _Scalar, int _Dim, int _Degree> struct SplineTraits<Spline<_Scalar, _Dim, _Degree>, Dynamic>
{
  ...
  enum {
    OrderAtCompileTime = _Degree == Dynamic ? Dynamic : _Degree + 1 /*!< The spline curve's order at compile-time. */
  };
  enum {
  ...
  : public SplineTraits<Spline<_Scalar, _Dim, _Degree>>
{
  ...
  /** \brief The data type used to store the values of the basis function derivatives. */
  typedef Array<_Scalar, Dynamic, Dynamic, RowMajor, NumOfDerivativesAtCompileTime, OrderAtCompileTime>
    BasisDerivativeType;
