...
template<int Dim> struct TreeTest
{
  typedef Matrix<double, Dim, 1> VectorType;
  ...
  {
    BallTypeList b;
    for (int i = 0; i < 500; ++i) { b.push_back(BallType(VectorType::Random(), 0.5 * internal::random(0., 1.))); }
  ...
  {
    BallTypeList b;
    for (int i = 0; i < 500; ++i) { b.push_back(BallType(VectorType::Random(), 0.01 * internal::random(0., 1.))); }
    KdBVH<double, Dim, BallType> tree(b.begin(), b.end());
