  ...
  bool positiveRangeGamma)
{
  ...
  if (positiveRangeAlpha && (eabis2[0] < 0)) eabis2[0] += Scalar(2 * EIGEN_PI);
  if (positiveRangeBeta && (eabis2[1] < 0)) eabis2[1] += Scalar(2 * EIGEN_PI);
  if (positiveRangeGamma && (eabis2[2] < 0)) eabis2[2] += Scalar(2 * EIGEN_PI);
...
template<typename EulerSystem, typename Scalar> void verify_euler(const Matrix<Scalar, 3, 1> &ea)
{
