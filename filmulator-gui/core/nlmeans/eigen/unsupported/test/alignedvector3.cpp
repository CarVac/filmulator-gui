...
template<typename T, typename Derived> T test_relative_error(const AlignedVector3<T> &a, const MatrixBase<Derived> &b)
{
...
template<typename Scalar> void alignedvector3()
{
...
{
  for (int i = 0; i < g_repeat; i++) { CALL_SUBTEST(alignedvector3<float>()); }
}
