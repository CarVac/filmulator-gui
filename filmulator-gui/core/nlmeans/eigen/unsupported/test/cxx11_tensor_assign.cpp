...
static void test_same_type()
{
  ...
  for (int i = 0; i < 5; ++i) { VERIFY_IS_EQUAL(dest_tensor(i), orig_tensor(i)); }
  ...
  for (int i = 0; i < 5; ++i) { VERIFY_IS_EQUAL(dest_array(i), orig_array(i)); }
  ...
  for (int i = 0; i < 5; ++i) { VERIFY_IS_EQUAL(dest[i], i + 1); }
}
...
static void test_compound_assign()
{
  ...
  for (int i = 0; i < 10; ++i) { VERIFY_IS_EQUAL(tensor(i), start_tensor(i) + offset_tensor(i)); }
