...
static void test_custom_unary_op()
{
  ...
  for (int i = 0; i < 6; i += 2) {
    for (int j = 0; j < 10; j += 2) { VERIFY_IS_EQUAL(result(i, j), tensor(i / 2, j / 2)); }
  }
  for (int i = 1; i < 6; i += 2) {
    for (int j = 1; j < 10; j += 2) { VERIFY_IS_EQUAL(result(i, j), 0); }
  }
}
...
static void test_custom_binary_op()
{
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < 7; ++k) { VERIFY_IS_APPROX(val(j, k), reference(j, k)); }
    }
  }
}
