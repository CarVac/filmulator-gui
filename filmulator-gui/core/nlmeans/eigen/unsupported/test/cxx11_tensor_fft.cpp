...
template<int DataLayout> static void test_fft_2D_golden()
{
...
static void test_fft_real_input_energy()
{
  ...
  typedef
    typename internal::conditional<isComplexInput == true, std::complex<RealScalar>, RealScalar>::type InputScalar;
  ...
  for (int i = 0; i < TensorRank; ++i) { fft[i] = i; }
  ...
  for (int i = 0; i < TensorRank; ++i) { VERIFY_IS_EQUAL(output.dimension(i), input.dimension(i)); }

  RealScalar energy_original = 0.0;
  RealScalar energy_after_fft = 0.0;

  for (int i = 0; i < total_size; ++i) { energy_original += numext::abs2(input(i)); }

  for (int i = 0; i < total_size; ++i) { energy_after_fft += numext::abs2(output(i)); }
