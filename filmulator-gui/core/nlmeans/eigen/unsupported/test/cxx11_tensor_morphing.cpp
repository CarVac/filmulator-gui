...
template<typename> static void test_simple_reshape()
{
...
template<typename> static void test_reshape_in_expr()
{
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 13; ++j) { VERIFY_IS_APPROX(res(i, j), m3(i, j)); }
  }
}

template<typename> static void test_reshape_as_lvalue()
{
...
template<int DataLayout> static void test_simple_slice()
{
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < 3; ++k) { VERIFY_IS_EQUAL(slice2(0, 0, i, j, k), tensor(1, 1, 3 + i, 4 + j, 5 + k)); }
    }
  }
}

template<typename = void> static void test_const_slice()
{
  const float b[1] = { 42 };
...
template<int DataLayout> static void test_slice_in_expr()
{
...
template<int DataLayout> static void test_slice_as_lvalue()
{
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 2; j < 5; ++j) {
      for (int k = 0; k < 5; ++k) { VERIFY_IS_EQUAL(result(i, j, k), tensor3(i, j - 2, k)); }
      for (int k = 5; k < 7; ++k) { VERIFY_IS_EQUAL(result(i, j, k), tensor4(i, j - 2, k - 5)); }
    }
  }
  ...
  for (int i = 0; i < 4; ++i) {
    for (int j = 2; j < 5; ++j) {
      for (int k = 0; k < 7; ++k) { VERIFY_IS_EQUAL(result(i, j, k), tensor5(i, j, k)); }
    }
  }
}

template<int DataLayout> static void test_slice_raw_data()
{
  ...
  if (DataLayout == ColMajor) {
    ...
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < 2; ++j) { VERIFY_IS_EQUAL(slice4.data()[i + 3 * j], tensor(i, 2 + j, 3, 4)); }
    }
  } else {
    ...
    for (int l = 0; l < 11; ++l) {
      for (int k = 0; k < 2; ++k) { VERIFY_IS_EQUAL(slice4.data()[l + 11 * k], tensor(1, 2, 3 + k, l)); }
    }
  }
...
template<int DataLayout> static void test_strided_slice()
{
  ...
  if (true) {
    ...
    for (int j = 0; j < 2; ++j) {
      for (int k = 0; k < 3; ++k) { VERIFY_IS_EQUAL(slice(j, k), tensor2(5 - 2 * j, 7 - k)); }
    }
  }
  ...
  if (true) {// test clamped degenerate interavls
    ...
    slice = tensor2.stridedSlice(indicesStart, indicesStop, strides);
    for (int j = 0; j < 7; ++j) {
      for (int k = 0; k < 11; ++k) { VERIFY_IS_EQUAL(slice(j, k), tensor2(j, 10 - k)); }
    }
  }
  ...
  if (true) {
    ...
    for (int i = 0; i < 2; ++i) {
      for (int j = 0; j < 2; ++j) {
        for (int k = 0; k < 3; ++k) { VERIFY_IS_EQUAL(slice(0, 0, i, j, k), tensor(1, 1, 4 - 2 * i, 4 + j, 7 - k)); }
      }
    }
  }
...
template<int DataLayout> static void test_strided_slice_write()
{
  typedef Tensor<float, 2, DataLayout> Tensor2f;
  typedef Eigen::DSizes<Eigen::DenseIndex, 2> Index2;
  ...
  for (int i = 0; i < 7; i++)
    for (int j = 0; j < 11; j++) { VERIFY_IS_EQUAL(tensor(i, j), tensor2(i, j)); }
}


template<int DataLayout> static void test_composition()
{
  ...
  for (int i = 0; i < 11; ++i) { VERIFY_IS_EQUAL(tensor(0, 0, i), matrix(2, i)); }
}
