...
static void test_float_rounding()
{
  ...
  for (int i = 0; i < 20; ++i) {
    for (int j = 0; j < 30; ++j) { VERIFY_IS_EQUAL(result(i, j), numext::round(ftensor(i, j))); }
  }
}
...
static void test_float_flooring()
{
  ...
  for (int i = 0; i < 20; ++i) {
    for (int j = 0; j < 30; ++j) { VERIFY_IS_EQUAL(result(i, j), numext::floor(ftensor(i, j))); }
  }
}
...
static void test_float_ceiling()
{
  ...
  for (int i = 0; i < 20; ++i) {
    for (int j = 0; j < 30; ++j) { VERIFY_IS_EQUAL(result(i, j), numext::ceil(ftensor(i, j))); }
  }
}
