...
template<typename T> T expfn(T x, int) { return std::exp(x); }

template<typename T> void test2dRotation(double tol)
{
  Matrix<T, 2, 2> A, B, C;
  T angle;
...
template<typename T> void test2dHyperbolicRotation(double tol)
{
...
template<typename T> void testPascal(double tol)
{
...
template<typename MatrixType> void randomTest(const MatrixType &m, double tol)
{
