...
template<typename T> void test2dRotation(const T &tol)
{
  Matrix<T, 2, 2> A, B, C;
  T angle, c, s;
...
template<typename T> void test2dHyperbolicRotation(const T &tol)
{
...
template<typename T> void test3dRotation(const T &tol)
{
  Matrix<T, 3, 1> v;
  T angle;
...
template<typename MatrixType> void testGeneral(const MatrixType &m, const typename MatrixType::RealScalar &tol)
{
...
template<typename MatrixType> void testSingular(const MatrixType &m_const, const typename MatrixType::RealScalar &tol)
{
  ...
  typedef typename internal::conditional<IsComplex, TriangularView<MatrixType, Upper>, const MatrixType &>::type
    TriangularType;
...
template<typename MatrixType> void testLogThenExp(const MatrixType &m_const, const typename MatrixType::RealScalar &tol)
{
  // we need to pass by reference in order to prevent errors with
  // MSVC for aligned data types ...
  MatrixType &m = const_cast<MatrixType &>(m_const);
