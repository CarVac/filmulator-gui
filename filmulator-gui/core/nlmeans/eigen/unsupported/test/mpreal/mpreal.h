...
#define MPREAL_HAVE_DYNAMIC_STD_NUMERIC_LIMITS// Enable extended std::numeric_limits<mpfr::mpreal> specialization.
                                              // Meaning that "digits", "round_style" and similar members are defined as
                                              // functions, not constants. See std::numeric_limits<mpfr::mpreal> at the
                                              // end of the file for more information.
...
#if (__has_feature(cxx_explicit_conversions) || (defined(__GXX_EXPERIMENTAL_CXX0X__) && __GNUC_MINOR__ >= 5) \
...
// MPFR 2.4.0 Specifics
  ...
  friend const mpreal urandom(gmp_randstate_t &state,
    mp_rnd_t rnd_mode);// use gmp_randinit_default() to init state, gmp_randclear() to clear
#endif

#if (MPFR_VERSION >= MPFR_VERSION_NUM(3, 1, 0))
  friend const mpreal grandom(gmp_randstate_t &state,
    mp_rnd_t rnd_mode);// use gmp_randinit_default() to init state, gmp_randclear() to clear
...
namespace internal {
  ...
  template<> struct result_type<mpreal>
  {
    typedef mpreal type;
  };
  template<> struct result_type<mpz_t>
  {
    typedef mpreal type;
  };
  template<> struct result_type<mpq_t>
  {
    typedef mpreal type;
  };
  template<> struct result_type<long double>
  {
    typedef mpreal type;
  };
  template<> struct result_type<double>
  {
    typedef mpreal type;
  };
  template<> struct result_type<unsigned long int>
  {
    typedef mpreal type;
  };
  template<> struct result_type<unsigned int>
  {
    typedef mpreal type;
  };
  template<> struct result_type<long int>
  {
    typedef mpreal type;
  };
  template<> struct result_type<int>
  {
    typedef mpreal type;
  };
  template<> struct result_type<long long>
  {
    typedef mpreal type;
  };
  template<> struct result_type<unsigned long long>
  {
    typedef mpreal type;
  };
}// namespace internal
...
template<typename real_t> inline mpreal &mpreal::operator=(const std::complex<real_t> &z) { return *this = z.real(); }
...
{
  return !isnan EIGEN_NOT_A_MACRO(a) && (mpfr_cmp_ui(a.mpfr_srcptr(), b) > 0);
}
...
{
  return !isnan EIGEN_NOT_A_MACRO(a) && (mpfr_cmp_si(a.mpfr_srcptr(), b) > 0);
}
...
{
  return !isnan EIGEN_NOT_A_MACRO(a) && (b == b) && (mpfr_cmp_ld(a.mpfr_srcptr(), b) > 0);
}
inline bool operator>(const mpreal &a, const double b)
{
  return !isnan EIGEN_NOT_A_MACRO(a) && (b == b) && (mpfr_cmp_d(a.mpfr_srcptr(), b) > 0);
}
...
// inline bool operator >= (const mpreal& a, const unsigned int b      ){  return !isnan EIGEN_NOT_A_MACRO (isnan()a) &&
// (mpfr_cmp_ui(a.mpfr_srcptr(),b) >= 0 );                }
inline bool operator>=(const mpreal &a, const long int b)
{
  return !isnan EIGEN_NOT_A_MACRO(a) && (mpfr_cmp_si(a.mpfr_srcptr(), b) >= 0);
}
...
template<class T> inline std::string toString(T t, std::ios_base &(*f)(std::ios_base &))
{
...
inline const mpreal fac_ui(unsigned long int v,
  mp_prec_t prec = mpreal::get_default_prec(),
...
inline const mpreal pow(const mpreal &a, const long double b, mp_rnd_t rnd_mode) { return pow(a, mpreal(b), rnd_mode); }

inline const mpreal pow(const mpreal &a, const double b, mp_rnd_t rnd_mode) { return pow(a, mpreal(b), rnd_mode); }
...
inline const mpreal pow(const long double a, const mpreal &b, mp_rnd_t rnd_mode) { return pow(mpreal(a), b, rnd_mode); }

inline const mpreal pow(const double a, const mpreal &b, mp_rnd_t rnd_mode) { return pow(mpreal(a), b, rnd_mode); }
...
public:
  ...
  inline static int max_digits10(mp_prec_t precision = mpfr::mpreal::get_default_prec()) { return digits10(precision); }
#else
