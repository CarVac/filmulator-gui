...
#define VERIFY_UNIFORM(SUFFIX, NAME, TYPE)                                                                   \
  {                                                                                                          \
    ...
    if (!value.isApprox(data)) { std::cerr << "Expected:\n" << value << "\n" << "got\n" << data << "\n\n"; } \
...
void test_openglsupport()
{
  ...
  if (glewInit() != GLEW_OK) { std::cerr << "Warning: Failed to initialize GLEW\n"; }
