...
namespace Eigen {
namespace internal {
  template<int Size> struct increment_if_fixed_size
  {
...
template<int Deg, typename POLYNOMIAL, typename SOLVER> bool aux_evalSolver(const POLYNOMIAL &pols, SOLVER &psolve)
{
  ...
  for (int i = 0; i < roots.size(); ++i) { evr[i] = std::abs(poly_eval(pols, roots[i])); }
  ...
  for (size_t i = 1; i < rootModuli.size() && distinctModuli; ++i) {
    if (internal::isApprox(rootModuli[i], rootModuli[i - 1])) { distinctModuli = false; }
  }
...
template<int Deg, typename POLYNOMIAL> void evalSolver(const POLYNOMIAL &pols)
{
  typedef typename POLYNOMIAL::Scalar Scalar;
  ...
  if (aux_evalSolver<Deg, POLYNOMIAL, PolynomialSolverType>(pols, psolve)) {
    ...
    for (size_t i = 0; i < calc_realRoots.size(); ++i) {
      ...
      for (size_t j = 0; j < calc_realRoots.size() && !found; ++j) {
        if (internal::isApprox(calc_realRoots[i], real_roots[j], psPrec)) { found = true; }
      }
      VERIFY(found);
    }
    ...
    if (hasRealRoot) { VERIFY(internal::isApprox(real_roots.array().abs().maxCoeff(), abs(r), psPrec)); }

    // Test absSmallestRealRoot
    r = psolve.absSmallestRealRoot(hasRealRoot);
    VERIFY(hasRealRoot == (real_roots.size() > 0));
    if (hasRealRoot) { VERIFY(internal::isApprox(real_roots.array().abs().minCoeff(), abs(r), psPrec)); }

    // Test greatestRealRoot
    r = psolve.greatestRealRoot(hasRealRoot);
    VERIFY(hasRealRoot == (real_roots.size() > 0));
    if (hasRealRoot) { VERIFY(internal::isApprox(real_roots.array().maxCoeff(), r, psPrec)); }

    // Test smallestRealRoot
    r = psolve.smallestRealRoot(hasRealRoot);
    VERIFY(hasRealRoot == (real_roots.size() > 0));
    if (hasRealRoot) { VERIFY(internal::isApprox(real_roots.array().minCoeff(), r, psPrec)); }
  }
}
...
template<typename _Scalar, int _Deg> void polynomialsolver(int deg)
{
  ...
  evalSolverSugarFunction<_Deg>(
    pols, realRoots.template cast<std::complex<typename NumTraits<_Scalar>::Real>>(), realRoots);
}
