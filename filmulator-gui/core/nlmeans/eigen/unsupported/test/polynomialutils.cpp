...
template<typename _Scalar, int _Deg> void realRoots_to_monicPolynomial_test(int deg)
{
  ...
  if (!evalToZero) { cerr << evr.transpose() << endl; }
  VERIFY(evalToZero);
}
...
template<typename _Scalar, int _Deg> void CauchyBounds(int deg)
{
  typedef internal::increment_if_fixed_size<_Deg> Dim;
  typedef Matrix<_Scalar, Dim::ret, 1> PolynomialType;
  typedef Matrix<_Scalar, _Deg, 1> EvalRootsType;
