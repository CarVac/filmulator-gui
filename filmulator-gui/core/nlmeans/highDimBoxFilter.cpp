...
void highDimBoxFilter(float *__restrict const A,
                      float *__restrict const W,
                      float *__restrict const C1chanT,
                      ptrdiff_t const numClusters,
                      float *__restrict output) {

    // Calculate the weighting (Wt) in this block across all clusters to preserve locallity. TODO: test moving this into
    // the per-cluster for loop
        ...
        for (ptrdiff_t destClusterIdx = 0; destClusterIdx < numClusters; destClusterIdx++) {
            for (ptrdiff_t sourceClusterIdx = 0; sourceClusterIdx < numClusters; sourceClusterIdx++) {
                for (ptrdiff_t yIdx = 0; yIdx < blockSize; yIdx++) {
                    ...
                    ptrdiff_t W_idx = (yIdx + S) + (xIdx + S) * expandedBlockSize
                                      + sourceClusterIdx * expandedBlockSize * expandedBlockSize;
    ...
    std::array<float, blockSize * expandedBlockSize>
      W_summedAreaTable; // W and WA get initialized to 0 first thing in the loop
    ...
    for (ptrdiff_t clusterIdx = 0; clusterIdx < numClusters; clusterIdx++) {
        ...
        // Lastly, when we read out the values, we will do the appropriate subtractions to calculate the blur in the y
        // direction

        // First create temporary summed area table for a blur in the x direction
        std::fill(W_summedAreaTable.begin(),
                  W_summedAreaTable.end(),
                  0); // TODO: I probably only need to zero out the first column
        std::fill(
          WA_summedAreaTable.begin(), WA_summedAreaTable.end(), 0); // TODO: might want to set to epsilon instead of 0
        ...
        for (ptrdiff_t xReadIdx = 0; xReadIdx < (1 + 2 * S); xReadIdx++) {
            for (ptrdiff_t c = 0; c < 3; c++) {
                for (ptrdiff_t y = 0; y < expandedBlockSize; y++) {
                    ...
                    ptrdiff_t W_idx =
                      y + xReadIdx * expandedBlockSize + clusterIdx * expandedBlockSize * expandedBlockSize;
        ...
        for (ptrdiff_t xWriteIdx = 1; xWriteIdx < blockSize; xWriteIdx++) {
            for (ptrdiff_t c = 0; c < 3; c++) {
                for (ptrdiff_t y = 0; y < expandedBlockSize; y++) {
                    ...
                    ptrdiff_t W_idx_add =
                      y + xImageReadAdd * expandedBlockSize + clusterIdx * expandedBlockSize * expandedBlockSize;
                    ptrdiff_t W_idx_sub =
                      y + xImageReadSub * expandedBlockSize + clusterIdx * expandedBlockSize * expandedBlockSize;

                    ptrdiff_t WA_sat_idx_write =
                      W_sat_idx_write
                      + c * blockSize * expandedBlockSize; // WA has same index as W, but shifted by the color
                    ptrdiff_t WA_sat_idx_read = W_sat_idx_read + c * blockSize * expandedBlockSize;
                    ptrdiff_t A_idx_add =
                      y + xImageReadAdd * expandedBlockSize + c * expandedBlockSize * expandedBlockSize;
                    ptrdiff_t A_idx_sub =
                      y + xImageReadSub * expandedBlockSize + c * expandedBlockSize * expandedBlockSize;

                    if (c == 0) {
                        W_summedAreaTable[W_sat_idx_write] =
                          W_summedAreaTable[W_sat_idx_read] + W[W_idx_add] - W[W_idx_sub];
                    }
                    WA_summedAreaTable[WA_sat_idx_write] =
                      WA_summedAreaTable[WA_sat_idx_read] + W[W_idx_add] * A[A_idx_add] - W[W_idx_sub] * A[A_idx_sub];
                }
            }
        }
        ...
        for (ptrdiff_t c = 0; c < 3; c++) {
            for (ptrdiff_t yWriteIdx = 1; yWriteIdx < expandedBlockSize;
                 yWriteIdx++) { // the cumsum of the first row is equal to the first row, so skip it. TODO: change this
                                // comment when filpping order
                for (ptrdiff_t xIdx = 0; xIdx < blockSize; xIdx++) {
        ...
        for (ptrdiff_t c = 0; c < 3; c++) {
            for (ptrdiff_t xIdx = 0; xIdx < blockSize; xIdx++) {
                ...
                if (yIdx == 0) {
                    if (c == 0) { Wb[Wb_idx] += Wt[Wt_idx] * W_summedAreaTable[W_sat_idx + S]; }
                ...
                for (ptrdiff_t yIdx = 1; yIdx < blockSize; yIdx++) {
                    ...
                    if (c == 0) {
                        Wb[Wb_idx] +=
                          Wt[Wt_idx]
                          * (W_summedAreaTable[W_sat_idx + S]
                             - W_summedAreaTable[W_sat_idx - S - 1]); // We read out a blurred value using the summed
                                                                      // area table. blur = cumsum(x+r) - cumsum(x-r)
                    }
                    B[B_idx] +=
                      Wt[Wt_idx] * (WA_summedAreaTable[WA_sat_idx + S] - WA_summedAreaTable[WA_sat_idx - S - 1]);
                }
            }
        }
    }
    ...
    for (ptrdiff_t chanIdx = 0; chanIdx < 3; chanIdx++) {
        for (ptrdiff_t xIdx = 0; xIdx < blockSize; xIdx++) {
            for (ptrdiff_t yIdx = 0; yIdx < blockSize; yIdx++) {
                ...
                ptrdiff_t A_idx =
                  yIdx + S + (xIdx + S) * expandedBlockSize + chanIdx * expandedBlockSize * expandedBlockSize;

                // Small values of Wb are unreliable. If ln(abs(Wb)) < -3, then disregard just fall back to the original
                // value.
