  ...
  float *__restrict output)
{
    ...
    for (ptrdiff_t destClusterIdx = 0; destClusterIdx < numClusters; destClusterIdx++) {
      for (ptrdiff_t sourceClusterIdx = 0; sourceClusterIdx < numClusters; sourceClusterIdx++) {
        for (ptrdiff_t yIdx = 0; yIdx < blockSize; yIdx++) {
          ...
          ptrdiff_t W_idx =
            (yIdx + S) + (xIdx + S) * expandedBlockSize + sourceClusterIdx * expandedBlockSize * expandedBlockSize;
  ...
  for (ptrdiff_t clusterIdx = 0; clusterIdx < numClusters; clusterIdx++) {
    ...
    std::fill(
      W_summedAreaTable.begin(), W_summedAreaTable.end(), 0);// TODO: I probably only need to zero out the first column
    std::fill(WA_summedAreaTable.begin(), WA_summedAreaTable.end(), 0);// TODO: might want to set to epsilon instead of
                                                                       // 0
      ...
      for (ptrdiff_t c = 0; c < 3; c++) {
        for (ptrdiff_t y = 0; y < expandedBlockSize; y++) {
          ...
          ptrdiff_t WA_sat_idx_write =
            W_sat_idx_write + c * blockSize * expandedBlockSize;// WA has same index as W, but shifted by the color
