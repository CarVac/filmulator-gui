...
bool kMeansNLMApprox(float* __restrict const I, const int maxNumClusters, const float clusterThreshold, const float h, const int sizeX, const int sizeY, float* __restrict output, ParameterManager* paramManager) {
	...
	bool canceled = false;
	#pragma omp parallel shared(canceled)
	{
		#pragma omp for
		for (ptrdiff_t blockIdx = 0; blockIdx < numBlocks; blockIdx++) {
			...
			std::array<float, expandedBlockSize * expandedBlockSize * numChannels> patchMeans = {};
			
			expandDims(IBlockCopy.data(), expandedDimensions.data(), patchMeans.data());
			...
			highDimBoxFilter(IBlockCopy.data(), W.data(), patchMeans.data(), C1ChanT.data(), numClusters, tileOutput.data());
