...
bool kMeansNLMApprox(float *__restrict const I,
...
#pragma omp cancel for
        ...
        std::vector<float> IBlockCopy(
          expandedBlockSize * expandedBlockSize
          * numChannels); // move from stack to global memory (avoid crashes on MacOs, with its small thread stack)
        ...
        for (ptrdiff_t c = 0; c < numChannels; c++) {
            for (ptrdiff_t xWriteIdx = 0; xWriteIdx < expandedBlockSize; xWriteIdx++) {
                for (ptrdiff_t yWriteIdx = 0; yWriteIdx < expandedBlockSize; yWriteIdx++) {
                    ...
                    IBlockCopy[yWriteIdx + xWriteIdx * expandedBlockSize + c * expandedBlockSize * expandedBlockSize] =
                      value;
                }
            }
        }
        ...
        std::vector<float> expandedDimensions(
          expandedBlockSize * expandedBlockSize * numChannels
          * patchSize); // move from stack to global memory (avoid crashes on MacOs, with its small thread stack)
        ...
        for (auto &sample : sampledLocs) { sample = distribution(generator); }
        ...
        for (ptrdiff_t dIdx = 0; dIdx < (patchSize * numChannels); dIdx++) {
            for (ptrdiff_t pIdx = 0; pIdx < numPatchesToSample; pIdx++) {
                sampledPatches[pIdx + dIdx * numPatchesToSample] =
                  expandedDimensions[sampledLocs[pIdx] + dIdx * expandedBlockSize * expandedBlockSize];
            }
        }
        ...
        calcW(
          expandedDimensions.data(), clusterCenters.data(), patchSize * 3, numClusters, expandedBlockSize, h, W.data());
            ...
            for (ptrdiff_t xReadIdx = 0; xReadIdx < blockSize; xReadIdx++) {
                for (ptrdiff_t yReadIdx = 0; yReadIdx < blockSize; yReadIdx++) {
                    ...
                    if ((yWriteIdx >= 0) & (yWriteIdx < sizeY) & (xWriteIdx >= 0) & (xWriteIdx < sizeX)) {
                        output[yWriteIdx * 3 + 3 * xWriteIdx * sizeY + c] =
                          tileOutput[yReadIdx + xReadIdx * blockSize + c * blockSize * blockSize];
                    }
                }
            }
        }
