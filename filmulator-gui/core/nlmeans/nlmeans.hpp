...
// Based on Fast High-Dimensional Bilateral and Nonlocal Means Filtering
// by Parvin Nair and Kunal N. Chaudhury (2018)
// https://arxiv.org/pdf/1811.02363
...
void calcW(float *const Aguide_ptr,
  float *const centers_ptr,
  ptrdiff_t rangeDims,
  ptrdiff_t numClusters,
  ptrdiff_t expandedBlockSize,
  float const h,
  float *W_ptr);
...
bool kMeansNLMApprox(float *const I,
...
std::tuple<std::vector<float>, std::vector<bool>, std::array<double, 2>, std::array<int, 2>>
  splitCluster(float *const X, const int numPoints);
