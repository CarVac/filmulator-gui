...
float Mad(const float *DataList, const int datalen)
{
  ...
  for (int i = 0; i < datalen; ++i) { histo[static_cast<int>(rtengine::min(32768.f, fabsf(DataList[i])))]++; }
...
float MadRgb(const float *DataList, const int datalen)
{
  ...
  // calculate histogram of absolute values of wavelet coeffs
  for (int i = 0; i < datalen; ++i) { histo[static_cast<int>(rtengine::min(65535.f, fabsf(DataList[i])))]++; }
...
void Noise_residualAB(const wavelet_decomposition &WaveletCoeffs_ab,
  float &chresid,
  float &chmaxresid,
  bool denoiseMethodRgb,
...
#ifdef _OPENMP
#pragma omp parallel for schedule(dynamic) collapse(2) reduction(+ : resid) reduction(max : maxresid) \
  num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
#endif
  for (int lvl = 0; lvl < WaveletCoeffs_ab.maxlevel(); ++lvl) {
    ...
    for (int dir = 1; dir < 4; ++dir) {
      ...
      const float madC = rtengine::SQR(
        denoiseMethodRgb ? MadRgb(WavCoeffs_ab[dir], Wlvl_ab * Hlvl_ab) : Mad(WavCoeffs_ab[dir], Wlvl_ab * Hlvl_ab));
...
void ShrinkAllAB(wavelet_decomposition &WaveletCoeffs_L,
...

{
  ...
  for (int p = 0; p < H_ab * W_ab; p++) { nvc[p] = noisevarchrom[p]; }
...
//        boxblur(sfaveab, sfaveabd, blurBuffer, level + 2, level + 2, W_ab, H_ab); //increase smoothness by locally
//        averaging shrinkage
#ifdef __SSE2__
    vfloat epsv = F2V(eps);
    vfloat sfabv;
    ...
    for (coeffloc_ab = 0; coeffloc_ab < H_ab * W_ab - 3; coeffloc_ab += 4) {
      ...
      STVFU(WavCoeffs_ab[dir][coeffloc_ab],
        LVFU(WavCoeffs_ab[dir][coeffloc_ab]) * (SQRV(sfaveabv) + SQRV(sfabv)) / (sfaveabv + sfabv + epsv));
    }
    ...
    for (; coeffloc_ab < H_ab * W_ab; ++coeffloc_ab) {
      ...
      WavCoeffs_ab[dir][coeffloc_ab] *=
        (rtengine::SQR(sfaveabd[coeffloc_ab]) + rtengine::SQR(sfab)) / (sfaveabd[coeffloc_ab] + sfab + eps);
    }// now chrominance coefficients are denoised
...
bool WaveletDenoiseAll_BiShrinkAB(wavelet_decomposition &WaveletCoeffs_L,
  wavelet_decomposition &WaveletCoeffs_ab,
  float *noisevarchrom,
  float madL[8][3],
  int local,
  ...
  int denoiseNestedLevels)
{
  ...
  for (int lvl = 0; lvl < maxlvl; ++lvl) {
    if (WaveletCoeffs_L.level_W(lvl) > maxWL) { maxWL = WaveletCoeffs_L.level_W(lvl); }

    if (WaveletCoeffs_L.level_H(lvl) > maxHL) { maxHL = WaveletCoeffs_L.level_H(lvl); }
  }
...
#pragma omp parallel num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
#endif
  {
    ...
    if (buffer[0] == nullptr || buffer[1] == nullptr || buffer[2] == nullptr) { memoryAllocationFailed = true; }
...
#pragma omp for schedule(dynamic) collapse(2)
      ...
      for (int lvl = maxlvl - 1; lvl >= 0; lvl--) {// for levels less than max, use level diff to make edge mask
        for (int dir = 1; dir < 4; ++dir) {
          ...
          if (lvl == maxlvl - 1) {
            ...
            for (int p = 0; p < Hlvl_ab * Wlvl_ab; p++) { nvc[p] = noisevarchrom[p]; }
            ...
            if (noisevarfc > 0.001f) {
              ...
              for (; coeffloc_ab < Hlvl_ab * Wlvl_ab; ++coeffloc_ab) {
                float mag_L = rtengine::SQR(WavCoeffs_L[dir][coeffloc_ab]);
                float mag_ab = rtengine::SQR(WavCoeffs_ab[dir][coeffloc_ab]);
                WavCoeffs_ab[dir][coeffloc_ab] *= rtengine::SQR(
                  1.f - xexpf(-(mag_ab / (nvc[coeffloc_ab] * mad_abr)) - (mag_L / (9.f * mad_Lr))) /*satfactor_a*/);
              }// now chrominance coefficients are denoised
    ...
    for (int i = 2; i >= 0; i--) { delete[] buffer[i]; }
  }
  return (!memoryAllocationFailed);
}

bool WaveletDenoiseAllAB(wavelet_decomposition &WaveletCoeffs_L,
  wavelet_decomposition &WaveletCoeffs_ab,
  float *noisevarchrom,
  float madL[8][3],
  int local,
  ...
  bool autoch,
  bool denoiseMethodRgb,
  int denoiseNestedLevels)// mod JD
...
#pragma omp for schedule(dynamic) collapse(2)
        ...
        for (int tiletop = 0; tiletop < imheight; tiletop += tileHskip) {
          for (int tileleft = 0; tileleft < imwidth; tileleft += tileWskip) {
            ...
            if (realred <= 0.f) { realred = 0.001f; }

            realblue = interm_med + intermblue;

            if (realblue <= 0.f) { realblue = 0.001f; }
...
#pragma omp parallel for schedule(dynamic, 16) num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
            ...
            for (int i = tiletop; i < tilebottom; ++i) {
              ...
              for (int j = tileleft; j < tileright; ++j) {
                ...
                if (((i1 | j1) & 1) == 0) { noisevarchrom[(i1 >> 1) * width2 + (j1 >> 1)] = 1.f; }
            ...
            if (execwavelet) {// gain time if user choose only median  sliders L <=1  slider chrom master < 1
              ...
              //  noisevarab_b);
              const std::unique_ptr<wavelet_decomposition> Ldecomp(
                new wavelet_decomposition(labdn->L[0], labdn->W, labdn->H, levwav, 1, 1, max(1, denoiseNestedLevels)));

              if (Ldecomp->memory_allocation_failed()) { memoryAllocationFailed = true; }
...
#pragma omp parallel for schedule(dynamic) collapse(2) num_threads(denoiseNestedLevels) if (denoiseNestedLevels > 1)
              ...
              std::unique_ptr<wavelet_decomposition> adecomp(
                new wavelet_decomposition(labdn->a[0], labdn->W, labdn->H, levwav, 1, 1, max(1, denoiseNestedLevels)));

              if (adecomp->memory_allocation_failed()) { memoryAllocationFailed = true; }

              if (!memoryAllocationFailed) {
                if (!WaveletDenoiseAllAB(*Ldecomp,
                      *adecomp,
                      noisevarchrom,
                      madL,
                      0,
                      ...
                      autoch,
                      denoiseMethodRgb,
                      denoiseNestedLevels)) {// enhance mode
              ...
              if (!memoryAllocationFailed) {
                std::unique_ptr<wavelet_decomposition> bdecomp(new wavelet_decomposition(
                  labdn->b[0], labdn->W, labdn->H, levwav, 1, 1, max(1, denoiseNestedLevels)));

                if (bdecomp->memory_allocation_failed()) { memoryAllocationFailed = true; }
