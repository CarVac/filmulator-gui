...
#pragma omp parallel for if (multiThread)
      ...
      for (int row = 0; row < H; ++row) {
        for (int col = 0; col < W; ++col) { dst[row][col] = src[row][col]; }
      }
    }
    return;
  }
...
#pragma omp for
    ...
    for (int row = 0; row < H; ++row) {
      ...
      for (int j = 1; j <= radius; j++) { tempval += src[row][j]; }
...
#pragma omp for nowait
    ...
    for (int col = 0; col < W - numCols + 1; col += 8) {
      ...
      for (int k = 0; k < numCols; ++k) { rowBuffer[0][k] = dst[0][col + k]; }

      for (int i = 1; i <= radius; ++i) {
        for (int k = 0; k < numCols; ++k) { dst[0][col + k] += dst[i][col + k]; }
      }

      for (int k = 0; k < numCols; ++k) { dst[0][col + k] /= len; }
...
#pragma omp single
#endif
    {
      ...
      if (remaining > 0) {
        ...
        float len = radius + 1;
        for (int k = 0; k < remaining; ++k) { rowBuffer[0][k] = dst[0][col + k]; }
        for (int row = 1; row <= radius; ++row) {
          for (int k = 0; k < remaining; ++k) { dst[0][col + k] += dst[row][col + k]; }
        }
        for (int k = 0; k < remaining; ++k) { dst[0][col + k] /= len; }
        for (int row = 1; row <= radius; ++row) {
...
#pragma omp parallel for if (multiThread)
      ...
      for (int row = 0; row < H; ++row) {
        for (int col = 0; col < W; ++col) { dst[row][col] = std::fabs(src[row][col]); }
      }
    }
    return;
  }
...
#pragma omp for
    ...
    for (int row = 0; row < H; ++row) {
      ...
      lineBuffer[0] = tempval;
      for (int j = 1; j <= radius; j++) { tempval += std::fabs(src[row][j]); }
