...
public:
  ...
  bool memory_allocation_failed() const { return memoryAllocationFailed; }

  const internal_type *const *level_coeffs(int level) const { return wavelet_decomp[level]->subbands(); }

  internal_type *const *level_coeffs(int level) { return wavelet_decomp[level]->subbands(); }

  const internal_type *get_coeff0() const { return coeff0; }

  internal_type *get_coeff0() { return coeff0; }
  ...
  int level_stride(int level) const { return wavelet_decomp[level]->stride(); }
...
template<typename E>
wavelet_decomposition::wavelet_decomposition(E *src,
  int width,
  int height,
  int maxlvl,
  int subsampling,
  ...
  int numThreads,
  int Daub4Len)
  : lvltot(0), subsamp(subsampling), m_w(width), m_h(height), coeff0(nullptr), memoryAllocationFailed(false)
{
  ...
  wavelet_decomp[lvltot] = new wavelet_level<internal_type>(src,
  ...
  if (wavelet_decomp[lvltot]->memoryAllocationFailed) { memoryAllocationFailed = true; }

  while (lvltot < maxlvl - 1) {
    lvltot++;
    bufferindex ^= 1;
    wavelet_decomp[lvltot] = new wavelet_level<internal_type>(buffer[bufferindex],
      buffer[bufferindex ^ 1] /*lopass*/,
      lvltot /*level*/,
      subsamp,
      wavelet_decomp[lvltot - 1]->width(),
      wavelet_decomp[lvltot - 1]->height(),
      wavfilt_anal,
      wavfilt_anal,
      wavfilt_len,
      wavfilt_offset,
...
template<typename E> void wavelet_decomposition::reconstruct(E *dst, const float blend)
{
  ...
  if (lvltot >= 1) {
    ...
    for (int lvl = lvltot; lvl > 0; lvl--) {
      ...
      wavelet_decomp[lvl]->reconstruct_level(
        tmpLo, tmpHi, coeff0, coeff0, wavfilt_synth, wavfilt_synth, wavfilt_len, wavfilt_offset);
  ...
  if (wavelet_decomp[0]->bigBlockOfMemoryUsed()) {// bigBlockOfMemoryUsed means that wavcoeffs[2] points to a block of
                                                  // memory big enough to hold the data
  ...
  if (tmpHi == nullptr) {
    ...
    if (!wavelet_decomp[0]->bigBlockOfMemoryUsed()) { delete[] tmpLo; }

    return;
  }


  wavelet_decomp[0]->reconstruct_level(
    tmpLo, tmpHi, coeff0, dst, wavfilt_synth, wavfilt_synth, wavfilt_len, wavfilt_offset, blend);
