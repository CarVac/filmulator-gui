...
template<typename T> class wavelet_level
{
  ...
  void AnalysisFilterHaarVertical(const T *const srcbuffer,
    T *dstLo,
    T *dstHi,
    const int width,
    const int height,
  ...
  void SynthesisFilterHaarHorizontal(const T *const srcLo,
    const T *const srcHi,
    T *dst,
    const int width,
    const int height);
  void
    SynthesisFilterHaarVertical(const T *const srcLo, const T *const srcHi, T *dst, const int width, const int height);

  void AnalysisFilterSubsampHorizontal(T *srcbuffer,
    T *dstLo,
    T *dstHi,
    float *filterLo,
    float *filterHi,
    ...
    const int row);
#ifdef __SSE2__
  void AnalysisFilterSubsampVertical(T *srcbuffer,
    T *dstLo,
    T *dstHi,
    float (*filterLo)[4],
    float (*filterHi)[4],
  ...
  void AnalysisFilterSubsampVertical(T *srcbuffer,
    ...
    const int row);
#endif
  void SynthesisFilterSubsampHorizontal(T *srcLo,
    T *srcHi,
    T *dst,
    float *filterLo,
    float *filterHi,
    ...
    const int height);
#ifdef __SSE2__
  void SynthesisFilterSubsampVertical(T *srcLo,
    T *srcHi,
    T *dst,
    float (*filterLo)[4],
    float (*filterHi)[4],
...
public:
  ...
  wavelet_level(E *src,
    ...
    int skipcrop,
    int numThreads)
    : lvl(level), subsamp_out((subsamp >> level) & 1), numThreads(numThreads), skip(1 << level), bigBlockOfMemory(true),
      memoryAllocationFailed(false), wavcoeffs(nullptr), m_w(w), m_h(h), m_w2(w), m_h2(h)
  {
    if (subsamp) {
      skip = 1;

      for (int n = 0; n < level; n++) { skip *= 2 - ((subsamp >> n) & 1); }
    ...
    if (!memoryAllocationFailed) { decompose_level(src, dst, filterV, filterH, len, offset); }
  }

  ~wavelet_level() { destroy(wavcoeffs); }

  T **subbands() const { return wavcoeffs; }

  T *lopass() const { return wavcoeffs[0]; }

  int width() const { return m_w2; }

  int height() const { return m_h2; }

  int stride() const { return skip; }

  bool bigBlockOfMemoryUsed() const { return bigBlockOfMemory; }

  template<typename E> void decompose_level(E *src, E *dst, float *filterV, float *filterH, int len, int offset);

  template<typename E>
  void reconstruct_level(E *tmpLo,
    E *tmpHi,
    E *src,
    E *dst,
    float *filterV,
...
template<typename T> T **wavelet_level<T>::create(int n)
{
  T *data = new (std::nothrow) T[3 * n];

  if (data == nullptr) { bigBlockOfMemory = false; }
...
template<typename T> void wavelet_level<T>::destroy(T **subbands)
{
  if (subbands) {
    if (bigBlockOfMemory) {
      delete[] subbands[1];
      ...
      for (int j = 1; j < 4; j++) {
        if (subbands[j] != nullptr) { delete[] subbands[j]; }
      }
    }
...
template<typename T>
void wavelet_level<T>::AnalysisFilterHaarHorizontal(const T *const RESTRICT srcbuffer,
  T *RESTRICT dstLo,
  T *RESTRICT dstHi,
  const int width,
  const int row)
...
template<typename T>
void wavelet_level<T>::AnalysisFilterHaarVertical(const T *const RESTRICT srcbuffer,
  T *RESTRICT dstLo,
  T *RESTRICT dstHi,
  const int width,
  const int height,
...
template<typename T>
void wavelet_level<T>::SynthesisFilterHaarHorizontal(const T *const RESTRICT srcLo,
  const T *const RESTRICT srcHi,
  T *RESTRICT dst,
  const int width,
  const int height)
{
...
#pragma omp parallel for num_threads(numThreads) if (numThreads > 1)
  ...
  for (int k = 0; k < height; k++) {
    for (int i = 0; i < skip; i++) { dst[k * width + i] = (srcLo[k * width + i] + srcHi[k * width + i]); }

    for (int i = skip; i < width; i++) {
      dst[k * width + i] =
        0.5f
        * (srcLo[k * width + i] + srcHi[k * width + i] + srcLo[k * width + i - skip] - srcHi[k * width + i - skip]);
    }
  }
}

template<typename T>
void wavelet_level<T>::SynthesisFilterHaarVertical(const T *const RESTRICT srcLo,
  const T *const RESTRICT srcHi,
  T *RESTRICT dst,
  const int width,
  const int height)
...
#pragma omp for nowait
    ...
    for (int i = 0; i < std::min(skip, height); i++) {
      for (int j = 0; j < width; j++) { dst[width * i + j] = (srcLo[i * width + j] + srcHi[i * width + j]); }
    }
...
#pragma omp for
    ...
    for (int i = skip; i < height; i++) {
      for (int j = 0; j < width; j++) {
        dst[width * i + j] = 0.5f
                             * (srcLo[i * width + j] + srcHi[i * width + j] + srcLo[(i - skip) * width + j]
                                - srcHi[(i - skip) * width + j]);
      }
    }
  }
}
...
template<typename T>
void wavelet_level<T>::AnalysisFilterSubsampHorizontal(T *RESTRICT srcbuffer,
  T *RESTRICT dstLo,
  T *RESTRICT dstHi,
  float *RESTRICT filterLo,
  float *RESTRICT filterHi,
  const int taps,
  const int offset,
  const int srcwidth,
  const int dstwidth,
...
#ifdef __SSE2__
template<typename T>
void wavelet_level<T>::AnalysisFilterSubsampVertical(T *RESTRICT srcbuffer,
  T *RESTRICT dstLo,
  T *RESTRICT dstHi,
  float (*RESTRICT filterLo)[4],
  float (*RESTRICT filterHi)[4],
  const int taps,
  const int offset,
  const int width,
  const int height,
...
template<typename T>
void wavelet_level<T>::SynthesisFilterSubsampHorizontal(T *RESTRICT srcLo,
  T *RESTRICT srcHi,
  T *RESTRICT dst,
  float *RESTRICT filterLo,
  float *RESTRICT filterHi,
...
#ifdef __SSE2__
template<typename T>
void wavelet_level<T>::SynthesisFilterSubsampVertical(T *RESTRICT srcLo,
  T *RESTRICT srcHi,
  T *RESTRICT dst,
  float (*RESTRICT filterLo)[4],
  float (*RESTRICT filterHi)[4],
  ...
  for (int i = 0; i < dstheight; i++) {
    ...
    if (LIKELY(i > skip * taps && i < (dstheight - skip * taps))) {// bulk
      ...
      for (k = 0; k < width - 3; k += 4) {
        ...
        for (int j = begin, l = 0; j < taps; j += 2, l += skip) {
          totv += ((LVF(filterLo[j][0]) * LVFU(srcLo[(i_src - l) * width + k])
                    + LVF(filterHi[j][0]) * LVFU(srcHi[(i_src - l) * width + k])));
        }
...
template<typename T>
template<typename E>
void wavelet_level<T>::decompose_level(E *src, E *dst, float *filterV, float *filterH, int taps, int offset)
{
  ...
  if (subsamp_out) {
    for (int i = 0; i < 2 * taps; i++) {
      for (int j = 0; j < 4; j++) { filterVarray[i][j] = filterV[i]; }
    }
  }
...
#pragma omp for
      ...
      for (int row = 0; row < m_h; row += 2) {
        AnalysisFilterSubsampVertical(
          src, tmpLo, tmpHi, filterVarray, filterVarray + taps, taps, offset, m_w, m_h, row);
        AnalysisFilterSubsampHorizontal(
          tmpLo, dst, wavcoeffs[1], filterH, filterH + taps, taps, offset, m_w, m_w2, row / 2);
        AnalysisFilterSubsampHorizontal(
          tmpHi, wavcoeffs[2], wavcoeffs[3], filterH, filterH + taps, taps, offset, m_w, m_w2, row / 2);
      }
    } else {
#ifdef _OPENMP
#pragma omp for
...
template<typename T>
template<typename E>
void wavelet_level<T>::reconstruct_level(E *tmpLo,
  E *tmpHi,
  E *src,
  E *dst,
  float *filterV,
  ...
  const float blend)
{
  if (memoryAllocationFailed) { return; }
  ...
  if (subsamp_out) {
    ...
    SynthesisFilterSubsampHorizontal(
      wavcoeffs[2], wavcoeffs[3], tmpHi, filterH, filterH + taps, taps, offset, m_w2, m_w, m_h2);
    SynthesisFilterSubsampHorizontal(src, wavcoeffs[1], tmpLo, filterH, filterH + taps, taps, offset, m_w2, m_w, m_h2);
    SynthesisFilterSubsampVertical(
      tmpLo, tmpHi, dst, filterVarray, filterVarray + taps, taps, offset, m_w, m_h2, m_h, blend);
  } else {
