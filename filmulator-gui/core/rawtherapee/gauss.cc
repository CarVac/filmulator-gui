...
void compute7x7kernel(float sigma, float kernel[7][7])
{
  ...
  for (int i = 0; i < 7; ++i) {
    for (int j = 0; j < 7; ++j) { kernel[i][j] /= sum; }
  }
}
...
void compute5x5kernel(float sigma, float kernel[5][5])
{
  ...
  for (int i = 0; i < 5; ++i) {
    for (int j = 0; j < 5; ++j) { kernel[i][j] /= sum; }
  }
}
...
#pragma omp single nowait
#endif
  {
    ...
    for (int j = 1; j < W - 1; j++) { dst[0][j] = b1 * (src[0][j - 1] + src[0][j + 1]) + b0 * src[0][j]; }
  ...
  for (int i = 1; i < H - 1; i++) {
    ...
    for (int j = 1; j < W - 1; j++) {
      dst[i][j] = c2 * (src[i - 1][j - 1] + src[i - 1][j + 1] + src[i + 1][j - 1] + src[i + 1][j + 1])
                  + c1 * (src[i - 1][j] + src[i][j - 1] + src[i][j + 1] + src[i + 1][j]) + c0 * src[i][j];
    }
...
// classical filtering if the support window is small and src != dst
template<class T>
void gauss3x3mult(T **RESTRICT src,
  T **RESTRICT dst,
  const int W,
  const int H,
  const T c0,
...
template<class T>
void gauss3x3div(T **RESTRICT src,
  T **RESTRICT dst,
  T **RESTRICT divBuffer,
  const int W,
  const int H,
  const T c0,
  const T c1,
...
#pragma omp for nowait
  ...
  for (int i = 1; i < H - 1; i++) {
    ...
    for (int j = 1; j < W - 1; j++) {
      tmp = (c2 * (src[i - 1][j - 1] + src[i - 1][j + 1] + src[i + 1][j - 1] + src[i + 1][j + 1])
             + c1 * (src[i - 1][j] + src[i][j - 1] + src[i][j + 1] + src[i + 1][j]) + c0 * src[i][j]);
...
#pragma omp single
#endif
  {
    ...
    dst[H - 1][W - 1] =
      rtengine::max(divBuffer[H - 1][W - 1] / (src[H - 1][W - 1] > 0.f ? src[H - 1][W - 1] : 1.f), 0.f);
  }
}

template<class T>
void gauss7x7div(T **RESTRICT src, T **RESTRICT dst, T **RESTRICT divBuffer, const int W, const int H, float sigma)
{
...
#pragma omp for schedule(dynamic, 16) nowait
  ...
  for (int i = 3; i < H - 3; ++i) {
    ...
    for (int j = 3; j < W - 3; ++j) {
      const float val = c31
                          * (src[i - 3][j - 1] + src[i - 3][j + 1] + src[i - 1][j - 3] + src[i - 1][j + 3]
                             + src[i + 1][j - 3] + src[i + 1][j + 3] + src[i + 3][j - 1] + src[i + 3][j + 1])
                        + c30 * (src[i - 3][j] + src[i][j - 3] + src[i][j + 3] + src[i + 3][j])
                        + c22 * (src[i - 2][j - 2] + src[i - 2][j + 2] + src[i + 2][j - 2] + src[i + 2][j + 2])
                        + c21
                            * (src[i - 2][j - 1] + src[i - 2][j + 1] * c21 + src[i - 1][j - 2] + src[i - 1][j + 2]
                               + src[i + 1][j - 2] + src[i + 1][j + 2] + src[i + 2][j - 1] + src[i + 2][j + 1])
                        + c20 * (src[i - 2][j] + src[i][j - 2] + src[i][j + 2] + src[i + 2][j])
                        + c11 * (src[i - 1][j - 1] + src[i - 1][j + 1] + src[i + 1][j - 1] + src[i + 1][j + 1])
                        + c10 * (src[i - 1][j] + src[i][j - 1] + src[i][j + 1] + src[i + 1][j]) + c00 * src[i][j];
...
template<class T>
void gauss5x5div(T **RESTRICT src, T **RESTRICT dst, T **RESTRICT divBuffer, const int W, const int H, float sigma)
{
...
#pragma omp for schedule(dynamic, 16) nowait
  ...
  for (int i = 2; i < H - 2; ++i) {
    ...
    for (int j = 2; j < W - 2; ++j) {
      const float val = c21
                          * (src[i - 2][j - 1] + src[i - 2][j + 1] + src[i - 1][j - 2] + src[i - 1][j + 2]
                             + src[i + 1][j - 2] + src[i + 1][j + 2] + src[i + 2][j - 1] + src[i + 2][j + 1])
                        + c20 * (src[i - 2][j] + src[i][j - 2] + src[i][j + 2] + src[i + 2][j])
                        + c11 * (src[i - 1][j - 1] + src[i - 1][j + 1] + src[i + 1][j - 1] + src[i + 1][j + 1])
                        + c10 * (src[i - 1][j] + src[i][j - 1] + src[i][j + 1] + src[i + 1][j]) + c00 * src[i][j];
...
#pragma omp for
  ...
  for (int i = 0; i < H; i++) {
    for (int j = 1; j < W - 1; j++) { temp[j] = (T)(c1 * (src[i][j - 1] + src[i][j + 1]) + c0 * src[i][j]); }
...
#pragma omp single
  ...
  for (int i = W - (W % 8); i < W; i++) {
    for (int j = 1; j < H - 1; j++) { temp[j] = c1 * (src[j - 1][i] + src[j + 1][i]) + c0 * src[j][i]; }

    dst[0][i] = src[0][i];

    for (int j = 1; j < H - 1; j++) { dst[j][i] = temp[j]; }
...
#pragma omp for
  ...
  for (int i = 0; i < W; i++) {
    for (int j = 1; j < H - 1; j++) { temp[j] = (T)(c1 * (src[j - 1][i] + src[j + 1][i]) + c0 * src[j][i]); }
...
#pragma omp single
  ...
  for (int i = H - (H % 4); i < H; i++) {
    ...
    float temp2Wm1 = src[i][W - 1] + M[0][0] * (tmp[W - 1][0] - src[i][W - 1])
                     + M[0][1] * (tmp[W - 2][0] - src[i][W - 1]) + M[0][2] * (tmp[W - 3][0] - src[i][W - 1]);
    float temp2W = src[i][W - 1] + M[1][0] * (tmp[W - 1][0] - src[i][W - 1]) + M[1][1] * (tmp[W - 2][0] - src[i][W - 1])
                   + M[1][2] * (tmp[W - 3][0] - src[i][W - 1]);
    float temp2Wp1 = src[i][W - 1] + M[2][0] * (tmp[W - 1][0] - src[i][W - 1])
                     + M[2][1] * (tmp[W - 2][0] - src[i][W - 1]) + M[2][2] * (tmp[W - 3][0] - src[i][W - 1]);
    ...
    for (int j = 0; j < W; j++) { dst[i][j] = tmp[j][0]; }
  }
}
#endif
...
template<class T> void gaussHorizontal(T **src, T **dst, const int W, const int H, const double sigma)
{
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) { M[i][j] /= (1.0 + b1 - b2 + b3) * (1.0 + b2 + (b1 - b3) * b3); }
...
#pragma omp for
  ...
  for (int i = 0; i < H; i++) {
    ...
    double temp2Wm1 = src[i][W - 1] + M[0][0] * (temp2[W - 1] - src[i][W - 1])
                      + M[0][1] * (temp2[W - 2] - src[i][W - 1]) + M[0][2] * (temp2[W - 3] - src[i][W - 1]);
    double temp2W = src[i][W - 1] + M[1][0] * (temp2[W - 1] - src[i][W - 1]) + M[1][1] * (temp2[W - 2] - src[i][W - 1])
                    + M[1][2] * (temp2[W - 3] - src[i][W - 1]);
    double temp2Wp1 = src[i][W - 1] + M[2][0] * (temp2[W - 1] - src[i][W - 1])
                      + M[2][1] * (temp2[W - 2] - src[i][W - 1]) + M[2][2] * (temp2[W - 3] - src[i][W - 1]);
    ...
    for (int j = 0; j < W; j++) { dst[i][j] = (T)temp2[j]; }
  }
}
...
#pragma omp single
  ...
  for (int i = W - (W % 8); i < W; i++) {
    ...
    float temp2Hm1 = src[H - 1][i] + M[0][0] * (tmp[H - 1][0] - src[H - 1][i])
                     + M[0][1] * (tmp[H - 2][0] - src[H - 1][i]) + M[0][2] * (tmp[H - 3][0] - src[H - 1][i]);
    float temp2H = src[H - 1][i] + M[1][0] * (tmp[H - 1][0] - src[H - 1][i]) + M[1][1] * (tmp[H - 2][0] - src[H - 1][i])
                   + M[1][2] * (tmp[H - 3][0] - src[H - 1][i]);
    float temp2Hp1 = src[H - 1][i] + M[2][0] * (tmp[H - 1][0] - src[H - 1][i])
                     + M[2][1] * (tmp[H - 2][0] - src[H - 1][i]) + M[2][2] * (tmp[H - 3][0] - src[H - 1][i]);
    ...
    for (int j = 0; j < H; j++) { dst[j][i] = tmp[j][0]; }
  }
}
#endif
...
#ifdef __SSE2__
template<class T>
void gaussVerticalSsemult(T **RESTRICT src, T **RESTRICT dst, const int W, const int H, const float sigma)
{
  double b1, b2, b3, B, M[3][3];
  calculateYvVFactors<double>(sigma, b1, b2, b3, B, M);
...
#pragma omp for nowait
  ...
  for (unsigned int i = 0; i < static_cast<unsigned>(std::max(0, W - numcols + 1)); i += numcols) {
    ...
    for (int k = 0; k < numcols; k++) {
      temp2Hm1[k] = src[H - 1][i + k] + M[0][0] * (temp2[H - 1][k] - src[H - 1][i + k])
                    + M[0][1] * (temp2[H - 2][k] - src[H - 1][i + k]) + M[0][2] * (temp2[H - 3][k] - src[H - 1][i + k]);
      temp2H[k] = src[H - 1][i + k] + M[1][0] * (temp2[H - 1][k] - src[H - 1][i + k])
                  + M[1][1] * (temp2[H - 2][k] - src[H - 1][i + k]) + M[1][2] * (temp2[H - 3][k] - src[H - 1][i + k]);
      temp2Hp1[k] = src[H - 1][i + k] + M[2][0] * (temp2[H - 1][k] - src[H - 1][i + k])
                    + M[2][1] * (temp2[H - 2][k] - src[H - 1][i + k]) + M[2][2] * (temp2[H - 3][k] - src[H - 1][i + k]);
    }

    for (int k = 0; k < numcols; k++) {
      dst[H - 1][i + k] = temp2[H - 1][k] = temp2Hm1[k];
      dst[H - 2][i + k] = temp2[H - 2][k] =
        B * temp2[H - 2][k] + b1 * temp2[H - 1][k] + b2 * temp2H[k] + b3 * temp2Hp1[k];
      dst[H - 3][i + k] = temp2[H - 3][k] =
        B * temp2[H - 3][k] + b1 * temp2[H - 2][k] + b2 * temp2[H - 1][k] + b3 * temp2H[k];
    }

    for (int j = H - 4; j >= 0; j--) {
      for (int k = 0; k < numcols; k++) {
        dst[j][i + k] = temp2[j][k] =
          B * temp2[j][k] + b1 * temp2[j + 1][k] + b2 * temp2[j + 2][k] + b3 * temp2[j + 3][k];
      }
    }
  }
...
#pragma omp single
  ...
  for (int i = W - (W % numcols); i < W; i++) {
    ...
    double temp2Hm1 = src[H - 1][i] + M[0][0] * (temp2[H - 1][0] - src[H - 1][i])
                      + M[0][1] * (temp2[H - 2][0] - src[H - 1][i]) + M[0][2] * (temp2[H - 3][0] - src[H - 1][i]);
    double temp2H = src[H - 1][i] + M[1][0] * (temp2[H - 1][0] - src[H - 1][i])
                    + M[1][1] * (temp2[H - 2][0] - src[H - 1][i]) + M[1][2] * (temp2[H - 3][0] - src[H - 1][i]);
    double temp2Hp1 = src[H - 1][i] + M[2][0] * (temp2[H - 1][0] - src[H - 1][i])
                      + M[2][1] * (temp2[H - 2][0] - src[H - 1][i]) + M[2][2] * (temp2[H - 3][0] - src[H - 1][i]);
  ...
  T **buffer2 = nullptr)
{
  ...
  if (useBoxBlur) {
    ...
    boxblur(src, dst, sizes[0], W, H, true);

    for (int i = 1; i < n; i++) { boxblur(dst, dst, sizes[i], W, H, true); }
  } else {
    if (sigma < GAUSS_SKIP) {
...
#endif
      if (src != dst) {
        for (int i = 0; i < H; ++i) { memcpy(dst[i], src[i], W * sizeof(T)); }
      }
