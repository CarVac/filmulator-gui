...
template<typename T> constexpr T pow5(T x) { return x * pow4(x); }
...
template<typename T, typename... ARGS> constexpr const T &min(const T &a, const T &b, const ARGS &...args)
{
...
template<typename T, typename... ARGS> constexpr const T &max(const T &a, const T &b, const ARGS &...args)
{
  return max(max(a, b), max(args...));
}

template<typename T> constexpr const T &LIM(const T &val, const T &low, const T &high)
{
  return max(low, min(val, high));
}

template<typename T> constexpr T LIM01(const T &a) { return max(T(0), min(a, T(1))); }

template<typename T> constexpr T CLIP(const T &a) { return LIM(a, static_cast<T>(0), static_cast<T>(MAXVALI)); }
...
template<typename T> inline T norm1(const T &x, const T &y) { return std::abs(x) + std::abs(y); }

template<typename T> inline T norm2(const T &x, const T &y) { return std::sqrt(x * x + y * y); }

template<typename T> inline T norminf(const T &x, const T &y) { return max(std::abs(x), std::abs(y)); }
...
constexpr std::uint8_t uint16ToUint8Rounded(std::uint16_t i) { return ((i + 128) - ((i + 128) >> 8)) >> 8; }

template<typename T> constexpr bool OOG(const T &val, const T &high = T(MAXVALI))
{
  return (val < T(0)) || (val > high);
}

template<typename T> void setUnlessOOG(T &out, const T &val)
{
  if (!OOG(out)) { out = val; }
}


template<typename T> bool invertMatrix(const std::array<std::array<T, 3>, 3> &in, std::array<std::array<T, 3>, 3> &out)
{
  ...
  if (std::abs(det) < 1.0e-10) { return false; }
...
template<typename T>
std::array<std::array<T, 3>, 3> dotProduct(const std::array<std::array<T, 3>, 3> &a,
...
template<typename T> std::array<T, 3> dotProduct(const std::array<std::array<T, 3>, 3> &a, const std::array<T, 3> &b)
{
  std::array<T, 3> res;

  for (int i = 0; i < 3; ++i) {
    res[i] = 0;
    for (int k = 0; k < 3; ++k) { res[i] += a[i][k] * b[k]; }
  }
