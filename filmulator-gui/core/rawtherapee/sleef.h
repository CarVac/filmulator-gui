...
__inline double xfabs(double x) { return longBitsToDouble(0x7fffffffffffffffLL & doubleToRawLongBits(x)); }
...
__inline double upper(double d) { return longBitsToDouble(doubleToRawLongBits(d) & 0xfffffffff8000000LL); }
...
__inline double xasin(double d) { return mulsign(atan2k(xfabs(d), sqrt((1 + d) * (1 - d))), d); }
...
__inline float xfabsf(float x) { return intBitsToFloat(0x7fffffffL & floatToRawIntBits(x)); }
...
__inline float xatan2f(float y, float x)
{
  ...
  r = mulsignf(r, x);
  if (xisinff(x) || x == 0)
    r = rtengine::RT_PI_F / 2 - (xisinff(x) ? (signf(x) * (float)(rtengine::RT_PI_F * .5f)) : 0);
...
__inline float xasinf(float d) { return mulsignf(atan2kf(fabsf(d), sqrtf((1.0f + d) * (1.0f - d))), d); }
