...
template<typename T>
void upscaleBilinear1D(const matrix<T> input, matrix<T> &output, const int outNumCols, const bool interleaved);
...
void downscale_and_crop(const matrix<float> &input,
    ...
    const double overallScaleFactor =
      max(double(inputXSize) / double(outputXSize), double(inputYSize) / double(outputYSize));
...
#pragma omp parallel for shared(output)
    ...
    if (integerScaleFactor != 1) { downscaleDivisible1D(bilinearX, bothX, integerScaleFactor, true); }
...
#pragma omp parallel for shared(output)
        for (int i = 0; i < outputNumRows; i++)
            for (int j = 0; j < outputNumCols; j++) {
                double sum = 0;
                for (int k = j * scaleFactor; k < (j + 1) * scaleFactor; k++) { sum += input(i, k); }
...
#pragma omp parallel for shared(output)
    for (int i = 0; i < outputNumRows; i++) {
        for (int j = 0; j < outputNumCols; j++) {
            ...
            if (interleaved)
                for (int c = 0; c < 3; c++) {
                    output(i, 3 * j + c) = startWeight * double(input(i, 3 * inputStart + c))
                                           + endWeight * double(input(i, 3 * inputEnd + c));
                }
            else
