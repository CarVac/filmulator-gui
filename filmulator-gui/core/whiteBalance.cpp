...
void temp_to_XYZ(float const temp, float &X, float &Y, float &Z)
{
  ...
  if (temp < 4000) {
    temp_x =
      -0.2661239e9 * pow(temp, -3.0f) + -0.2343589e6 * pow(temp, -2.0f) + 0.8776956e3 * pow(temp, -1.0f) + 0.179910f;
  } else {
    temp_x =
      -3.0258469e9 * pow(temp, -3.0f) + 2.1070379e6 * pow(temp, -2.0f) + 0.2226347e3 * pow(temp, -1.0f) + 0.24039f;
  }

  if (temp < 2222) {
    temp_y = -1.10638140f * pow(temp_x, 3.0f) + -1.34811020f * pow(temp_x, 2.0f) + 2.18555832f * temp_x + -0.20219683f;
  } else if (temp < 4000) {
    temp_y = -0.95494760f * pow(temp_x, 3.0f) + -1.37418593f * pow(temp_x, 2.0f) + 2.09137015f * temp_x + -0.16748867f;
  } else {
    temp_y = 3.08175800f * pow(temp_x, 3.0f) + -5.87338670f * pow(temp_x, 2.0f) + 3.75112997f * temp_x + -0.37001483f;
  }
...
void matrixVectorMult(const float r, const float g, const float b, float &x, float &y, float &z, const float mat[3][3])
{
...
void matrixMatrixMult(const float left[3][3], const float right[3][3], float (&output)[3][3])
{
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      output[i][j] = 0;
      for (int k = 0; k < 3; k++) { output[i][j] += left[i][k] * right[k][j]; }
    }
  }
}

constexpr float srgb2xyz[3][3] = { { 0.4124564, 0.3575761, 0.1804375 },
  { 0.2126729, 0.7151522, 0.0721750 },
  { 0.0193339, 0.1191920, 0.9503041 } };

// Computes the raw color space multipliers.
void whiteBalancePreMults(const float temperature,
  const float tint,
  const float cam_xyz[3][3],
  float &rMult,
  float &gMult,
  float &bMult)
{
...
void whiteBalancePostMults(float temperature,
  float tint,
  float camToRgb[3][3],
  float rCamMul,
  float gCamMul,
  ...
  float &bMult)
{
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < 3; j++) { cout << camToRgb[i][j] << " "; }
    cout << endl;
  }
  ...
  XYZ_to_sRGB(XIllum, YIllum, ZIllum, rIllum, gIllum, bIllum);
  XYZ_to_sRGB(XBase, YBase, ZBase, rBase, gBase, bBase);
...
int daylightScore(const int illuminant)
{
  ...
  case 21:
    return 0;// D65 (6500k)
  case 19:
    return 1;// standard light C (6774k)
  case 1:
    return 2;//"daylight"
  case 9:
    return 3;//"fine weather"
  case 4:
    return 4;//"flash"
  case 12:
    return 5;// daylight fluorescent (5700-7100k)
  case 20:
    return 6;// D55 (5500k)
  case 10:
    return 7;//"cloudy weather"
  case 23:
    return 8;// D50 (5000k)
  case 13:
    return 9;// day white fluorescent (4600-5400k)
  case 18:
    return 10;// standard light B (4874k)
  case 2:
    return 11;// fluorescent
  case 22:
    return 12;// D75 (7500k)
  case 11:
    return 13;//"shade"
  case 14:
    return 14;// cool white fluorescent (3900-4500k)
  case 15:
    return 15;// white fluorescent (3200-3700k)
  case 17:
    return 16;// stardard light A (2855k)
  case 3:
    return 17;//"tungsten"
  case 24:
    return 18;//"ISO studio tungsten"
  case 255:
    return 19;//"other light source"
  case 0:
    return 20;// unknown
  default:
    return 21;
...
float wbDistance(const array<float, 2> tempTint,
  const float cam_xyz[3][3],
  const float rMulTarget,
  const float gMulTarget,
  const float bMulTarget)
{
  float rMult, gMult, bMult;
...
void whiteBalance(matrix<float> &input,
  ...
  float rCamMul,
  float gCamMul,
  float bCamMul,// what the camera asked for and is already applied
  float rPreMul,
  float gPreMul,
  float bPreMul,// reference for camera's daylight wb
  float expCompMult)
{
  float rMult, gMult, bMult;
  whiteBalancePostMults(
    temperature, tint, cam2rgb, rCamMul, gCamMul, bCamMul, rPreMul, gPreMul, bPreMul, rMult, gMult, bMult);
...
#pragma omp for schedule(dynamic) nowait
    for (int i = 0; i < nRows; i++) {
      for (int j = 0; j < nCols; j += 3) {
        output(i, j) = max(
          0.0f, transform[0][0] * input(i, j) + transform[0][1] * input(i, j + 1) + transform[0][2] * input(i, j + 2));
        output(i, j + 1) = max(
          0.0f, transform[1][0] * input(i, j) + transform[1][1] * input(i, j + 1) + transform[1][2] * input(i, j + 2));
        output(i, j + 2) = max(
          0.0f, transform[2][0] * input(i, j) + transform[2][1] * input(i, j + 1) + transform[2][2] * input(i, j + 2));
      }
    }
  }
}
