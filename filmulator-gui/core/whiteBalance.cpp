...
void matrixVectorMult(const float r,
                      const float g,
                      const float b,
                      float      &x,
                      float      &y,
...
#pragma omp for schedule(dynamic) nowait
        for (int i = 0; i < nRows; i++) {
            for (int j = 0; j < nCols; j += 3) {
                output(i, j) = max(0.0f,
                                   transform[0][0] * input(i, j) + transform[0][1] * input(i, j + 1)
                                     + transform[0][2] * input(i, j + 2));
                output(i, j + 1) = max(0.0f,
                                       transform[1][0] * input(i, j) + transform[1][1] * input(i, j + 1)
                                         + transform[1][2] * input(i, j + 2));
                output(i, j + 2) = max(0.0f,
                                       transform[2][0] * input(i, j) + transform[2][1] * input(i, j + 1)
                                         + transform[2][2] * input(i, j + 2));
            }
        }
    }
}
