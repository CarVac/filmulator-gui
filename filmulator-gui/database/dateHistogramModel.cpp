...
void DateHistogramModel::setQuery(const QSqlQuery &query, const int timezone)
{
    ...
    dateQuery.first();
    m_today = dateQuery.value(0).toFloat();
    ...
    if (dateQuery.first())
    {
        m_firstDay = dateQuery.value(0).toFloat();
    } else {
        m_firstDay = m_today;
    }

    //The row count of all the data we will eventually serve to the view.
    m_rowCount = 1 + floor(m_today - m_firstDay);
    ...
    if (m_dataVector.capacity() < m_rowCount*2)
    {
        m_dataVector.reserve(2*m_rowCount);
    }
    else
    {
    ...
    {
        m_dataVector[i*2] = m_firstDay + i;
        m_dataVector[i*2+1] = 0;
    }
    ...
    for (int i = 0; i < rowCount; i++)
    {
        ...
        int dayIndex = floor(julianDay-m_firstDay);
        int imageCount = queryModel.data(countModelIndex).toInt();
        m_dataVector[dayIndex*2+1] = imageCount;
    }
    endResetModel();
}
    ...
    {//It's one of our columns.
        ...
        else
        {
            ...
            {//The sixth column is the count.
                value = m_dataVector[index.row()*2 + 1];
            }
            else
            {
                value = 0;
