...
DBSuccess setupDB(QSqlDatabase *db)
{
  ...
  if (!dir.cd(dirstr)) {
    if (!dir.mkpath(dirstr)) {
      std::cout << "Could not create database directory: " << dirstr.toStdString() << std::endl;
  ...
  query.exec(
    ...
    ",STthumbWritten integer"//-1 for error, 0 for not written or
                             // invalidated, 1 for written
    ",STbigThumbWritten integer"// same as above
    ");");
  query.exec(
    "create index if not exists TimeIndex"
    " on SearchTable (STcaptureTime);");

  // 2. The table which carries more file info, like exifs and thumbnail
  // locations.
  query.exec(
  ...
  query.exec(
    "create table if not exists ProcessingTable ("
  ...
  // This will be of the same structure as ProcessingTable except for
  ...
  query.exec(
    "CREATE TABLE IF NOT EXISTS LensPrefs ("
    "ExifCamera varchar"
    ",ExifLens varchar"
    ",LensfunLens varchar"
    ",LensfunCa integer"
  ...
  query.prepare(
    "REPLACE INTO ProfileTable values "
    "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?"
    ",?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
  ...
  // Thickness of active layer (not necessarily the same units as above. I
  // think. Maybe. -CV)
  ...
  // Proportionality constant for growth of silver crystals relative to
  ...
  // Number of times the developer solution is mixed during the development
  // process
  ...
  // Constant that influences the extent of the diffusion; yes this is
  // redundant.
  ...
  // Constant that affects the ratio of intra-layer and layer-reservoir
  // diffusion; yes this is redundant.
  ...
  // How much to offset the exposure tone curve to the right while maintaining
  // the toe
  ...
  // where the ui point about which we adjust rotation is, relative to the image
  // dimensions
  ...
  // Because older versions would erroneously overwrite the file usage count on
  // setting a new location, we need to set them all to 1 SELECT COUNT(*),
  // FTfileID FROM FileTable INNER JOIN SearchTable WHERE STsourceHash=FTfileID
  // GROUP BY FTfileID;
  ...
  case 0:
    ...
    " "FROM integers a " "CROSS JOIN integers b " "CROSS JOIN integers c "
  ...
  query.exec("COMMIT TRANSACTION;");// finalize the transaction only after
                                    // writing the version.
