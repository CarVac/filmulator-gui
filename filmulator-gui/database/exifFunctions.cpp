...
{
  // const bool isCR3 = QString::fromStdString(fullFilename).endsWith(".cr3",
  // Qt::CaseInsensitive);
  ...
  // exiv2 and libraw read the image capture date as if the camera time zone is
  // the computer time zone
  ...
  QString::fromStdString(exifData["Exif.Image.DateTime"].toString()); cout <<
  "exifUtcTime Exif.Image.DateTime: " <<
  exifData["Exif.Image.DateTime"].toString() << endl; cout << "exifUtcTime
  Exif.Image.DateTimeOriginal: " <<
  exifData["Exif.Image.DateTimeOriginal"].toString() << endl; cout <<
  "exifUtcTime Exif.Image.DateTimeOriginal: " <<
  exifData["Exif.Image.DateTimeOriginal"].toString() << endl; if
  (exifDateTime.length()==0)
      {
          //leica DNG seems to not have DateTime
          exifDateTime =
  QString::fromStdString(exifData["Exif.Image.DateTimeOriginal"].toString());
      }

      cameraDateTime = QDateTime::fromString(exifDateTime, "yyyy:MM:dd
  hh:mm:ss"); } else { //we have to rely on libraw for CR3
...
{
  if (shutterSpeed == 0) { return QString("0/1"); }
...
{
  if (shutterSpeed == 0) { return Exiv2::Rational(0, 1); }
  // Two methods we'll have compete with one another
...
{
  if (floatIn < 0.01) { return Exiv2::Rational(0, 1); }
...
int exifRating(const std::string fullFilename)
{
  if (QString::fromStdString(fullFilename).endsWith(".cr3", Qt::CaseInsensitive)) {
    ...
    if (maker.compare("Canon") == 0) { return min(5, max(0, (int)xmpData["Xmp.xmp.Rating"].toInt64())); }
    return 0;
  }
}
...
QString identifyLens(const std::string fullFilename)
{
  ...
  if (lensModel.length() > 0) {
    ...
    if (lensList) {
      ...
      if (lensList[0]->Score > 25) { lensName = QString(lensList[0]->Model); }
      /*
      int i = 0;
      while (lensList[i])
      {
