...
QDateTime exifUtcTime(const std::string fullFilename, const int cameraTZ) {
    ...
    if (!isCR3) //we can use exiv2
    {
        ...
        cout << "exifUtcTime Exif.Image.DateTimeOriginal: " << exifData["Exif.Image.DateTimeOriginal"].toString() <<
    endl; cout << "exifUtcTime Exif.Image.DateTimeOriginal: " << exifData["Exif.Image.DateTimeOriginal"].toString() <<
    endl; if (exifDateTime.length()==0)
        {
