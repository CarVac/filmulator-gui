...
ImportModel::ImportModel(QObject *parent) : SqlModel(parent)
{
  ...
  rawNameFilters << "*.CR2" << "*.cr2" << "*.CR3" << "*.cr3" << "*.NEF" << "*.nef" << "*.DNG" << "*.dng" << "*.RW2"
                 << "*.rw2" << "*.IIQ" << "*.iiq" << "*.ARW" << "*.arw" << "*.PEF" << "*.pef" << "*.RAF" << "*.raf"
                 << "*.ORF" << "*.orf" << "*.SRW" << "*.srw";

  // Set up the files that it'll show in the file picker
  dirNameFilters << "Raw image files (*.CR2 *.cr2 *.CR3 *.cr3 *.NEF *.nef *.DNG *.dng *.RW2 *.rw2 *.IIQ *.iiq *.ARW "
                    "*.arw *.PEF *.pef *.RAF *.raf *.ORF *.orf *.SRW *.srw)";// << "All files (*)";

  // Set up the import worker thread
  ImportWorker *worker = new ImportWorker;
  worker->moveToThread(&workerThread);
  connect(this,
    SIGNAL(workForWorker(const QFileInfo,
      const int,
      const int,
      const QString,
      const QString,
      ...
      const bool)),
    worker,
    SLOT(importFile(const QFileInfo,
      const int,
      const int,
      const QString,
      const QString,
...
bool ImportModel::pathContainsDCIM(const QString dir, const bool notDirectory)
{
  ...
  } else if (dir.length() < 4)// so that / itself doesn't lead to reading the whole filesystem
  {
    ...
    for (int i = 0; i < dirList.size(); i++) {
      if (pathContainsDCIM(dirList.at(i).absoluteFilePath(), false)) { return true; }
    }
  }
  return false;
}
  ...
  const bool onlyCheck)
{
  ...
  for (int i = 0; i < rawNameFilters.size(); i++) {
    if (file.fileName().endsWith(rawNameFilters.at(i).mid(1))) { isReadableFile = true; }
  }
...
void ImportModel::importFileList(const QString name, const bool importInPlace, const bool replaceLocation)
{
  ...
  for (int i = 0; i < nameList.size(); i++) {
    ...
    if (Validity::invalid == validity) { break; }
  }
  if (Validity::valid == validity) {
    // If it's valid, we run it again, this time enqueuing them.
    for (int i = 0; i < nameList.size(); i++) { importFile(nameList.at(i), importInPlace, replaceLocation, false); }
    if (queue.size() > 0) {
      paused = false;
      startWorker(queue.front());
    }
...
QString ImportModel::importFileNow(const QString name, Settings *settingsObj)
{
  ...
  const QString searchID = worker->importFile(params.fileInfoParam,
    ...
    params.noThumbnail);
  delete worker;
  if (searchID != "") { emit enqueueThis(searchID); }
  return searchID;
}
...
{
  QMutexLocker locker(&mutex);
  if (changedST) { emit searchTableChanged(); }
...
{
  if (enqueue) { emit enqueueThis(STsearchID); }
}

void ImportModel::startWorker(const importParams params)
{
  if (!workerThread.isRunning()) { workerThread.start(QThread::LowPriority); }
