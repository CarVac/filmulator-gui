...
OrganizeModel::OrganizeModel(QObject *parent) : SqlModel(parent)
{
...
QString OrganizeModel::adaptableModelQuery(const bool searchIDOnly)
{
  ...
  if (ratingSort == 1) {
    queryString.append("SearchTable.STrating ASC, ");
  } else if (ratingSort == -1) {
    queryString.append("SearchTable.STrating DESC, ");
  }

  if (processedSort == 1) {
    queryString.append("SearchTable.STlastProcessedTime ASC, ");
  } else if (processedSort == -1) {
    queryString.append("SearchTable.STlastProcessedTime DESC, ");
  }

  if (importSort == 1) {
    queryString.append("SearchTable.STimportTime ASC, ");
  } else if (importSort == -1) {
    queryString.append("SearchTable.STimportTime DESC, ");
  }
...
void OrganizeModel::incrementRating(const QString searchID, const int ratingChange)
{
  ...
  if (rating < 0) {
    ...
    if (ratingChange > 0) { rating = -1 + ratingChange; }
