...
QSqlQuery QueueModel::modelQuery() {
    ...
    queryString.append("INNER JOIN SearchTable ON SearchTable.STsearchID=QueueTable.QTsearchID ");
...
void QueueModel::enQueue(const QString searchID) {
    ...
    if (success) { alreadyInQueue = query.value(0).toInt() == 1; }
    ...
    if (alreadyInQueue) {
        ...
        query.prepare(
          "INSERT OR IGNORE INTO QueueTable "
...
void QueueModel::batchEnqueue(const QString searchQuery) {
    ...
    while (query.next()) { searchIDList << query.value(0).toString(); }
    ...
    if (newMaxIndex != maxIndex) { cout << "queue index mismatch!" << endl; }
