...
lf_db_return getAndSaveFile(std::string url)
{
  ...
  if (curl) {
    ...
    if (f) {
      ...
      if (res != CURLE_OK) { result = LENSFUN_DBUPDATE_RETRIEVE_RETRIEVEFAILED; }
      fclose(f);
    } else
      result = LENSFUN_DBUPDATE_RETRIEVE_FILEOPENFAILED;
    curl_easy_cleanup(curl);
  } else
    result = LENSFUN_DBUPDATE_RETRIEVE_INITFAILED;
...
lf_db_return lensfun_dbcheck(int version, std::string dbpath, std::string dburl)
{
  ...
  for (int i = 0; i < int(fields.size()); i++) {
    if (atoi(fields[i].c_str()) == dbversion) { versionavailable = true; }
  }
  ...
  // check version_x/timestamp.txt against timestamp, if timestamp is <=, tell user the database is already at the
  // current version
  struct stat b;
...
lf_db_return lensfun_dbupdate(int version, std::string dbpath, std::string dburl)
{
  ...
  if (stat(dbdir.c_str(), &b) == 0) {
    ...
    int ts = atoi(buffer.str().c_str());
    if (ts >= timestamp) {
      // err(string_format("local timestamp: %d >= server timestamp: %d; local database is the most current
  ...
  if (!isdirectory) {
    ...
    for (int i = 0; i < int(flist.size()); i++) { remove(flist[i].c_str()); }
  }
