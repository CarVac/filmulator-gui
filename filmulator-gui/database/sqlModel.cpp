...
int SqlModel::columnCount(const QModelIndex &parent) const { return queryModel.columnCount(parent); }

int SqlModel::rowCount(const QModelIndex &parent) const { return queryModel.rowCount(parent); }

QHash<int, QByteArray> SqlModel::roleNames() const { return m_roleNames; }
...
void SqlModel::refreshAll() { emit dataChanged(createIndex(0, 0), createIndex(rowCount(), columnCount())); }
...
// When there's an update (not insertion or deletion), it should change everywhere. Unless that affects sorting.
