...
int main(int argc, char *argv[]) {
    ...
    cout << QDateTime::currentDateTime().toString("hh:mm:ss.zzz ").toStdString() << "creating qqmlapplicationengine"
         << endl;
    ...
    if (translator.load(QLocale(), QLatin1String("filmulator-gui"), QLatin1String("_"),
    QLatin1String("qrc:///tl/translations/")))
    {
    ...
    */
    ...
    cout << QDateTime::currentDateTime().toString("hh:mm:ss.zzz ").toStdString()
         << "assigning parametermanager property" << endl;
    engine.rootContext()->setContextProperty("paramManager", paramManager);
    cout << QDateTime::currentDateTime().toString("hh:mm:ss.zzz ").toStdString() << "connecting parametermanager"
         << endl;
    QObject::connect(
      paramManager, SIGNAL(updateTableOut(QString, int)), switchboard, SLOT(updateTableIn(QString, int)));
    ...
    QObject::connect(
      organizeModel, SIGNAL(updateTableOut(QString, int)), switchboard, SLOT(updateTableIn(QString, int)));
