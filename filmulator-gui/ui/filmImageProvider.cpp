...
FilmImageProvider::FilmImageProvider(ParameterManager *manager)
  : QObject(0), pipeline(HighCache, WithHisto, HighQuality), quickPipe(HighCache, WithHisto, PreviewQuality),
...
void FilmImageProvider::shufflePipelines() {
    ...
    } else if (newID == prevID) // swap new and old
    {
        ...
        // paramManager->getValidityWhenCanceled() << endl;
...
void FilmImageProvider::customWB(const float xCoord, const float yCoord) {
    ...
    if (useQuickPipe) {
        ...
        quickPipe.sampleWB(
          xCoord, yCoord, rotation, cropHeight, cropAspect, cropVoffset, cropHoffset, red, green, blue);
    } else {
