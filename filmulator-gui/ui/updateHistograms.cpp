...
#pragma omp parallel for reduction(+ : lHist[ : 128]) reduction(+ : rHist[ : 128]) reduction(+ : gHist[ : 128]) \
  reduction(+ : bHist[ : 128])
    for (int i = 0; i < image.nr(); i = i + 1) {
        for (int j = 0; j < image.nc(); j = j + 3) {
            unsigned short luma = ushort(0.2126 * image(i, j) + 0.7151 * image(i, j + 1) + 0.0722 * image(i, j + 2));
...
void FilmImageProvider::updateFloatHistogram(Histogram           &hist,
    ...
    // oddH and oddV should be 0.5 if they're odd, 0 otherwise
    const float oddH = (!(int(round((imWidth - width) / 2.0)) * 2 == (imWidth - width))) * 0.5f;
    const float oddV = (!(int(round((imHeight - height) / 2.0)) * 2 == (imHeight - height))) * 0.5f;
  ...
  reduction(+ : bHist[ : 128])
    for (int i = startY; i < endY; i = i + 1) {
        for (int j = startX * 3; j < endX * 3; j = j + 3) {
            float luma = 0.2126f * image(i, j) + 0.7151f * image(i, j + 1) + 0.0722f * image(i, j + 2);
...
void FilmImageProvider::updateHistRaw(const matrix<float> &image,
...
void FilmImageProvider::updateHistPreFilm(const matrix<float> &image,
                                          const float          maximum,
                                          const int            rotation,
                                          const float          cropHeight,
                                          const float          cropAspect,
                                          const float          cropHoffset,
                                          const float          cropVoffset) {
    updateFloatHistogram(preFilmHist, image, maximum, rotation, cropHeight, cropAspect, cropHoffset, cropVoffset);
    emit histPreFilmChanged();
    return;
}

void FilmImageProvider::updateHistPostFilm(const matrix<float> &image,
                                           const float          maximum,
                                           const int            rotation,
                                           const float          cropHeight,
                                           const float          cropAspect,
                                           const float          cropHoffset,
                                           const float          cropVoffset) {
    updateFloatHistogram(postFilmHist, image, maximum, rotation, cropHeight, cropAspect, cropHoffset, cropVoffset);
